...
/* gengzongjie transplant for unify switch 2013-5-2 */
...
/* Begin:gengzongjie added for sending arp packets to assoc sta in wifipos function 2013-12-31 */
#define ICMP_DATA_LEN 45

int WIFIPOS_PING_SEND( struct ieee80211vap *vap )
{
    ...
    struct ieee80211_node *ni_next = NULL;
    struct ieee80211_node_table *nt = &ic->ic_sta;
    u_int16_t pkt_len = sizeof(struct ieee80211_frame) + sizeof(wlan_ng_prism2_header) + 128;
    u_int16_t pkt_hdr_len = pkt_len - ICMP_DATA_LEN - 16;
	int ret = 0;
	
    TAILQ_FOREACH_SAFE(ni, &nt->nt_node, ni_list, ni_next) {
        ieee80211_ref_node(ni);
        if ((ni->ni_associd == 0) || (vap != ni->ni_vap))  {
	    ...
	    skb = (struct sk_buff *)wbuf_alloc(ic->ic_osdev, WBUF_TX_DATA, pkt_len);
	    if (skb == NULL) {
			ieee80211_free_node(ni);
	        return -ENOMEM;
	    }
	    ...
	    skb->dev = ((osif_dev *)vap->iv_ifp)->netdev;
	    skb->protocol = htons(0x0019);


	    skb_reserve(skb , pkt_hdr_len);

	    data = (u_int8_t *)skb->data; 
	    memset(data, 0x00, ICMP_DATA_LEN);
	    skb->len = ICMP_DATA_LEN; 
	        
	    icmpheader = (struct icmphdr *)skb_push(skb, sizeof(struct icmphdr));
	    icmpheader->type = ICMP_ECHO;
	    icmpheader->code = 0;
	    icmpheader->un.echo.id = htons(0x0200);
	    icmpheader->un.echo.sequence = htons(seq);
	    icmpheader->checksum = 0;
	    icmpheader->checksum = (__sum16)ip_compute_csum((void *)icmpheader, 53 );


	    iphdr = (struct iphdr *)skb_push(skb, sizeof(struct iphdr));
	    put_unaligned(0x45, (unsigned char *)iphdr);
		iphdr->tos = 0;
	    put_unaligned(htons(skb->len), &(iphdr->tot_len));
	    iphdr->id       = 0;	//htons(id++);
		iphdr->frag_off = 0;	// htons(0x4000);
		iphdr->ttl      = 64;
		iphdr->protocol = IPPROTO_ICMP;
	    put_unaligned(ni->saddr, &(iphdr->saddr));
		put_unaligned(ni->daddr, &(iphdr->daddr)); 
	    ...
	    llc->llc_snap.ether_type = ETHERTYPE_IP;
	    ...
	    *(u_int16_t *)wh->i_dur = 0;
	    ...
	    IEEE80211_ADDR_COPY(wh->i_addr1, ni->ni_macaddr); //real -det = sta mac
	    IEEE80211_ADDR_COPY(wh->i_addr2, vap->iv_myaddr); //real - src = vap ssid
	    IEEE80211_ADDR_COPY(wh->i_addr3, ni->ni_srcmac); //other addr = gw addr
...
#if 0
	    ret = wlan_vap_send(vap, skb);
		if (ret < 0)
		{
			printk("WIFIPOS: send ping packets to assoc sta failed.\n");
			return ret;
		}
#endif
	}
...
EXPORT_SYMBOL(WIFIPOS_PING_SEND);
/* Begin:gengzongjie added end */
...
//			if(printk_ratelimit())
//			printk("%s----------tos:%d------------\n",__func__,ip->tos);
			tid = tos_to_wmm_map((ip->tos >> 2), vap);//Begin:Modified by zhaoyang1 for optimize map of tos vlan & 80211e up 2013-01-28
//			if(printk_ratelimit())
//			printk("%s----------ac:%d------------\n",__func__,ac);
			ac = TID_TO_WME_AC(tid);//Begin:Modified by zhaoyang1 for optimize map of tos vlan & 80211e up 2013-01-28*/
		}else{
			tid = (wbuf_classify(wbuf) & 0x7);
			ac = TID_TO_WME_AC(tid);
		}
...
/* gengzongjie transplant for unify switch 2013-5-2 */
#if SUPPORT_UNIFY_SWITCH
/*Begin : modify by liuqiang for finding netdevice by station MAC for unify switch 802.3 tunnel*/
struct net_device * capwap_getvap_from_ni(unsigned char * nimac)
{
	...
	for(i = 0; i < 2; i ++)
	{
		sprintf(buff, "wifi%d",i);
		#if LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,22)
    		tmp_dev = dev_get_by_name(buff);
		#else
    		tmp_dev = dev_get_by_name(&init_net,buff);
		#endif
		if(tmp_dev)
		{
			struct ieee80211com *ic_tmp = (struct ieee80211com *)ath_netdev_priv(tmp_dev);
			ni = ieee80211_find_node(&ic_tmp->ic_sta, nimac);
			
	        if (ni != NULL) 
			{
	            ieee80211_unref_node(&ni);
	            dev_put(tmp_dev);
				dev = ((osif_dev *)ni->ni_vap->iv_ifp)->netdev;
				return dev;
	        }
		}
	}
...
EXPORT_SYMBOL(capwap_getvap_from_ni);
/*End : modify by liuqiang for finding netdevice by station MAC for unify switch 802.3 tunnel*/
#endif
/* gengzongjie transplant end */
...
/* gengzongjie added for traffic balance to verify the length of assocresp from ac */
#ifndef IEEE80211_VERIFY_LENGTH(_len,_minlen)
...
#define OSIF_TO_NETDEV(_osif) (((osif_dev *)(_osif))->netdev)
	...
	/* gengzongjie added for traffic balance to analyse assocresp from ac */ 
	int subtype;
	u_int8_t *frm = NULL;
	u_int8_t *efrm = NULL;
	u_int16_t status;
	...
	/* gengzongjie transplant for unify switch 2013-5-2 */
	/*Begin: modify by liuqiang for setting protected frame bit for unify switch  2012-11-14*/
	 
#if SUPPORT_UNIFY_SWITCH
	if(thinap && (htons(0x0019) == wbuf->protocol))
	{
		struct llc *llc_type = NULL;
    	u_short frametype = 0;
		u_int16_t hdrspace = 0;
		...
		if (IEEE80211_FC0_TYPE_DATA == type) {
			...
			if (llc_type != NULL) {
        		if (wbuf->len >= LLC_SNAPFRAMELEN 
					&& llc_type->llc_dsap == LLC_SNAP_LSAP 
        			&& llc_type->llc_ssap == LLC_SNAP_LSAP 
        			&& llc_type->llc_control == LLC_UI 
        			&& llc_type->llc_snap.org_code[0] == 0 
        			&& llc_type->llc_snap.org_code[1] == 0 
        			&& llc_type->llc_snap.org_code[2] == 0) {
			...
			if(IEEE80211_VAP_IS_PRIVACY_ENABLED(vap) 
				&& (frametype != __constant_htons(ETHERTYPE_PAE))
				&& (frametype != __constant_htons(ETHERTYPE_WAI))) {
                wh->i_fc[1] |= 0x40U; // set protected frame bit
			}
		}
	}
#endif

	/*End: modify by liuqiang for setting protected frame bit for unify switch 2012-11-14*/
	/* gengzongjie transplant end */
...
//	if(stop_transmit &&(vap->iv_opmode == IEEE80211_M_HOSTAP)){  //caizhibang changed for WDS root 
...
//	}
    ...
    if (thinap) {
        if (wbuf->protocol == htons(0x0019))
        {
			...
			/* gengzongjie added for traffic balance to verify the assocresp from ac */
			subtype = wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK;
			/* gengzongjie added end */
            ...
            else 
            {
                ...
                if (vap->iv_traffic_balance) //Modified by zhaoyang1 for optimizing traffic balance based wlan 2013-06-19
                {
					...
					/* gengzongjie added for traffic balance to analyse the status code in assocresp from ac */
					if ((type == IEEE80211_FC0_TYPE_MGT) && 
						...
						reassoc = (subtype == IEEE80211_FC0_SUBTYPE_REASSOC_RESP) ? 1 : 0;
						if (IEEE80211_STATUS_TOOMANY == status) {
						...
						ieee80211_send_assocresp(ni, reassoc, status);
						ieee80211_free_node(ni);
						ni = NULL;
						goto bad;
					}
...
#if 0
        else if(vap->vap_splitmac)
        {
/*<Begin : caizhibang modify for autelan 802.3 tunnel 2012-5-25 */        
            if(vap->vap_splitmac == 2)
            	{
                  if(wbuf->protocol == htons(0x0030))    
                  	{
			     goto out;
		       }
	  	}
...
/*End:Modify by zhaoyang1 for data forward base users 2012-11-24 */
		...
		else if (vap->vap_splitmac == 1)// tunnel mode
		{
			/*Begin:Modified by zhaoyang1 for shifting down four-way-handshake 2013-01-28*/
			struct ether_header *eh = (struct ether_header *)wbuf_header(wbuf);
			u_int8_t packet_type = *(u_int8_t *)(wbuf->data + sizeof(struct ether_header) + 1); 
			if (( __constant_htons(ETHERTYPE_PAE) == eh->ether_type) && (3 == packet_type)) {
				;
			}
			else {
				goto bad;
		...
		else if (vap->vap_splitmac == 2)//802.3 tunnel
		{
			struct ether_header *eh = (struct ether_header *)wbuf_header(wbuf);
			u_int8_t packet_type = *(u_int8_t *)(wbuf->data + sizeof(struct ether_header) + 1); 
			if ((wbuf->protocol == htons(0x0030)) || 
				((( __constant_htons(ETHERTYPE_PAE) == eh->ether_type)) && (3 == packet_type))) {
				;
			}
			else {
				goto bad;
			}
			/*End:Modified by zhaoyang1 for shifting down four-way-handshake 2013-01-28*/
		}
		else if(vap->vap_splitmac == 3)//ubc mode 
		{
			;
		}
		
		else if(vap->vap_splitmac == 0)//zdx  mod local mode 
		{
			;
		}
		else // error mode 
...
/*<Begin: yanggs add for vlan wds 2012-11-25*/
	if ((vap->iv_opmode == IEEE80211_M_HOSTAP) && (((ni->ni_vid > 0) && (ni->ni_vid <= VLAN_VID_MASK)) || (ni == vap->iv_bss)))
...
send80211:
    if (thinap && (wbuf->protocol == htons(0x0019))) {
        ...
        if ((ni != vap->iv_bss)&&!ieee80211_node_is_authorized(ni) && is_data)
        {  //Modified by zhaoyang1 for optimizing traffic balance based wlan 2013-06-19
        	/*zhaoyang modify for thought the dhcp packet*/			 
...
/*End:Added by duanmingzhe for thinap*/
    ...
    {   
           ...
           (vap->vap_tl_down_srtcm_vap.sr_cir > 0))
        {
            ...
            // In these two situations, we send it right away.
        ...
        {   
                 ...
                 * after the node_is_paused check above, pause operation is ref counted 
                 */  
...
/* gengzongjie added for traffic balance to release the above define */ 
#undef IEEE80211_VERIFY_LENGTH(_len,_minlen);
/* gengzongjie added end */
#undef OSIF_TO_NETDEV(_osif)
}
...
//	int datalen = 0;/*suzhaoyu add for traffic correction*/
    ...
    if (thinap) 
    {
        if (wbuf->protocol == htons(0x0019)) 
        {
            ...
            /*Begin: added by wangjia for ONLINEBUG-754, 802.11 tunnel QoS correction. 2012-12-21. */
            {
            	int ac, tid;
            	int headerlen, qos_start, llc_start;
            	
            	u_int8_t *qos;
            	u_int8_t header[64]; // Max Len: FC(2)+DUR(2)+ADDR*3(6*3)+SEQ(2)+ADDR4(6)+QoS(2)+LLC(8, DSAP(1)+SSAP(1)+SNAP(6))
            	
            	int hasqos = ((type == IEEE80211_FC0_TYPE_DATA) && 
                              (subtype == IEEE80211_FC0_SUBTYPE_QOS)) ? 1 : 0;
            	...
            	{ 
            		...
            		{
            			qos = ((struct ieee80211_qosframe_addr4 *)wh)->i_qos;
            			headerlen = sizeof(struct ieee80211_frame_addr4);
            		}
            		else
            		{
            			qos = ((struct ieee80211_qosframe *)wh)->i_qos;
            			headerlen = sizeof(struct ieee80211_frame);
            		}

            		qos_start  = headerlen;
            		headerlen += sizeof(struct ieee80211_qoscntl);

            		/*Align header */
            		if (ic->ic_flags & IEEE80211_F_DATAPAD) {
            			headerlen = roundup(headerlen, sizeof(u_int32_t));
            		} 
            		
            		llc_start  = headerlen;
            		headerlen += sizeof(struct llc);
            		/*Now we have the real 802.11 header length of skb.*/
            		
            		/**
            		 * In 802.11 tunnel mode, skb's header is 802.11 format. 
            		 * So we just change its header length(offset), makes it looks 
            		 * like a 802.3 packet. In order to use ieee80211_classify 
            		 * to find out skb's priority.
            		 **/
            		skb_pull(wbuf, headerlen);
            		skb_push(wbuf, sizeof(struct ether_header));
            		/* calculate priority so driver can find the tx queue */
            		if (ieee80211_classify(ni, wbuf)) {
            			IEEE80211_DPRINTF(vap, IEEE80211_MSG_OUTPUT,
            							  "%s: discard, classification failure (%s)\n",
            		...
            		/*Revert to the original 802.11 packet. */
            		skb_pull(wbuf, sizeof(struct ether_header));
            		skb_push(wbuf, headerlen);

            		wh = (struct ieee80211_frame *)wbuf->data;
                    
            		if(!addqos) {         // station doesn't support QoS, Remove QoS Field
            			OS_MEMZERO(header, sizeof(header));
            			OS_MEMCPY(header, wbuf_header(wbuf), qos_start);
            			skb_pull(wbuf, llc_start);

            			/*Align header */
            			if (ic->ic_flags & IEEE80211_F_DATAPAD) {
            				qos_start = roundup(qos_start, sizeof(u_int32_t));
            			} 
            			skb_push(wbuf, qos_start);
            			OS_MEMCPY(wbuf_header(wbuf), header, qos_start);

                        wh = (struct ieee80211_frame *)wbuf->data;	
            			// Clear Qos bit, change frame type form 'QoS Data' to 'Data'.
            			wh->i_fc[0] &= (IEEE80211_FC0_SUBTYPE_DATA | ~IEEE80211_FC0_SUBTYPE_MASK);  
            			subtype = IEEE80211_FC0_SUBTYPE_DATA;
            		}
            		else if (addqos) {    // station support QoS, Update QoS Field    			
            			if ( (vap->iv_ccx_evtable && vap->iv_ccx_evtable->wlan_ccx_check_msdu_life) &&
            				!(vap->iv_ccx_evtable->wlan_ccx_check_msdu_life(vap->iv_ccx_arg, tid))){
            				IEEE80211_DPRINTF(vap, IEEE80211_MSG_OUTPUT, "%s, MSDU Lifetime Exceed discard QOS traffic\n", __func__);
            				goto bad;
            			...
            			qos[1] = 0; 
            			/* Fill in the sequence number from the TID sequence space. */
            			*(u_int16_t *)&wh->i_seq[0] = htole16(ni->ni_txseqs[tid] << IEEE80211_SEQ_SEQ_SHIFT);
            			ni->ni_txseqs[tid]++;
                    ...
                    qos_start  = headerlen;
            		headerlen += sizeof(struct llc);
                    OS_MEMCPY(header, wbuf->data, qos_start);
                    ...
                    skb_push(wbuf, headerlen);
                    OS_MEMCPY(wbuf->data, header, qos_start);
                    
                    wh = (struct ieee80211_frame *)wbuf->data;
                    qos = ((struct ieee80211_qosframe *)wbuf->data)->i_qos;                    
            ...
            /*End: added by wangjia, 802.11 tunnel QoS correction. 2012-12-14. */
...
/* gengzongjie transplant from service proveder 2013-5-2 */
    //if (addhtc) {    //deleted by zhanghu to kill the cpe of bug when wmm is 0 in the 11n mode TDDEVA-289
    if (addqos && addhtc) {    //added by zhanghu to kill the cpe of bug when wmm is 0 in the 11n mode TDDEVA-289
/* gengzongjie transplant end */
