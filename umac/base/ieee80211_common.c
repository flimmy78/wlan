...
static u_int8_t id(u_int8_t channel_num)
{
	if (channel_num < 36) {
		return channel_num;
	} else if (channel_num >= 36 && channel_num <= 64) {
		return (channel_num / 4 + 5); 
	} else if (channel_num >= 100 && channel_num <= 140) {
		return (channel_num / 4 - 3);
	} else {
		return (channel_num / 4 - 4); 
	}
}

static void
autelan_add_to_blacklist(struct ieee80211com *ic , 
							struct ieee80211_autelan_rogue_ap_blacklist *ik) 
{
	struct autelan_rogue_ap_channel_list *chan_list = NULL;
	struct autelan_rogue_ap_list *rogue_ap = NULL;

	chan_list = &ic->ic_autelan_rogue_ap_channel_list[id(ik->arg1)];
	spin_lock(&chan_list->autelan_rogue_ap_lock);
	rogue_ap = chan_list->rogue_ap;

	while (rogue_ap) {
		if (0 == memcmp(ik->arg2, rogue_ap->bssid, IEEE80211_ADDR_LEN)) {
			printk("AP is already in the list\n");
			break;
		}
		rogue_ap = rogue_ap->next;
	} 
	
	if (NULL == rogue_ap) { //ap is not in the blacklist
		struct autelan_rogue_ap_list *new = 
				(struct autelan_rogue_ap_list *)kmalloc(sizeof(struct autelan_rogue_ap_list), GFP_KERNEL);
		if (NULL == new) {
			printk("Kmalloc is failed!\n");
			return ;
		}
		memset(new, 0, sizeof(struct autelan_rogue_ap_list));
		new->next = NULL;
		new->prev = NULL;
		memcpy(new->bssid, ik->arg2, IEEE80211_ADDR_LEN);

		if (NULL == chan_list->rogue_ap) { //blacklist is empty yet
			new->prev = new;
			chan_list->rogue_ap = new;
		} else {
			chan_list->rogue_ap->prev->next = new;
			new->prev = chan_list->rogue_ap->prev;
			chan_list->rogue_ap->prev = new;
		}
	}
	spin_unlock(&chan_list->autelan_rogue_ap_lock);
}


static void
autelan_remove_from_blacklist(struct ieee80211com *ic , 
							struct ieee80211_autelan_rogue_ap_blacklist *ik) 
{
	...
	if (NULL == rogue_ap) {
		...
		if (NULL == rogue_ap) { //ap is not in the blacklist
			printk("AP isn't in the blacklist!\n");
		} else { //ap is in the blacklist
			if (chan_list->rogue_ap == rogue_ap) { //the first node
				if (NULL != rogue_ap->next) { //more than one nodes
					rogue_ap->next->prev = rogue_ap->prev;
				} 
				chan_list->rogue_ap = rogue_ap->next;
			} else if (NULL == rogue_ap->next) { //the last node
				chan_list->rogue_ap->prev = rogue_ap->prev;
				rogue_ap->prev->next = NULL;
			} else { //in the middle
				rogue_ap->next->prev = rogue_ap->prev;
				rogue_ap->prev->next = rogue_ap->next;
			}
			rogue_ap->next = NULL;
			rogue_ap->prev = NULL;
			kfree(rogue_ap);
...
static void
autelan_clr_blacklist(struct ieee80211com *ic) 
{
	struct autelan_rogue_ap_channel_list *chan_list = NULL;
	struct autelan_rogue_ap_list *rogue_ap = NULL;
	u_int8_t i = 0;
	...
	for (; i < 64; i++) {
		chan_list = &ic->ic_autelan_rogue_ap_channel_list[i];
		spin_lock(&chan_list->autelan_rogue_ap_lock);
		while (chan_list->rogue_ap) {
			rogue_ap = chan_list->rogue_ap;
			chan_list->rogue_ap = rogue_ap->next;
			if (NULL != rogue_ap->next) {
				rogue_ap->next->prev = rogue_ap->prev;
			}
			rogue_ap->next = NULL;
			rogue_ap->prev = NULL;
...
static void 
autelan_display_blacklist(struct ieee80211com *ic , 
							struct ieee80211_autelan_rogue_ap_blacklist *ik)
{
	struct autelan_rogue_ap_channel_list *chan_list = NULL;
	struct autelan_rogue_ap_list *rogue_ap = NULL;
	...
	while (NULL != rogue_ap) {
		memcpy(ik->arg2+i*6, rogue_ap->bssid, IEEE80211_ADDR_LEN);
		i++;
		rogue_ap = rogue_ap->next;
	}
	ik->arg1 = i;
...
ieee80211_ifattach(struct ieee80211com *ic, IEEE80211_REG_PARAMETERS *ieee80211_reg_parm)
{
	...
	*/
	...
	/*Begin:zhaoyang1 add for rogue ap*/	
	ic->ic_autelan_add_to_blacklist = autelan_add_to_blacklist;
	ic->ic_autelan_remove_from_blacklist = autelan_remove_from_blacklist;
	ic->ic_autelan_display_blacklist = autelan_display_blacklist;
	ic->ic_autelan_clr_blacklist = autelan_clr_blacklist;
	/*End: zhaoyang1 add for rogue ap*/
    return 0;
}
...
ieee80211_ifdetach(struct ieee80211com *ic)
{
	...
	/*zhaoyang1 add for rogue ap*/
	ic->ic_autelan_add_to_blacklist = NULL;
	ic->ic_autelan_remove_from_blacklist = NULL;
	ic->ic_autelan_display_blacklist = NULL;
	ic->ic_autelan_clr_blacklist = NULL;
	/*End: zhaoyang1 add for rogue ap*/
...
ieee80211_iter_vap_opmode(void *arg, struct ieee80211vap *vaphandle)
{
    ...
    case IEEE80211_M_HOSTAP:
        /*lijiyong add begin to fix the bug of bgscan cause sta can not assoc to ap*/
        if (vaphandle->iv_state == IEEE80211_S_RUN) {
