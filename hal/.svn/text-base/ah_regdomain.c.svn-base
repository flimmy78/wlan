...
getEepromRD(struct ath_hal *ah)
{
	...
	/*lisongbai modify for setCountryID=840 fail on PB44V2 2011-08-23 */
	u_int16_t  tmp_rd = AH_PRIVATE(ah)->ah_currentRD &~ WORLDWIDE_ROAMING_FLAG;
	if(tmp_rd == 0x30 || tmp_rd == 0 ) {
		if(ah->ah_countryCode == 156) {
			tmp_rd = 0x52;
		}
		else if(ah->ah_countryCode == 840) {
			tmp_rd = 0x3a;
		}
	}
	return tmp_rd;
...
#ifndef __APPLE__ /* Apple EEPROMs do not have the Midband bit set */
/*
...
#endif
	/*zhaoyang1 transplant from 717*/
    ah->ah_countryCode = cc;  //Added by duanmingzhe for can't setting countryID to 156 2011-08-24
    /*zhaoyang1 transplant end*/
    rd = getEepromRD(ah);
...
findCTLByCountryCode(HAL_CTRY_CODE countrycode, HAL_BOOL is2G)
{
    ...
    /*lijiyong add begin to debug the bug of ENAEX-34*/
    printk(KERN_DEBUG "%s: Debug ENAEX-34, the num of country in Rdt(%p) is %u\n", __func__, &Rdt, Rdt.allCountriesCt);
    /*lijiyong add end*/
...
#define CHANNEL_QUARTER_BW    5
    ...
    if (AH_PRIVATE(ah)->ah_countryCode == CTRY_DEFAULT) {
        ...
        if ((AH_PRIVATE(ah)->ah_currentRD == APL9_WORLD) ||
            (AH_PRIVATE(ah)->ah_currentRD == MKK8_MKKA))
