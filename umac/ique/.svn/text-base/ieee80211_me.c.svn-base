...
static int ieee80211_me_SnoopListGetEvnode(struct ieee80211vap *vap, u_int8_t *gaL2, u_int8_t *table, u_int16_t vlan_id)
{
    u_int32_t count = 0;
    u_int32_t idx = 0;
    ...
    KASSERT((table != NULL), ("table is NULL"));
    
    TAILQ_FOREACH(ni_tmp, &nt->nt_node, ni_list) {
        if( ni_tmp->ni_vap == vap && ni_tmp->ni_associd != 0) {
            
            IEEE80211_ADDR_COPY(&table[idx], ni_tmp->ni_macaddr);
            idx += IEEE80211_ADDR_LEN;
            count++;

            /*the information for debugging */
            if(vap->iv_me->me_debug & IEEE80211_ME_DBG_DEBUG){
                u_int8_t *mac = &table[idx - IEEE80211_ADDR_LEN];
...
ieee80211_me_SnoopConvert(struct ieee80211vap *vap, wbuf_t wbuf)
{
	...
	/* gengzongjie transplant the modification by zhanghu for converting multiply frame to umcast frame for every node 2013-4-28 */	
    /*Begin:Add by zhanghu for converting multiply frame to umcast frame for every node 2012-12-24*/
    if(vap->iv_me->me_to_Enode_enable & IEEE80211_ME_DBG_TO_NODE){
        newmaccnt = ieee80211_me_SnoopListGetEvnode(vap, eh->ether_dhost, &newmac[0][0],vlan_id);
    }else
    /*End:Add by zhanghu for converting multiply frame to umcast frame for every node 2012-12-24*/
	/* gengzongjie transplant end */
    ...
    do {	
        ...
        if (newmaccnt > 0 && vap->iv_me->mc_snoop_enable) {
            ...
            /*if umcast frame send to every node , not update the snoop list.*/
            if ( !(vap->iv_me->me_to_Enode_enable & IEEE80211_ME_DBG_TO_NODE) ){
            /*End:Add by zhanghu for converting multiply frame to umcast frame for every node 2012-12-24*/
			/* gengzongjie transplant end */
