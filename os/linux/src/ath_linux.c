...
static void autelan_attack_interfere_ap_fn(unsigned long data);
/*zhaoyang1 add end*/
...
ath_getstats(struct net_device *dev)
{
	...
	/* AUTELAN-Added-Begin : &tuqiang for wifiX data statistics 2014-07-04 */
	
	stats->rx_bytes = as->ast_rx_bytes;

	stats->tx_bytes = as->ast_tx_bytes + as->ast_be_xmit_bytes
		+ as->ast_tx_mgmt_bytes + ans->tx_bytes;
...
/* Begin:gengzj added for suppressing illegal ap 2013-11-27 */
	/*zhaoyang1 add start for interfere ap*/
	#if 0
	init_timer(&(ic->interfere_ap_attack));
	ic->interfere_ap_attack.function = autelan_attack_interfere_ap_fn;
	ic->interfere_ap_attack.data = (unsigned long) ic;
	ic->interfere_ap_attack.expires = jiffies + HZ / 100;
	#endif
	//add_timer(&(ic->interfere_ap_attack));
	/*zhaoyang1 add end*/
/* End:gengzj added end */
    return 0;
...
/*End:caizhibang add for channel utility>*/
	/*Begin:Added by zhaoyang1 for fixing ap reboot by uninstalling wlan driver 2013-05-31*/
	del_timer(&tl_dequeue_cache_timer);
	/*End:Added by zhaoyang1 for fixing ap reboot by uninstalling wlan driver 2013-05-31*/
...
enum {
	ATH_THINAP = 1,
	//ATH_TRAFFIC_BALANCE = 2, Modified by zhaoyang1 for optimizing traffic balance based wlan 2013-06-19
	/*Begin:Added by duanmingzhe for monitor process*/
...
/* Begin:gengzj added for rrm scan 2013-11-26 */
#if 0
	/*suzhaoyu add start iwlist ath scanning modify PCAPVXN-87*/
...
/* End:gengzongjie added end */
	...
	/*End:Added by duanmingzhe for 80211 cache*/
    AUTELAN_DRV_MONITOR_ENABLE =28,//AUTELAN:zhaoenjuan transplant for driver monitor switch 
	...
	ATH_MCAST_QUEUE_DEBUG = 40,	/*  added by huangtao for mcast queue 2013-5-24*/
    /*<Begin:caizhibang add for channel utility */
    ATH_CHANNEL_UTILITY_SWITCH = 41,
    ATH_5G_CHANNEL_UTILITY = 42,
    ATH_2G_CHANNEL_UTILITY = 43,
    ATH_CHANNEL_UTILITY_TIMER = 44,
    /*End:caizhibang add for channel utility>*/
...
/* Begin:gengzj added for rfid scan 2013-11-26 */
	ATH_SUPPORT_RFID_SCAN = 47,
/* End:gengzongjie added end */

/* Begin:gengzj added for rrm scan 2013-11-26*/
	ATH_IWLIST_SCAN_MODE = 48,
	ATH_IWLIST_SCAN_CHAN = 49,
	ATH_IWLIST_LISTEN_TIME_EACH_CHAN = 50,
/* End:gengzongjie added end */
/* Begin:gengzj added for suppressing illegal ap 2013-11-27 */


	/*zhaoyang1 add start for interfere ap*/
	ROGUE_AP_RESTRAIN  = 51,
	/*zhaoyang1 add end*/

/* End:gengzj added end */
/* Begin:gengzongjie added for sending arp packets to assoc sta in wifipos function 2013-12-31 */
	WIFIPOS_PING_SEND_ENABLE = 52,
/* End:gengzongjie added end */
/*AUTELAN-Begin:zhaoenjuan transplant for driver monitor proc varies*/
    BSTUCK_ENBALE =53,
	BMISS_CHECK = 54,
    AUTELAN_DRV_MONITOR_REBOOT=59,
    AUTELAN_DRV_HANG_REBOOT = 60,
/*AUTELAN-End:zhaoenjuan transplant for driver monitor proc varies*/

/* Begin:Add by chenxf for powersave performance 2014-06-05 */
	ATH_PS_LMAC = 61,
	ATH_PS_REORDER = 62,
	ATH_PS_DROP = 63,
	ATH_PS_FIFO_DETPH = 64,
/* End:Add by chenxf for powersave performance 2014-06-05 */

	/* AUTELAN-Added-Begin : &tuqiang for openessid */
	ATH_OPENESSID_DEBUG = 65,
	/* AUTELAN-Added-End : tuqiang@autelan.com */
};
...
ATH_SYSCTL_DECL(ath_sysctl_halparam, ctl, write, filp, buffer, lenp, ppos)
{
		...
		if(0 == ret)
		{
			switch ((long)ctl->extra2) {
				...
				case ATH_IWLIST_SCAN_ONEBYONE:
					...
					ic->ic_iwlist_scan_onebyone=val;
					/* End:gengzongjie added end */
					break;
				...
				/* Begin:gengzj added for suppressing illegal ap 2013-11-27 */
				/*zhaoyang1 add start for interfere ap*/
				case ROGUE_AP_RESTRAIN:
					ic->ic_rogue_ap_restrain = val;
					break;
				/*zhaoyang1 add end*/
				/* End:gengzj added end */
				...
				/*End: add for 5G priority by caizhibang  2012-09-27> */
	/*AUTELAN-Begin:zhaoenjuan transplant for drv monotor*/
				case BMISS_CHECK:
					bmiss_check = val;
				break;
				...
				break;
	/*AUTELAN-End:zhaoenjuan transplant for drv monotor*/
    ...
    /* End:Add by chenxf for powersave performance 2014-06-05 */
    			...
    			break;
                /*End:Added by zhanghu for increasing rate*/
                /*Begin: added by huangtao for mcast queue 2013-5-24*/
                case ATH_MCAST_QUEUE_DEBUG:
        			if(val >= 0 && val <= 3 )
        				mcast_queue_debug  = val;
    			break;
                /*End: added by huangtao for mcast queue 2013-5-24*/
                /*<Begin:caizhibang add for channel utility*/
                ...
                case ATH_CHANNEL_UTILITY_TIMER:
                        ...
                        break;
                /*End:caizhibang add for channel utility>*/       
				/* Begin:gengzongjie added for rfid scan 2013-11-26*/
				case ATH_SUPPORT_RFID_SCAN:
					rfid_scan = val;
				break;
				/* End:gengzongjie added end */

				/* Begin:gengzongjie added for rrm 2013-11-26*/
				case ATH_IWLIST_SCAN_MODE:
					ic->ic_iwlist_scan_mode = val;
					break;
				case ATH_IWLIST_SCAN_CHAN:
					ic->ic_iwlist_scan_chan = val;
					break;
				case ATH_IWLIST_LISTEN_TIME_EACH_CHAN:
					ic->ic_iwlist_listen_time_each_chan = val;
					break;
				/* End:gengzongjie added end */
     			/* Begin:gengzongjie added for sending arp packets to assoc sta in wifipos function 2013-12-31 */
                case WIFIPOS_PING_SEND_ENABLE:
                      wifipos_ping_send_enable = val;
                       break;
				/* End:gengzongjie added end */
	/*AUTELAN-Begin:zhaoenjuan transplant for drv monotor*/
                case BSTUCK_ENBALE:
	...
	/*AUTELAN-End:zhaoenjuan transplant for drv monotor*/
		...
		switch ((long)ctl->extra2) {
	        ...
	        case ATH_IWLIST_SCAN_ONEBYONE:
				...
				/* Begin:gengzongjie added for rrm 2013-11-26 */
		        val = ic->ic_iwlist_scan_onebyone;
			...
			/*End : add for 5G priority by caizhibang  2012-09-27> */
/*AUTELAN-Begin:zhaoenjuan transplant for drv monitor proc varible*/
			case BMISS_CHECK:
				val = bmiss_check;
				break;
			...
			case AUTELAN_DRV_HANG_REBOOT:
                  ...
                  break;  
/*AUTELAN-End:zhaoenjuan transplant for drv monitor proc varible*/
			...
			/* Begin:gengzongjie added for rrm 2013-11-26 */
			case ATH_IWLIST_SCAN_MODE:
				val = ic->ic_iwlist_scan_mode;
				break;
			case ATH_IWLIST_SCAN_CHAN:
				val = ic->ic_iwlist_scan_chan;
				break;
			case ATH_IWLIST_LISTEN_TIME_EACH_CHAN:
				val = ic->ic_iwlist_listen_time_each_chan;
				break;
			/* End:gengzongjie added end */
			/* Begin:gengzongjie added for sending arp packets to assoc sta in wifipos function */
			case  WIFIPOS_PING_SEND_ENABLE:
				  val = wifipos_ping_send_enable;
				   break;
			/* End:gengzongjie added end */
...
static const ctl_table ath_sysctl_template[] = {
...
#endif
    /*suzhaoyu add end*/
	/*AUTELAN-Begin: zhaoenjuan transplant for when check to many bmiss reboot the device */
	{ .ctl_name     = CTL_AUTO,
	...
	/*AUTELAN-End:zhaoenjuan transplant for when check to many bmiss reboot the device */
...
/* Begin:gengzj added for suppressing illegal ap 2013-11-27 */
	/*zhaoyang1 add start for interfere ap*/
	{ .ctl_name     = CTL_AUTO,
      .procname     = "rogue_ap_restrain",
      .mode         = 0644,
      .proc_handler = ath_sysctl_halparam,
       .extra2       = (void *)ROGUE_AP_RESTRAIN,
     },
	/*zhaoyang add end*/
/* End:gengzj added end */
    ...
    /*Begin: added by huangtao for mcast queue 2013-5-24*/
    { .ctl_name = CTL_AUTO,
      .procname     = "mcast_queue_debug",
      .mode         = 0644,
      .proc_handler = ath_sysctl_halparam,
      .extra2   = (void *)ATH_MCAST_QUEUE_DEBUG,
    },
    /*End: added by huangtao for mcast queue 2013-5-24*/
    /*<Begin:caizhibang add for channel utility*/
    { .ctl_name = CTL_AUTO,
	...
	{ .ctl_name 	= 	CTL_AUTO,
	  ...
	  .mode			=	0644,
	  .proc_handler =	ath_sysctl_halparam,
	  .extra2		=	(void *)ATH_SUPPORT_RFID_SCAN,
	},
	/* End:gengzj added end */

	/* Begin:gengzj added for rrm 2013-11-26*/
	{ .ctl_name = CTL_AUTO,
      .procname     = "iwlist_scan_mode",
      .mode         = 0644,
      .proc_handler = ath_sysctl_halparam,
      .extra2   = (void *)ATH_IWLIST_SCAN_MODE,
    },
    { .ctl_name = CTL_AUTO,
      .procname     = "iwlist_scan_chan",
      .mode         = 0644,
      .proc_handler = ath_sysctl_halparam,
      .extra2   = (void *)ATH_IWLIST_SCAN_CHAN,
    },
    { .ctl_name = CTL_AUTO,
      .procname     = "iwlist_listen_time_each_chan",
      .mode         = 0644,
      .proc_handler = ath_sysctl_halparam,
      .extra2   = (void *)ATH_IWLIST_LISTEN_TIME_EACH_CHAN,
    },
	/* End:gengzj added end */

	/* Begin:gengzongjie added for sending arp packets to assoc sta in wifipos function 2014-1-2 */
    { .ctl_name         = CTL_AUTO,
      .procname         = "wifipos_ping_send_enable",
      .mode             = 0644,
      .proc_handler     = ath_sysctl_halparam,
      .extra2           = (void *)WIFIPOS_PING_SEND_ENABLE,  
    },
	/* End:gengzongjie added end */

    { .ctl_name         = CTL_AUTO,
      .procname         = "openessid_debug",
      .mode             = 0644,
      .proc_handler     = ath_sysctl_halparam,
      .extra2           = (void *)ATH_OPENESSID_DEBUG,  
    },
...
static void autelan_attack_interfere_ap_fn(unsigned long data)
{
	...
	vap = TAILQ_FIRST(&ic->ic_vaps);
	if (NULL == vap) {
		//printk("__%s__vap is null__%d__\n", __func__, __LINE__);
		mod_timer(&ic->interfere_ap_attack, jiffies + HZ);
		return ;
	}
	osifp = (osif_dev  *)vap->iv_ifp;
	
	if (0 == ic->ic_ath_sta_interfere) {
		mod_timer(&ic->interfere_ap_attack, jiffies + HZ);
		return ;
	}
	
	if (0 == ic->ic_interfere_ap_channel_num) { // There is no interfere ap now
		mod_timer(&ic->interfere_ap_attack, jiffies + HZ);
		return ;
	}

#ifdef ATH_SUPPORT_P2P
	opmode = osifp->os_opmode;
#else
		   ...
		   check count to 0 to pass beacon count check to reduce total scan time. */
	{
		...
		wlan_set_default_scan_parameters(vap,scan_params,opmode,true,true,
										connected_flag,true,0,ssid_list,0);
	}
#else
	//wlan_set_default_scan_parameters(vap,scan_params,opmode,true,true,true,true,n_ssid,ssid_list,0);
	wlan_set_default_scan_parameters(vap,scan_params,opmode,true,true,true,true,0,ssid_list,0);
#endif
	
	scan_params->flags = IEEE80211_SCAN_PASSIVE | IEEE80211_SCAN_ALLBANDS;
	scan_params->type = IEEE80211_SCAN_BACKGROUND;
	/* XXX tunables */
	scan_params->min_dwell_time_passive = 15;
	scan_params->max_dwell_time_passive = 25;
	
	scan_params->num_channels = ic->ic_interfere_ap_channel_num;
	scan_params->chan_list = OS_MALLOC(osifp->os_handle, sizeof(u_int32_t) * scan_params->num_channels, GFP_KERNEL);
	for(i=0; i<ic->ic_interfere_ap_channel_num; i++) {
		chan = ic->ic_chan_scan_cache + i;
		scan_params->chan_list[i] = chan->chan_num;
	}
	...
	OS_FREE(scan_params);
	mod_timer(&ic->interfere_ap_attack, jiffies + HZ * ic->ic_scanning_switch);
}
/*zhaoyang1 add end*/
/* End:gengzj added end */
#endif
