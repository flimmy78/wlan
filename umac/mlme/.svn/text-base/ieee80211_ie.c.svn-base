 ...
 */
u_int8_t *
ieee80211_add_rates(u_int8_t *frm, const struct ieee80211_rateset *rs, struct ieee80211com *ic)
{
    int nrates;
    ...
    u_int8_t j = 0;
    u_int8_t index = 0;
    int support_nrates = 0;
    u_int8_t support_rates[IEEE80211_RATE_MAXSIZE] = {0};
#ifndef  FOR_BIT
#define FOR_BIT(parm)    (sizeof(parm)*8)
    ...
    if(ic->ic_rate_mask & ((u_int64_t)1 << (FOR_BIT(ic->ic_rate_mask) - 1))){
        ...
        for(i = 0; i < IEEE80211_RATE_MAXSIZE; i++){
            ...
            for(j = 0; j < max; j++){
                if( rate_index[index][j] != 0 && (rs->rs_rates[i] & IEEE80211_RATE_VAL) == rate_index[index][j]){
                    break;
                }
            }
            if(max == j){
            ...
            if( 0 == ((ic_rate_mask >> j) & 0x01))
            {
                continue;
            }
            else
            {
                support_rates[support_nrates] = rate_index[index][j];
                if((ic_rate_mask >> (j+max)) & 0x01){
                    support_rates[support_nrates] |= IEEE80211_RATE_BASIC;
                }
                else{
                    support_rates[support_nrates] &= ~IEEE80211_RATE_BASIC;
                }
    ...
    *frm++ = IEEE80211_ELEMID_RATES;
    ...
    /*Begin:zhanghu added for beacon_frame rate support 2011-10-25*/
	if(ic->ic_rate_mask & ((u_int64_t)1 << (FOR_BIT(ic->ic_rate_mask) -1 )))
	{
		if (support_nrates > IEEE80211_RATE_SIZE){
			support_nrates = IEEE80211_RATE_SIZE;
        }
		*frm++ = support_nrates;
		memcpy(frm,support_rates, support_nrates);
		return frm + support_nrates;
	}
    /*End:zhanghu added for beacon_frame rate support*/
 ...
 */
u_int8_t *
ieee80211_add_xrates(u_int8_t *frm, const struct ieee80211_rateset *rs, struct ieee80211com *ic)
{
    
    /*add a parameter named ic by zhanghu for control rate*/
    /*
    ...
    if(ic->ic_rate_mask & ((u_int64_t)1 << (FOR_BIT(ic->ic_rate_mask) - 1))){
        ...
        for(i = 0; i < IEEE80211_RATE_MAXSIZE; i++){
			...
			if(support_nrates > IEEE80211_RATE_SIZE){
				...
				*frm++ = support_nrates - IEEE80211_RATE_SIZE;
	
				OS_MEMCPY(frm, support_rates + IEEE80211_RATE_SIZE, support_nrates - IEEE80211_RATE_SIZE);
		...
		/*End:zhanghu added for beacon_frame rate support*/
    else if (rs->rs_nrates > IEEE80211_RATE_SIZE) {
...
/* gengzongjie added for traffic balance to add ie in probreq */
u_int8_t *
ieee80211_add_band_rssi(u_int8_t *frm, struct ieee80211com *ic, struct ieee80211_rx_status *rs)
{
	static const u_int8_t oui[3] = { AUTE_OUI_BYTES }; 
	...
	*frm = RSSI_OUI_SUBTYPE;
	frm++;
	*frm = 0x01;
	frm++;
	*frm = (u_int8_t)rs->rs_rssi;
	return 0;
}
...
/*End: added by zhanghu for incresing rate*/
    ...
    case 1:
        ...
        if(ic->ic_mcs_mask & (1 << (FOR_BIT(ic->ic_mcs_mask) - 1))){
            rx_mcs[IEEE80211_RX_MCS_1_STREAM_BYTE_OFFSET] = ((u_int8_t)ic->ic_mcs_mask & IEEE80211_RX_MCS_ALL_NSTREAM_RATES);    
        }
        /*End: added by zhanghu for incresing rate*/
    ...
    case 2:
        ...
        if(ic->ic_mcs_mask & (1 << (FOR_BIT(ic->ic_mcs_mask) - 1))){
            rx_mcs[IEEE80211_RX_MCS_1_STREAM_BYTE_OFFSET] = ((u_int8_t)(ic->ic_mcs_mask) & IEEE80211_RX_MCS_ALL_NSTREAM_RATES);    
            rx_mcs[IEEE80211_RX_MCS_2_STREAM_BYTE_OFFSET] = ((u_int8_t)(ic->ic_mcs_mask >> 8) & IEEE80211_RX_MCS_ALL_NSTREAM_RATES);
        }    
        /*End: added by zhanghu for incresing rate*/
    ...
    case 3:
        ...
        if(ic->ic_mcs_mask & (1 << (FOR_BIT(ic->ic_mcs_mask) - 1))){
            rx_mcs[IEEE80211_RX_MCS_1_STREAM_BYTE_OFFSET] = ((u_int8_t)(ic->ic_mcs_mask) & IEEE80211_RX_MCS_ALL_NSTREAM_RATES);    
            rx_mcs[IEEE80211_RX_MCS_2_STREAM_BYTE_OFFSET] = ((u_int8_t)(ic->ic_mcs_mask >> 8) & IEEE80211_RX_MCS_ALL_NSTREAM_RATES);
            rx_mcs[IEEE80211_RX_MCS_3_STREAM_BYTE_OFFSET] = ((u_int8_t)(ic->ic_mcs_mask >> 16) & IEEE80211_RX_MCS_ALL_NSTREAM_RATES);
        }    
        /*End: added by zhanghu for incresing rate*/
