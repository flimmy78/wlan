...
static void list_stations_all(const char *ifname);
...
/*pengruofeng--add start for sta association time 2011-6-2*/
#if  defined(WNDAP350) || defined(WNAP550) || defined(AQ2000N) || defined(XJAP93) || defined(XHAP93) || defined(TDPB44_02) || defined(XJPB44_02)
#define HZ 1000
#else
#define HZ 250
#endif

#define SECS(n) (n / HZ) % 60
#define MINS(n) ((n / HZ) / 60) % 60
#define HRS(n)  (n / HZ) / 3600
/*pengruofeng--add end 2011-6-2*/
...
vap_create(struct ifreq *ifr)
{
	...
	if (s < 0)
	{
		...
		return;//AUTELAN--Added by duanmingzhe fix bug from coverity check
	}
	...
	if (memcmp(oname, ifr->ifr_name, IFNAMSIZ) != 0)
		printf("%s\n", ifr->ifr_name);
	close(s); //AUTELAN--Added by duanmingzhe fix bug from coverity check "resource leak"
}
...
#define LIST_STATION_ALLOC_SIZE 24*1024
	...
	do {
		...
		vp = (u_int8_t *)(si+1);
		{
			...
			printf("RXSEQ ");
			for(i=0;i<18;i++)
				printf("%6d",si->isi_rxseqs[i]>>4);
			printf("\n");
			printf("TXSEQ ");
			for(i=0;i<17;i++)
				printf("%6d",(si->isi_txseqs[i] & 0xfff));
			printf("\n\n");
		}
...
#define LIST_STATION_ALLOC_SIZE 24*1024
    ...
    u_int32_t txrate;
	/*zhaoyang1 transplant from 717*/
	u_int32_t rxrate = 0;  /*pengruofeng--add for rxrate 2011-6-2*/
	char a_time[9];	/*pengruofeng--add for sta association time 2011-6-2*/
	/*zhaoyang1 transplant end*/
	...
	if (s < 0){
		free(buf);
		err(1, "socket(SOCK_DRAGM)");
		return; //AUTELAN--Added by duanmingzhe for fix error from coverity check
	}

	(void) memset(&iwr, 0, sizeof(iwr));
	/*zhaoyang1 transplant from 717*/
	(void) memset(&a_time, '\0', sizeof(a_time));	/*pengruofeng--add for sta association time 2011-6-2*/
	/*zhaoyang1 transplant end*/
	(void) strncpy(iwr.ifr_name, ifname, sizeof(iwr.ifr_name));
	iwr.ifr_name[sizeof(iwr.ifr_name) - 1] = '\0';
	iwr.u.data.pointer = (void *) buf;
	...
	/*zhaoyang1 transplant from 717*/
	printf("%-17.17s %4s %4s %6s  %6s  %4s %6s %8s %8s %6s %6s %6s %8s %10s %s\n"	/*pengruofeng--modify for sta association time and rxrate 2011-6-2*/
		, "ADDR"
		, "AID"
		, "CHAN"
		, "TxRATE"
		...
		, "ERP"
		, "STATE"
		, "A_TIME"	/*pengruofeng--add for sta association time 2011-6-2*/
	    , "HTCAPS"
	);
	/*zhaoyang1 transplant end*/
	cp = buf;
	do {
		...
		sprintf(a_time, "%02d:%02d:%02d", HRS(si->isi_assoc_time), MINS(si->isi_assoc_time), SECS(si->isi_assoc_time));
		/*pengruofeng--add end 2011-6-2*/
		printf("%17s %4u %4d %6dM %6dM %4d %6d %8d %8d %6s %6s %6x %8x %10s %s"	/*pengruofeng --modify for sta association time 2011-6-2*/
			, ieee80211_ntoa(si->isi_macaddr)
			, IEEE80211_AID(si->isi_associd)
			, ieee80211_mhz2ieee(si->isi_freq)
			, txrate 
			, rxrate  /*pengruofeng--add for rxrate 2011-6-2*/
			, si->isi_rssi
			, si->isi_inact
			, si->isi_txseqs[0]
			, si->isi_rxseqs[0]
...
list_scan(const char *ifname)
{
	...
	do {
		...
		printf("%-14.*s  %s  %3d  %3dM %2d:%-2d  %3d %-4.4s"
			...
			/*zhaoyang1 transplant from 717*/
			,sr->isr_freq                //ieee80211_mhz2ieee(sr->isr_freq) suzhaoyu modified
			/*zhaoyang1 transplant end*/
...
#define	N(a)	(sizeof(a)/sizeof(a[0]))
    ...
    //AUTELAN-Begin:Added by duanmingzhe fix bug from coverity check "wrong sizeof argument"
	//memset(iwr, 0, sizeof(iwr));
	memset(iwr, 0, sizeof(struct iwreq));
	//AUTELAN-Begin:Added by duanmingzhe fix bug from coverity check "wrong sizeof argument"
	strncpy(iwr->ifr_name, ifname, IFNAMSIZ);
