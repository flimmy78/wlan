...
#ifdef VOW_TIDSCHED
static INLINE void ath_tx_queue_tid(struct ath_softc *sc, struct ath_txq *txq, struct ath_atx_tid *tid);
#else
...
/*Start: Add by zhanghu to statistics of every frame by the rate*/
static int
ath_ni_statistics_rate_for_tx_frame(struct ath_buf *bf)
{
    struct ath_node *an = (struct ath_node *) bf->bf_node;  
    struct ieee80211_node *ni = (struct ieee80211_node*)an->an_node;
    u_int8_t    rateCode = 0;
    u_int8_t    dot11Rate = 0;
    

    rateCode = ni->ni_tx_cur_rateCode;
    dot11Rate = ni->ni_tx_cur_dot11Rate;

    if((rateCode & MCS0)){
        (ni->ni_tx_mcs_count[rateCode & ~MCS0])++;
    }else if(2 == dot11Rate){
        (ni->ni_tx_rate_index[0].count)++;
    }else if(4 == dot11Rate){
        (ni->ni_tx_rate_index[1].count)++;
    }else if(11 == dot11Rate){
        (ni->ni_tx_rate_index[2].count)++;
    }else if(22 == dot11Rate){
        (ni->ni_tx_rate_index[3].count)++;
    }else{
        u_int8_t    left = 0;
        u_int8_t    right = 7;
        ...
        while(left <= right && left != 0xFF && right != 0xFF){
            if(ni->ni_tx_rate_index[middle + 4].dot11Rate < dot11Rate){
                left = middle + 1;
            }else if(ni->ni_tx_rate_index[middle + 4].dot11Rate > dot11Rate){
                right = middle - 1;
            }else{
                (ni->ni_tx_rate_index[middle + 4].count)++;
                break; 
            }
        ...
        if(left > right || left == 0xFF || right == 0xFF){
            printk("%s:%d is not found rateCode=0x%x\n", __func__, dot11Rate, rateCode);
            return -1;
        }   
    }
...
static int
ath_vap_statistics_rate_for_tx_frame(struct ath_buf *bf)
{
    struct ath_node *an = (struct ath_node *) bf->bf_node;  
    struct ieee80211_node *ni = (struct ieee80211_node*)an->an_node;
    ...
    if((rateCode & MCS0)){
        (vap->iv_tx_mcs_count[rateCode & ~MCS0])++;
    }else if(2 == dot11Rate){
        (vap->iv_tx_rate_index[0].count)++;
    }else if(4 == dot11Rate){
        (vap->iv_tx_rate_index[1].count)++;
    }else if(11 == dot11Rate){
        (vap->iv_tx_rate_index[2].count)++;
    }else if(22 == dot11Rate){
        (vap->iv_tx_rate_index[3].count)++;
    }else{
        u_int8_t    left = 0;
        u_int8_t    right = 7;
        u_int8_t    middle = (left + right)/2;
        ...
        while(left <= right && left != 0xFF && right != 0xFF){
            if(vap->iv_tx_rate_index[middle + 4].dot11Rate < dot11Rate){
                left = middle + 1;
            }else if(vap->iv_tx_rate_index[middle + 4].dot11Rate > dot11Rate){
                right = middle - 1;
            }else{
                (vap->iv_tx_rate_index[middle + 4].count)++;
                break; 
            }
            middle = (left + right)/2;
        }
...
static int
ath_vap_statistics_rate_for_tx_aggr_frame(struct ath_buf *bf, struct ath_buf *bf_first)
{
    struct ath_node *an_first = (struct ath_node *) bf_first->bf_node;  
    struct ieee80211_node *ni_first = (struct ieee80211_node*)an_first->an_node;    
    struct ieee80211vap *vap = ni_first->ni_vap;
    u_int8_t    rateCode = 0;    
    u_int8_t    dot11Rate = 0;
    ...
    rateCode = ni_first->ni_tx_cur_rateCode;    
    dot11Rate = ni_first->ni_tx_cur_dot11Rate;
...
static int
ath_ni_statistics_rate_for_tx_aggr_frame(struct ath_buf *bf, struct ath_buf *bf_first)
{
    ...
    )
{
    ...
    if (statuscode == IEEE80211_STATUS_SUCCESS) {
         ...
         */
        /*zhaoyang1 transplant from 717*/
        /*xiaruixin modify for wapi performance */
...
#else
    //AUTELAN-Added-Begin:duanmingze for workaround ASSERT problem
    //ASSERT(!TX_BUF_BITMAP_IS_SET(tid->tx_buf_bitmap, cindex));
    if(TX_BUF_BITMAP_IS_SET(tid->tx_buf_bitmap, cindex))
    {
		printk("%s Assert(tid->tx_buf_bitmap[%d >> TX_BUF_BITMAP_LOG2_WORD_SIZE] & (1 << (%d & TX_BUF_BITMAP_WORD_MASK)) != 0)\n",__func__,cindex,cindex);
    	if(kes_debug_print_handle){\
	    	kes_debug_print_handle(KERN_EMERG "%s Assert(tid->tx_buf_bitmap[%d >> TX_BUF_BITMAP_LOG2_WORD_SIZE] & (1 << (%d & TX_BUF_BITMAP_WORD_MASK)) != 0)\n",__func__,cindex,cindex);
		}
    }
    //AUTELAN-Added-End:duanmingze for workaround ASSERT problem
...
/*AUTELAN-zhaoenjuan transplant for drv monitor tid trace*/
	tid->add_paused_trace[tid->add_trace_index] = 5;
	tid->add_trace_index ++;
	if(tid->add_trace_index > 7)
		tid->add_trace_index = 0;
    ATH_TX_RESUME_TID(sc, tid);
...
EXPORT_SYMBOL(ath_tx_resume_tid); //AUTELAN-Added:duanmingzhe for tid stuck
        ...
        ath_bufhead *bf_head, ieee80211_tx_control_t *txctl)
{
	...
	__11nstatsn(sc, tx_bytes, wbuf_get_pktlen(bf->bf_mpdu));
...
/*zhaoyang modify end*/
    if (!TAILQ_EMPTY(&tid->buf_q) || tid->paused || txq->axq_depth) {
        ...
        ath_tx_queue_tid(txq, tid);
#endif
        if (!txq->axq_depth && !tid->paused) /*Autelan-Added:duanmingzhe for patch tx-stuck patchs from case 01146407*/
        {
               ath_txq_schedule(sc, txq);
        }
        return 0;
...
//#if ATH_SUPPORT_VOW_DCS /*Autelan-Added:duanmingzhe for patch tx-stuck patchs from case 01146407*/
    if (txctl->ht) {
        /* update starting sequence number for subsequent ADDBA request */
        INCR(tid->seq_start, IEEE80211_SEQ_MAX);
    }
//#endif
    ...
    /*Start: Add by zhanghu to statistics of every frame by the rate*/
    {
        ...
        if(txctl->isdata && !txctl->ismcast){
            ath_ni_statistics_rate_for_tx_frame(bf);
        }
        if(txctl->isdata){
            ath_vap_statistics_rate_for_tx_frame(bf);
        }
    }
    /*End: Add by zhanghu to statistics of every frame by the rate*/
                  ...
                  ath_bufhead *bf_head, ieee80211_tx_control_t *txctl)
{
    ...
    bf = TAILQ_FIRST(bf_head);
    //bf->bf_isampdu = 1; /*Autelan-Added:duanmingzhe for patch tx-stuck patchs from case 01146407*/
      ...
      break;
        ...
        if (!BAW_WITHIN(tid->seq_start, tid->baw_size, bf->bf_seqno)) {
			...
			if(bf == bf_first)
			{
				if(tid->baw_head == tid->baw_tail)
				{
					int pertidBufCount=0,bufQCount=0,j,tidBufCount=0;
					struct ath_buf * buf;
					printk("\n**********************************************************");
					printk("\n  TID queue not aggregated error kick in workaround  ");
					printk("\n**********************************************************");

					printk("\ntid %p pause %d : sched_normal %d send_normal %d : ", tid, tid->paused,tid->seq_sched_normal,tid->seq_send_normal);
					...
					printk("\ntidBufCount %d  bufQCount %d \n  ", tidBufCount, bufQCount);

					printk("\n -------------------------------------------------------------- \n");
					//                    tid->seq_start=DECR(bf->bf_seqno, IEEE80211_SEQ_MAX);
					printk("First buf ht state: %d\n",bf->bf_ht);
					sc->sc_ieee_ops->dump_pkt((const u_int8_t *)bf->bf_vdata);
					{
						int i;
						struct ath_desc * ds = (struct ath_desc *)bf->bf_desc;
						printk( "%s:  bf_daddr %08x len %d"
							,__func__, 
							(int)bf->bf_daddr, bf->bf_buf_len[0]);
						printk( "%s: desc :  "
							"%08x %08x %08x %08x \n",
        ...
        if (unlikely(bf->bf_useminrate)) {
            ...
            if (bf_first == bf) { /* Case I, per comments */
                ...
                ASSERT(nframes == 0);
                nframes++;
                ath_tx_addto_baw(sc, tid, bf); /*Autelan-Added:duanmingzhe for patch tx-stuck patchs from case 01146407*/
                break;
       ...
       if ((ATH_IS_VOWEXT_RCA_ENABLED(sc)) || (ATH_IS_VOWEXT_AGGRSIZE_ENABLED(sc)) ) {
        ...
        /*Start: Add by zhanghu to statistics of every frame by the rate*/
        ath_ni_statistics_rate_for_tx_aggr_frame(bf, bf_first);
        ath_vap_statistics_rate_for_tx_aggr_frame(bf, bf_first);
        /*End: Add by zhanghu to statistics of every frame by the rate*/
...
{
    struct ath_buf *bf, *tbf; /*Autelan-Added:duanmingzhe for patch tx-stuck patchs from case 01146407*/
    int legacy;
    ATH_AGGR_STATUS status = ATH_AGGR_DONE;
    ath_bufhead bf_head;
...
#ifdef ATH_SWRETRY
        if ((sc->sc_enhanceddmasupport) && an && an->an_softxmit_qdepth >= HAL_TXFIFO_DEPTH) {
            /* SW retry pending queue is alreay full - do not add any more buffer to the HW */
            break;
        }
#endif
...
ath_tx_sched_normal(struct ath_softc *sc, struct ath_txq *txq, ath_atx_tid_t *tid)
{
    ...
    do {
        ...
        bf = TAILQ_FIRST(&tid->buf_q);
		/*zhaoyang modify for add CL101673.fix.patch for dusi*/
#if 0 /*Autelan-Added:duanmingzhe for patch tx-stuck patchs from case 01146407*/
#if ATH_SUPPORT_VOW_DCS
	/*zhaoyang modify end*/
		if (bf->bf_ht) {
            /* update starting sequence number for subsequent ADDBA request */
...
ath_tx_sched_aggr(struct ath_softc *sc, struct ath_txq *txq, ath_atx_tid_t *tid)
{
    ...
    do {
...
#ifdef ATH_SWRETRY
        if ((sc->sc_enhanceddmasupport) && tan && tan->an_softxmit_qdepth >= HAL_TXFIFO_DEPTH) {
            /* SW retry pending queue is alreay full - do not add any more buffer to the HW */
            break;
        }
#endif
        ...
        TAILQ_INIT(&bf_q);
		/*zhaoyang modify for add CL101673.fix.patch for dusi*/
		DPRINTF(sc, ATH_DEBUG_PWR_SAVE, "%s calling tx_form_aggr\n",
			__func__);
		/*zhaoyang modify end*/
...
struct timeval    	tid_last_time;	/*AUTELAN-zhaoenjuan transplant for drv monitor tid trace*/
/*
 * Tx scheduling logic
 * NB: must be called with txq lock held
 */
...
ath_txq_schedule(struct ath_softc *sc, struct ath_txq *txq)
{
    ...
    TAILQ_HEAD(,ath_atx_tid)paused_tid_q;
	struct timeval time_adjust_end;/*AUTELAN-zhaoenjuan transplant for drv monitor tid trace*/

    /* Begin:Add by chenxf for powersave performance 2014-06-05 */
	TAILQ_HEAD(,ath_atx_ac)tmp_ps_acq; 
...
__11nstats(sc, aute_tx_schedule);//AUTELAN:zhaoenjuan transplant for drv monitor tx_schedule stats

#if ATH_SUPPORT_VOWEXT
    struct ath_atx_ac *first_schd_ac = NULL;
    int if_first_ac = 1;
	/*zhaoyang modify for add CL101673.fix.patch for dusi*/
	DPRINTF(sc, ATH_DEBUG_PWR_SAVE, "%s called schedule of txq\n",
	 __func__);
...
#endif
						/*zhaoyang modify for add CL101673.fix.patch for dusi*/
						DPRINTF(sc, ATH_DEBUG_PWR_SAVE, "%s calling tx_sched_aggr\n",
						__func__);
						/*zhaoyang modify end*/
                        ath_tx_sched_aggr(sc, txq, tid);
                    } else {
...
#endif      
			/*Begin:Added by zhaoyang1 for llc packet error 2012-12-13*/
			{
				struct llc *llc; 
				wbuf_t wbuf;
				struct ieee80211_frame *wh;
				...
				if (!(wh->i_fc[1] & IEEE80211_FC1_WEP)) { /*lijiyong add to avoid encrypted data enter this process, so as to fix ENBUG-93 2013-09-25*/
				 	/*Begin: yanggongsheng add for wds frame pass 2013-01-15*/
					if ((wh->i_fc[1] & IEEE80211_FC1_DIR_MASK) == IEEE80211_FC1_DIR_DSTODS) {
						if ((wh->i_fc[0] & 0x80)>0) {
							llc = (struct llc *)((u_int8_t *)wh + roundup(sizeof(struct ieee80211_qosframe_addr4), sizeof(u_int32_t)));
						}
						else {
							llc = (struct llc *)((u_int8_t *)wh + roundup(sizeof(struct ieee80211_frame_addr4), sizeof(u_int32_t)));
						}
					}
					/*End: yanggongsheng add for wds frame pass 2013-01-15*/
					else if ((wh->i_fc[0] & 0x80)>0) {
						llc = (struct llc *)((u_int8_t *)wh + roundup(sizeof(struct ieee80211_qosframe), sizeof(u_int32_t)));
					}
					else {
						llc = (struct llc *)((u_int8_t *)wh + roundup(sizeof(struct ieee80211_frame), sizeof(u_int32_t)));
					}
					
					if ((wh->i_fc[0] & IEEE80211_FC0_TYPE_MASK) == IEEE80211_FC0_TYPE_DATA) {
						if ((llc->llc_dsap != LLC_SNAP_LSAP || llc->llc_ssap != LLC_SNAP_LSAP) && !IEEE80211_IS_BROADCAST(wh->i_addr1)) {
							bf->bf_retries = sw_retry_limit;
						}
					}
				}
			}
			/*End:Added by zhaoyang1 for llc packet error 2012-12-13*/
