    ...
    u_int16_t *indexL, u_int16_t *indexR)
{
    ...
    */
	/*Begin:Modified by zhaoyang1 for Coverity check 2013-07-02*/
	if (NULL == pList) {
		return AH_FALSE;
	}
	/*End:Modified by zhaoyang1 for Coverity check 2013-07-02*/
	if (target <= pList[0])
    {
        *indexL = *indexR = 0;
        return AH_TRUE;
    ...
    u_int8_t *pVpdList, u_int16_t numIntercepts, u_int8_t *pRetVpdList)
{
    ...
    {
		/*Begin:Modified by zhaoyang1 for Coverity check 2013-07-01*/
		if (getLowerUpperIndex(currPwr, pPwrList, numIntercepts,
            &(idxL), &(idxR)) != AH_TRUE) {
			return AH_FALSE;
		}
