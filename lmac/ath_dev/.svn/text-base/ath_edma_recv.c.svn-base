...
ath_rx_intr(ath_dev_t dev, HAL_RX_QUEUE qtype)
{
    ...
    do {
        ...
        if (wbuf == NULL)
        {
            sc->rx_null_cnt++;//AUTELAN-zhaoenjuan transplant for drv_monitor stats
            break;
        }
        bf = ATH_GET_RX_CONTEXT_BUF(wbuf);
        ...
        if (HAL_EINPROGRESS == retval) {
            sc->rx_incomplete_cnt++;//AUTELAN-zhaoenjuan transplant for drv_monitor stats
            break;
        }
		...
		if (ps_lmac) {
			...
			if ((rxs->rs_status & ~HAL_RXERR_KEYMISS) == 0)	{
				...
				if(sc->sc_ieee_ops->aute_ps_check != NULL)
					sc->sc_ieee_ops->aute_ps_check(sc->sc_ieee, wh, rxs->rs_keyix, true);
			}
		}		
...
ath_rx_handler(ath_dev_t dev, int flush, HAL_RX_QUEUE qtype)
{
	...
	/*AUTELAN-Begin:zhaoenjuan transplant for drv_monitor stats*/
	...
	/*AUTELAN-End:zhaoenjuan transplant for drv_monitor stats*/
...
ath_rx_edma_tasklet(ath_dev_t dev, int flush)
{
    ...
    /*lijiyong add begin to check rx stuck for ENBUG-79*/
    sc->sc_stats.ast_rx++;
    /*lijiyong add end*/
