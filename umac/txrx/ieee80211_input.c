...
ieee80211_is_mcastecho(struct ieee80211vap *vap, struct ieee80211_frame *wh)
{
 ...
 /**
  * Description:
  *  When recv a pkt from a sta which not belong to this vap, ap will send back a deauth to this sta.
  *  Because the deauth-sending func allocs mem from system memory directly, and free asynchronously,
  *  when the flow is large enough, cause out of memory. So we control the sending-frequency by this func. 
  *
  * Return:
  *  1: too fast
  *  0: not fast
  * 
  * Called by ieee80211_input_data()
  * add by zhaohan@autelan.com
  */
static int32_t ieee80211_deauth_send_too_fast(struct ieee80211vap *vap)
{
    u_long cur_time = jiffies;

    if ((cur_time > vap->iv_deauth_timestamp) &&  /* prevent jiffies overflow */
        ((cur_time - vap->iv_deauth_timestamp) < vap->iv_deauth_send_interval))
    {
        /* too fast, return true */
        return 1;
    }

    /* it's ok, return 0 */
    vap->iv_deauth_timestamp = cur_time;
    return 0;
}
...
/*<Begin : caizhibang modify for autelan 802.3 tunnel 2012-5-25 */			
  ...
  /* End : caizhibang modify for autelan 802.3 tunnel 2012-5-25 >*/
   ...
   // u_int8_t tid;
	...
	/* gengzongjie transplant from service provider 2013-4-27 */
   ...
   /* gengzongjie transplant end */
	...
	/*Begin:Added by zhaoyang1 for shifting down four-way-handshake 2013-01-28*/
	u_int8_t packet_type = 0;
	/*End:Added by zhaoyang1 for shifting down four-way-handshake 2013-01-28*/
	/* gengzongjie transplant from service provider 2013-4-27 */
    rssithre  = ic->ic_node_getrssi(ni , -1 , WLAN_RSSI_RX);     //added by zhanghu for avg rssi
    //if (ni->ni_rssi < vap->iv_rssi_threshold)		//gengzongjie modified 2013-4-27
	/*Beging:Modified by zhaoyang1 for optimizing lower rssi limit 2013-05-31*/
	...
	/* gengzongjie transplant end */
    {
		...
		//FOA just send timeout_netlink to ac.
			...
			printk("%s %d a sta(%02x:%02x:%02x:%02x:%02x:%02x) average rssi(%d) is low than limit(%d), kick it out!\n", \
					__func__,__LINE__,ni->ni_macaddr[0],ni->ni_macaddr[1],ni->ni_macaddr[2], \
					ni->ni_macaddr[3],ni->ni_macaddr[4],ni->ni_macaddr[5],rssithre,vap->iv_rssi_threshold);		
	...
	/*End:Modified by zhaoyang1 for optimizing lower rssi limit 2013-05-31*/
    /*ended by chenming*/
#if 0
    ...
    {    
        ...
        if ((atomic_read(&(ni->ni_packet_number)) / jiffies_to_msecs(current_tick - ni->ni_traffic_tick)) * 1000 <= ni->ni_packet_max_number) 
	{        
            ;
        } 
	else 
    ...
    {    
        ...
        if ((atomic_read(&(ni->ni_packet_number)) / jiffies_to_msecs(current_tick - ni->ni_traffic_tick)) * 1000 <= vap->every_node_packet_max_number) 
	{        
            ;
        } 
	else 
    ...
    {
        atomic_add((int)(wbuf->len), &(ni->ni_packet_number));
    }
    else
    {
    ...
    if (vap->vap_traffic_limit) 
    {
        ...
        if ((atomic_read(&(vap->vap_packet_number)) / jiffies_to_msecs(current_tick - vap->vap_traffic_tick)) * 1000 <= vap->vap_packet_max_number) 
	{
            atomic_add((int)(wbuf->len), &(vap->vap_packet_number));
        } 
	else 
        {    
            goto bad;
...
/* Begin: gengzj added for wifipos 2013-11-26 */			
			...
			if ((sc->sc_locate == 0) && (rfid_scan == 0))//pengdecai  edit " rfid_scan == 0" to "vap->iv_loacte == 0"
			{
	            /* NB: caller deals with reference */
            	if (ieee80211_vap_ready_is_set(vap) && !ieee80211_deauth_send_too_fast(vap)) /* add by zhaohan@autelan.com. Control the sending freq of deauth frames, 2013-12-12 */
				{
    ...
    if (thinap) {
        ...
        if (tunnel_local_state == 1) {//tunnel mode
            ...
            if (ieee80211_node_is_authorized(ni)) {
				...
				if ((frametype == __constant_htons(ETHERTYPE_PAE)) && (3 == packet_type)) {
					wbuf = wbuf_capture;
					wbuf_capture = NULL;
...
/*Begin:Modified by zhaoyang1 for shifting down four-way-handshake 2013-01-28*/
		if ((frametype == __constant_htons(ETHERTYPE_PAE)) && (0x03 == packet_type)) {
			wbuf->protocol = __constant_htons(0x0023);
		}
local_forward:
        /*End : caizhibang modify for autelan 802.3 tunnel 2012-5-25 >*/
        /*ljy--modified end*/
        if (((frametype == __constant_htons(ETHERTYPE_PAE)) && (0x03 != packet_type)) || frametype == __constant_htons(ETHERTYPE_PRE_PAE) || frametype == __constant_htons(ETHERTYPE_WAI)) /*ljy--modified for wapi*/
        {	
...
#define HAS_SEQ(type, subtype)   (((type & 0x4) == 0) && ((type | subtype) != QOS_NULL))
    struct ieee80211com *ic = ni->ni_ic;
/* Begin:gengzj added for rfid scan */
    struct ath_softc_net80211   *scn = ATH_SOFTC_NET80211(ic);
    struct ath_softc *sc = scn->sc_dev;
/* End:gengzongjie added end */
    struct ieee80211_frame *wh;
	...
	/* gengzongjie added for traffic balance to add band and rssi ie in probreq */
	u_int8_t *frm = NULL;
...
/* Begin:gengzj added for rfid scan 2013-11-26 */
	if(rfid_scan && ((type == IEEE80211_FC0_TYPE_MGT)||((type == IEEE80211_FC0_TYPE_DATA)&&(dir == IEEE80211_FC1_DIR_TODS)))){
		u_int8_t *skb_data;
		wbuf_t rfid_wbuf = NULL;
	    u_int8_t rssi = 0;
		u_int32_t rfid_wbuf_size = RFID_SCAN_WBUF_SIZE;
		#if 0
		u_int8_t mac[6];
		mac[0] = 0xb0;
		mac[1] = 0x8e;
		...
		mac[4] = 0x00;
		mac[5] = 0xac;
		if(memcmp(wh->i_addr2, mac, 6) == 0)
		{
			printf("rs->rs_rssi: %d\n",rs->rs_rssi);
			printf("u_int8_t rssi->rs_rssi: %d\n",(u_int8_t)rs->rs_rssi);
		}
		#endif
		
		rssi = (u_int8_t)(rs->rs_rssi);
		//rfid_wbuf = wbuf_copy(wbuf);
		
		rfid_wbuf = ath_rxbuf_alloc(sc, rfid_wbuf_size);

		if(rfid_wbuf == NULL)
		{
			printk("malloc rfid_wbuf failed!\n");
			goto bad;
		}
		if(!(pskb_expand_head(rfid_wbuf, 0, rfid_wbuf_size, GFP_ATOMIC)))
		{
			if(wbuf_get_pktlen(wbuf) > rfid_wbuf_size)
			{
				memcpy(skb_put(rfid_wbuf, rfid_wbuf_size), wbuf_header(wbuf), rfid_wbuf_size);
			}
			else
				memcpy(skb_put(rfid_wbuf, wbuf_get_pktlen(wbuf)), wbuf_header(wbuf), wbuf_get_pktlen(wbuf));
		}
		
		skb_data = skb_push(rfid_wbuf, sizeof(rfid_header));
		memcpy(skb_data, &(rssi), 1);
		rfid_wbuf->data = skb_data;
		if(rfid_wbuf != NULL){
			rfid_wbuf->dev = dev;
            #if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,22))
            rfid_wbuf->mac.raw = rfid_wbuf->data;
            #else
            skb_reset_mac_header(rfid_wbuf);
            #endif
			rfid_wbuf->ip_summed = CHECKSUM_NONE;
            rfid_wbuf->pkt_type = PACKET_OTHERHOST;				
            rfid_wbuf->protocol = __constant_htons(0x0040); /* send 80211frames to aplocator */
            ret = netif_rx(rfid_wbuf);
			if(NET_RX_DROP == ret){
				goto bad;
			}
		}else{
			printk("rfid_wbuf is empty!\n");
		}
	}
/* End:gengzongjie added end */
...
/* End: gengzj added end */
    ...
    if (type == IEEE80211_FC0_TYPE_DATA) {
        ...
        {   
	        ...
	        if(IEEE80211_IS_CHAN_5GHZ(ic->ic_curchan) || 
               (IEEE80211_IS_CHAN_2GHZ(ic->ic_curchan) && (rs->rs_rssi >= sta_signal_strength_thr))) //lijiyong modify from ni->ni_rssi to rs->rs_rssi for Optimization the check condition
            /*End, wangjia 2012-10-16 */
            {
    ...
    if (rs->rs_rssi < vap->iv_rssi_threshold) //lijiyong modify from ni->ni_rssi to rs->rs_rssi for Optimization the check condition
    {
      if(type == IEEE80211_FC0_TYPE_MGT && \
            ...
            if (thinap) {
                ...
                * ********************************************************************/
                ...
                switch ((wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK)
                    ...
                    case 4:
                    /*probe req*/
                        if (vap->iv_traffic_balance) {	/*xmeng add for thin ap traffic balance*/ //Modified by zhaoyang1 for optimizing traffic balance based wlan 2013-06-19
                            /* Begin, Add by linrongqi, 2012-10-16. */
                            IEEE80211_ADDR_COPY(wh->i_addr3,vap->iv_myaddr);
                            /* End, Add by linrongqi, 2012-10-16. */
						/* gengzongjie added for traffic balance to modify probreq before send to ac */
						if (IEEE80211_IS_BROADCAST(wh->i_addr1)) {
							IEEE80211_ADDR_COPY(wh->i_addr1, vap->iv_myaddr);
						} 
						/* gengzongjie added end */
                        } else {
                            break;
...
/*<Begin : caizhibang modify for autelan 802.3 tunnel 2012-5-25*/						
 ...
 /*End : caizhibang modify for autelan 802.3 tunnel 2012-5-25>*/	              
				...
				/* gengzongjie added for traffic balance to add ie in reassoc req before send to ac */
				if (vap->iv_traffic_balance && (subtype == IEEE80211_FC0_SUBTYPE_PROBE_REQ ||
					subtype == IEEE80211_FC0_SUBTYPE_ASSOC_REQ ||
					subtype == IEEE80211_FC0_SUBTYPE_REASSOC_REQ)) {
					frm = wbuf_capture->tail;
					wbuf_capture->tail = skb_put(wbuf_capture, 11);
					if (0 == ieee80211_add_band_rssi(frm, ic, rs)) {
					}
					else {
						printk("__Added ie to reassoc req failed__,__%s__%d__\n",__func__,__LINE__);
					}
				}
				/* gengzongjie added end */
