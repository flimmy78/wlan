...
{
	struct ieee80211_node *ni = wbuf_get_node(skb); //Added by zhaoyang1 for optimizing vlan setting based on user 2013-01-22
...
#else
	/*Begin:Modified by zhaoyang1 for four-way-handshake 2013-05-04*/
	if (__constant_htons(0x0023) == skb->protocol) {
    	skb->protocol = eth_type_trans(skb, dev);
		skb->protocol = __constant_htons(0x0023);
	} else {
		skb->protocol = eth_type_trans(skb, dev);
	}
	/*Ends:Modified by zhaoyang1 for four-way-handshake 2013-05-04*/
#endif
	...
	if (ni != NULL) {
		if ((ni->ni_vid > 0) && (ni->ni_vid <= VLAN_VID_MASK)) {
			...
			skb = vlan_put_tag(skb, ni->ni_vid);
			if (!skb)
				return;
			skb_pull(skb, ETH_HLEN);
		}
    ...
    osif_getstats(struct net_device *dev)
{
    ...
    const struct ieee80211_mac_stats *broadmacstats;
    if (!vap) {
	return stats;
    }
    ...
    broadmacstats = &vap->iv_broadcase_stats;
/*zhaoyang1 transplant from 717*/
/*Begin:Added by duanmingzhe for information report*/
    stats->tx_multicast_packets = (unsigned long)(multimacstats ->ims_tx_packets + broadmacstats->ims_tx_packets);
    stats->tx_multicast_bytes = (unsigned long)(multimacstats ->ims_tx_bytes+ broadmacstats->ims_tx_bytes);
    stats->rx_multicast_packets = (unsigned long)(multimacstats ->ims_rx_packets + broadmacstats->ims_rx_packets);
    stats->rx_multicast_bytes = (unsigned long)(multimacstats ->ims_rx_bytes+ broadmacstats->ims_rx_bytes);
    stats->tx_broadcast_packets = broadmacstats->ims_tx_packets;
    stats->tx_broadcast_bytes = broadmacstats->ims_tx_bytes;
    stats->rx_broadcast_packets = broadmacstats->ims_rx_packets;
    stats->rx_broadcast_bytes = broadmacstats->ims_rx_bytes;
/*End:Added by duanmingzhe for information report*/
/*Begin:Added by pengruofeng for information report 2011-5-12*/
    stats->tx_retry_packets = unimacstats ->ims_tx_retry_packets;
    stats->tx_retry_bytes = unimacstats ->ims_tx_retry_bytes;
    stats->rx_retry_packets = unimacstats ->ims_rx_retry_packets;
    stats->rx_retry_bytes = unimacstats ->ims_rx_retry_bytes;
/*End:Added by pengruofeng for information report 2011-5-12*/
    /*zhaoyang modify for tunnel count CHINAMOBILE-271*/
    stats->multicast = stats->rx_multicast_packets;
    /*zhaoyang modify end*/   
	/*zhaoyang modify for retry packet count ISSUES-665*/
	stats->tx_retry_mgmt_packets = unimacstats ->ims_tx_retry_mgmt_packets;
    stats->tx_retry_mgmt_bytes = unimacstats ->ims_tx_retry_mgmt_bytes;
    stats->rx_retry_mgmt_packets = unimacstats ->ims_rx_retry_mgmt_packets;
    stats->rx_retry_mgmt_bytes = unimacstats ->ims_rx_retry_mgmt_bytes;
	/*zhaoyang modify end*/
    stats->rx_packets = (unsigned long)(unimacstats->ims_rx_packets + stats->rx_multicast_packets);
    //stats->rx_packets = (unsigned long)(unimacstats->ims_rx_packets + multimacstats->ims_rx_packets);
    //printk("rx = %lu uni = %lu multi = %lu broad = %lu \n",stats->rx_packets,(unsigned long)(unimacstats->ims_rx_packets),(unsigned long)(multimacstats->ims_rx_packets),(unsigned long)(broadmacstats->ims_rx_packets));
    stats->tx_packets = (unsigned long)(unimacstats->ims_tx_packets + stats->tx_multicast_packets);
    //stats->tx_packets = (unsigned long)(unimacstats->ims_tx_packets + multimacstats->ims_tx_packets);
    //printk("tx = %lu uni = %lu multi = %lu broad = %lu \n",stats->tx_packets,(unsigned long)(unimacstats->ims_tx_packets),(unsigned long)(multimacstats->ims_tx_packets),(unsigned long)(broadmacstats->ims_tx_packets));
    /*End:Added by duanmingzhe for information report*/
	/*zhaoyang1 transplant end*/
	...
	/*zhaoyang1 transplant from 717*/
    /*Begin:Added by duanmingzhe for information report*/
    stats->tx_bytes = (unsigned long)(unimacstats->ims_tx_bytes+ stats->tx_multicast_bytes);
	...
	/*zhaoyang1 transplant from 717*/
    /*Begin:Added by duanmingzhe for information report*/
    stats->rx_bytes = (unsigned long)(unimacstats->ims_rx_bytes+ stats->rx_multicast_bytes);
	...
	stats->rx_errors = vapstats->is_rx_tooshort
		...
		/*zhaoyang modify for error frame count CHINAMOBILE-105*/
			...
			+ unimacstats->ims_rx_badkeyid
			+ multimacstats->ims_rx_badkeyid
			+ unimacstats->ims_rx_unencrypted
			+ multimacstats->ims_rx_unencrypted
			+ unimacstats->ims_rx_ccmpreplay
			+ multimacstats->ims_rx_ccmpreplay
			+ unimacstats->ims_rx_ccmpformat
			+ multimacstats->ims_rx_ccmpformat
			+ unimacstats->ims_rx_ccmpmic
			+ multimacstats->ims_rx_ccmpmic
			+ unimacstats->ims_rx_wpireplay
			+ multimacstats->ims_rx_wpireplay
...
#endif
			/*zhaoyang1 transplant from 717*/
			/*Begin:Added by pengruofeng for counting of error packets 2011-5-12*/
			+ vap->iv_stats.is_rx_errors;
			/*End:Added by pengruofeng for counting of error packets 2011-5-12*/
			
			/*Begin:Added by pengruofeng for counting of error frames 2011-5-12*/
			stats->rx_frame_errors = vap->iv_stats.is_rx_error_frames + stats->rx_errors;//zhaoyang modify for error frame count CHINAMOBILE-105
			/*End:Added by pengruofeng for counting of error frames 2011-5-12*/
			/*zhaoyang1 transplant end*/
...
/* Begin: gengzj added for wifipos 2013-11-26 */
	...
	//printk("%s vap radio_id = %d\n",__func__,vap->iv_radioid);
...
*/
static int
ifc_name2unit(const char *name, u_int32_t *unit) /*modified by chenming*/
{
    const char *cp;
    *unit = 0;
    if(OS_MEMCMP(name, "athscanwifi", 11) == 0)  // 4 scan vap
    {
        cp = name + 11;
        for(;*cp != '\0'; cp++)
        {
	...
	else if(OS_MEMCMP(name, "athlocatewifi", 13) == 0)
	{
        cp = name + 13;
        for(;*cp != '\0'; cp++)
        {
    ...
    /*Begin:Modify by wangzhicong for set wlan name  2013-04-27*/
    //else if(OS_MEMCMP(name, "ath", 3) == 0 &&  ('0' <= *(name+3) && *(name+3) <= '3'))  // 4 wds vap
    else if(OS_MEMCMP(name, "ath", 3) == 0 &&  ('0' <= *(name+3) && *(name+3) <= '3') && *(name+4) == '\0')  // 4 wds vap
    /*Eng:Modify by wangzhicong for set wlan name 2013-04-27*/
    {
        cp = name + 3;
        for(;*cp != '\0'; cp++)
        {
...
/*zhaoyang1 transplant from 717*/
static void
osif_delete_wlanunit(u_int32_t unit) /*modified by chenming*/
/*zhaoyang1 transplant end*/
{
    /* NB: covered by rtnl_lock */
...
//    dev->priv = vap;    // lisongbai modify it!!!,if open,it will panic when create the vap on XJPB44_02 platform!!!!! //del by duanmingzhe
#endif
