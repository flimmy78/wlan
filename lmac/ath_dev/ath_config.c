...
extern void ieee80211_display_list(void);
u_int32_t openessid_param  = 0x20;   /* default timeout is 32s */   

/* openessid_param datail form . bits (0 - 19)
^--------------------------------------------------------------------------------------------------------------^
...
ath_set_config(ath_dev_t dev, ath_param_ID_t ID, void *buff)
{
    ...
    int retval = 0;
    u_int32_t mask = 0; // added by zhanghu
...
/*modified by chenming for rate display*/
    switch(ID)
    {
        case ATH_PARAM_TXCHAINMASK:
             ...
             if (!*(u_int32_t *)buff) {
                 sc->sc_ath_ops.set_tx_chainmask(sc,hal_chainmask);
             } else {
                sc->sc_ath_ops.set_tx_chainmask(sc,*(u_int32_t *)buff & hal_chainmask);
             }
        break;
        ...
        case ATH_PARAM_RXCHAINMASK:
             ...
             if (!*(u_int32_t *)buff) {
                 sc->sc_ath_ops.set_rx_chainmask(sc,hal_chainmask);
             } else {
                 sc->sc_ath_ops.set_rx_chainmask(sc,*(u_int32_t *)buff & hal_chainmask);
             }
        break;

        case ATH_PARAM_TXCHAINMASKLEGACY:
             sc->sc_ath_ops.set_tx_chainmasklegacy(sc,*(u_int32_t *)buff & hal_chainmask);
        break;

        case ATH_PARAM_RXCHAINMASKLEGACY:
            sc->sc_ath_ops.set_rx_chainmasklegacy(sc,*(u_int32_t *)buff & hal_chainmask);
        break;
/*ended by chenming*/
/*zhaoyang1 transplant end*/
        ...
        /*Begin: Transplant from v5 cmcc, wangjia.*/
        /*lijiyong add begin for debug cca*/
        case ATH_PARAM_CCA_THRESHOLD:
            ath_hal_setCcaThreshold(sc->sc_ah, (*(int *)buff));
            break;
        /*lijiyong add end*/
        
        /*lijiyong add begin for debug noise immunity level*/
        case ATH_PARAM_OFDM_NOISE_IMMUNITY_LEVEL:
            ath_hal_setOfdmNoiseImmunityLevel(sc->sc_ah, (*(int *)buff));
            break;

        case ATH_PARAM_CCK_NOISE_IMMUNITY_LEVEL:
            ath_hal_setCckNoiseImmunityLevel(sc->sc_ah, (*(int *)buff));
            break;
        /*lijiyong add end*/
        /*End: Transplant from v5 cmcc, wangjia.*/
		
        /*Begin: Added by wangjia, for disable NF. */
        case ATH_PARAM_DEBUG_NF:
            if(*(int *)buff)
                sc->DisableNF = 1;
            else
                sc->DisableNF = 0;             
        ...
        case ATH_PARAM_RATE_MASK_CAP:
            mask = (u_int32_t)(*(u_int32_t*)buff);
            mask &= 0x80;
            ((struct ieee80211com *)sc->sc_ieee)->ic_rate_mask &= ~((u_int64_t)0x80 << (8*sizeof(u_int64_t) - 8));
            ((struct ieee80211com *)sc->sc_ieee)->ic_rate_mask |= (u_int64_t)mask << (8*sizeof(u_int64_t) - 8);
        break;
        case ATH_PARAM_RATE_MASK_G:
            mask = (u_int32_t)(*(u_int32_t*)buff);
            mask &= 0x00FFFFFF;
            ((struct ieee80211com *)sc->sc_ieee)->ic_rate_mask &= ~((u_int64_t)0x00FFFFFF << 8*sizeof(u_int32_t));
            ((struct ieee80211com *)sc->sc_ieee)->ic_rate_mask |= (u_int64_t)mask << 8*sizeof(u_int32_t);
        break;
        case ATH_PARAM_RATE_MASK_B:
            mask = (u_int32_t)(*(u_int32_t*)buff);
            mask &= 0x000000FF;
            ((struct ieee80211com *)sc->sc_ieee)->ic_rate_mask &= ~((u_int64_t)0x000000FF << 8*sizeof(u_int16_t));
            ((struct ieee80211com *)sc->sc_ieee)->ic_rate_mask |= (u_int64_t)mask << 8*sizeof(u_int16_t);
        break;
        case ATH_PARAM_RATE_MASK_A:
            mask = (u_int32_t)(*(u_int32_t*)buff);
            mask &= 0x0000FFFF;
            ((struct ieee80211com *)sc->sc_ieee)->ic_rate_mask &= ~((u_int64_t)0x0000FFFF);
            ((struct ieee80211com *)sc->sc_ieee)->ic_rate_mask |= mask;
        break;
        case ATH_PARAM_MCS_MASK:
            mask = (u_int32_t)(*(u_int32_t*)buff);
            mask &= 0xFFFFFF;            
            ((struct ieee80211com *)sc->sc_ieee)->ic_mcs_mask &= ~((u_int32_t)0xFFFFFF);
            ((struct ieee80211com *)sc->sc_ieee)->ic_mcs_mask |= (u_int32_t)mask;

        break;
        case ATH_PARAM_MCS_CAP:
            mask = (u_int32_t)(*(u_int32_t *)buff);
            mask &= 0x80;
            ((struct ieee80211com *)sc->sc_ieee)->ic_mcs_mask &= ~((u_int32_t)0x80 << (8*sizeof(u_int32_t) - 8));
            ((struct ieee80211com *)sc->sc_ieee)->ic_mcs_mask |= (u_int32_t)mask << (8*sizeof(u_int32_t) - 8);
        break;
        /*End: added by zhanghu for increasing rate*/

		/* AUTELAN-Added-Begin : &tuqiang for openessid */
		case ATH_OPENESSID_PARAM:
		openessid_param = *(u_int32_t *)buff & 0xFFFFF;
		if( (openessid_param & 0xff) < 0x10 ){
			//printk("Advise : timeout >= 15s is better\n");
			//openessid_param |= 0xf;
		}
		/* AUTELAN-Added-End : tuqiang@autelan.com */
		break;
...
//            *(int *)buff = sc->sc_eap_lowest_rate;
        ...
        /*lijiyong add begin for debug cca*/
        case ATH_PARAM_CCA_THRESHOLD:
            *(int *)buff = ath_hal_getCcaThreshold(sc->sc_ah);
            break;
        case ATH_PARAM_CTL_NF:
            *(int *)buff = ath_hal_get_ctl_nf(sc->sc_ah);
            break;
        case ATH_PARAM_EXT_NF:
            *(int *)buff = ath_hal_get_ext_nf(sc->sc_ah);
            break;
        /*lijiyong add end*/
        /*End: Transplant from v5 cmcc, wangjia.*/
        ...
        /*Begin: Added by wangjia, for disable NF. */
        case ATH_PARAM_DEBUG_NF:
            *(int *)buff = sc->DisableNF;
            break; 
        /*End: Added by wangjia, for disable NF. */
        
        /*Start: added by zhanghu for increasing rate*/
        case ATH_PARAM_RATE_MASK_CAP:
            mask = (((struct ieee80211com *)sc->sc_ieee)->ic_rate_mask) >> (8*sizeof(u_int64_t) - 8);
            mask &= 0x000000FF;
            *(u_int32_t *)buff = mask;
            break;
        case ATH_PARAM_RATE_MASK_G:
            mask = (((struct ieee80211com *)sc->sc_ieee)->ic_rate_mask) >> 8*sizeof(u_int32_t);
            mask &= 0x00FFFFFF;
            *(u_int32_t *)buff = mask;
            break;
        case ATH_PARAM_RATE_MASK_B:
            mask = (((struct ieee80211com *)sc->sc_ieee)->ic_rate_mask) >> 8*sizeof(u_int16_t);
            mask &= 0x000000FF;
            *(u_int32_t *)buff = mask;
            break;
        case ATH_PARAM_RATE_MASK_A:
            mask = ((struct ieee80211com *)sc->sc_ieee)->ic_rate_mask;
            mask &= 0x0000FFFF;
            *(u_int32_t *)buff = mask;
            break;    
        case ATH_PARAM_MCS_MASK:
            mask = ((struct ieee80211com *)sc->sc_ieee)->ic_mcs_mask;
            *(int *)buff = mask & 0x00FFFFFF;
            break;
        case ATH_PARAM_MCS_CAP:            
            mask = (((struct ieee80211com *)sc->sc_ieee)->ic_mcs_mask) >> (8*sizeof(u_int32_t) - 8);
            mask &= 0xFF;
            *(u_int32_t *)buff = mask;
            break;
        /*End: added by zhanghu for increasing rate*/
		...
		case ATH_OPENESSID_PARAM:
			*(u_int32_t *)buff = openessid_param;
			printk("openessid_param : 0x%x\n",*(u_int32_t *)buff);
			ieee80211_display_list();
			break;
