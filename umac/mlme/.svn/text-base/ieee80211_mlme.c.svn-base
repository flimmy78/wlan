...
#define WAIT_RX_INTERVAL 10000
    ...
    case IEEE80211_M_BTAMP:
		...
		ieee80211_allsta_leave_send_event(vap, 2, 4);
#endif
		/*suzhaoyu add end*/
        wlan_iterate_station_list(vap, sta_disassoc, NULL);
        break;
...
int wlan_mlme_cancel(wlan_if_t vaphandle)
{
        ...
        (mlme_priv->im_request_type != MLME_REQ_NONE)) 
    {
        ...
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,"%s: %s", __func__, "Trigger early timeout\n");
                              ...
                              u_int8_t *challenge, u_int8_t challenge_length, wbuf_t wbuf)
{
    ...
    case IEEE80211_M_HOSTAP:
	  IEEE80211_NOTE_WBUF_MGMT_DEBUG(vap, wbuf, " <INFO> [Step 02 - RECV AUTH] %s: mode = HOSTAP(6)\n", __func__);
...
#if AUTELAN_SOLUTION2
			ni->ni_maintype = 4;
			ni->ni_subtype = reason_code;
		IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
		  	" <INFO> [Step 05 - RECV DEAUTH] %s: recv station deauth(0x%02X%04X), report delete station\n", 
...
void ieee80211_mlme_recv_disassoc(struct ieee80211_node *ni, u_int32_t reason_code)
{
    ...
    case IEEE80211_M_BTAMP:
        if (ni != vap->iv_bss) {
			...
			ni->ni_subtype = reason_code;
		IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
		  	" <INFO> [Step 06 - RECV DISASSOC] %s: recv station disassoc(0x%02X%04X), report delete station\n", 
		  	__func__, ni->ni_maintype, ni->ni_subtype);
			ieee80211_sta_leave_send_event(ni);
#endif
			/*suzhaoyu add end*/
...
static void mlme_timeout_callback(struct ieee80211vap *vap, IEEE80211_STATUS  ieeeStatus)
{
    ...
    case MLME_REQ_JOIN_INFRA:
        ...
        if (MLME_STOP_WAITING_FOR_JOIN(mlme_priv) == TRUE) {
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,"%s: %s", __func__, "Cancelled the Join Operation as it took too long\n");
   ...
   case MLME_REQ_JOIN_ADHOC:
    ...
    case MLME_REQ_AUTH:
         ...
         */
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,"%s: %s", __func__, "Cancelled the Auth Operation as it took too long\n");
    ...
    case MLME_REQ_ASSOC:
         ...
         */
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,"%s: %s", __func__, "Cancelled the Assoc Operation as it took too long\n");
    ...
    case MLME_REQ_REASSOC:
         ...
         */
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME,"%s: %s", __func__, "Cancelled the Reassoc Operation as it took too long\n");
...
extern u_int32_t ps_lmac;  //Add by chenxf for powersave performance 2014-06-05
...
ieee80211_mlme_node_pwrsave(struct ieee80211_node *ni, int enable)
{
    ...
    /* Begin:Modified by chenxf for powersave performance 2014-06-05 */
	if (ps_lmac) // chenxf add for powersave perf
		ieee80211_mlme_node_exit_pwrsave_ap(ni, enable); // deal with exit PS issues
	else {

    	ieee80211_mlme_node_pwrsave_ap(ni,enable);  // default flows
	}
	/* End:Modified by chenxf for powersave performance 2014-06-05 */
}
