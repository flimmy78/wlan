...
extern void init_openessid(struct ieee80211vap *vap);
/* AUTELAN-Added-End : tuqiang@autelan.com */

/*zhaoyang1 transplant from 717*/
unsigned int ic_mac = 0,vap_mac = 0; //duanmingzhe added for redefine the creating rule of vap mac
/*zhaoyang1 transplant end*/
...
#endif
/*zhaoyang1 transplant from 717*/
/*ljy--add begin to redefine the creating rule of vap mac*/
...
/*zhaoyang1 transplant from 717*/
/*add:pengruofeng delete redeclaration 2011-6-10*/
#if 0
...
#endif
/*end:pengruofeng delete redeclaration 2011-6-10*/
/*zhaoyang1 transplant end*/
...
static void autelan_wmm_init(struct ieee80211vap *vap) {
...
static void ath_vap_iter_vap_create(void *arg, wlan_if_t vap) 
{
	...
	/*zhaoyang1 transplant from 717*/
    /*Begin:duanmingzhe added for redefine the creating rule of vap mac*/
    ...
    {
        vap_mac = 0;
        memcpy(&vap_mac, &(vap->iv_myaddr[3]), 3);
        vap_mac = vap_mac >> 8;
        if (vap_mac < ic_mac)
        vap_mac += 0x1000000;
        (*pid_mask) |= (1 << NEW_ATH_GET_VAP_ID(ic_mac, vap_mac));
    }
    /*End:duanmingzhe added for redefine the creating rule of vap mac*/
	/*zhaoyang1 transplant end*/
}
               ...
               const u_int8_t      bssid[IEEE80211_ADDR_LEN])
{
    ...
    {
        memcpy(&ic_mac, &(ic->ic_myaddr[3]), 3);
        ic_mac = ic_mac >> 8;
    }
	/*ljy--add end*/
	/*zhaoyang1 transplant end*/
...
#endif 
	/*ljy--modified begin to redefine the creating rule of vap mac*/
	if (new_dispatch_mac == 0)
	{
		ieee80211vap_get_macaddr(vap, myaddr);
		ATH_SET_VAP_BSSID(myaddr, wlan_vap_get_hw_macaddr((wlan_if_t)vap), id);
	...
	else if (new_dispatch_mac == 2)
	{
		...
		NEW_ATH_SET_VAP_BSSID(vap_mac, id);
		vap_mac = vap_mac << 8;
		memcpy(&(vap->iv_myaddr[3]), &vap_mac, 3);
	}
	/*ljy--modified end*/
	/*zhaoyang1 transplant end*/
    ...
    vap->iv_osif_stop = osif_vap_stop;     
    vap->iv_kick_all = osif_vap_init; 
    ...
    /*Start: Add by zhanghu to statistics of every frame by the rate*/
    {
        ...
        memset(vap->iv_tx_mcs_count, 0, sizeof(vap->iv_tx_mcs_count));    
        memset(vap->iv_rx_mcs_count, 0, sizeof(vap->iv_rx_mcs_count));
        
        memcpy(vap->iv_tx_rate_index, init_value, sizeof(init_value));
        memcpy(vap->iv_rx_rate_index, init_value, sizeof(init_value));    
    }
    /*End: Add by zhanghu to statistics of every frame by the rate*/

    /*zhaoyang1 transplant from 717*/
	autelan_wmm_init(vap);	/*pengruofeng--add for autelan private wmm 2011-6-1*/
	/*zhaoyang1 transplant end*/
...
static void node_traffic_limit_timer_fn(unsigned long data)
{
	struct ieee80211_node *ni = (struct ieee80211_node *) data;
	u_long current_tick = jiffies;

	/*ljy--add begin to avoid null pointer*/
	if (NULL == ni)
	{
		return;
	}
	...
	if (ni->ni_traffic_limit)
	{
		...
		if (time_after(current_tick, (ni->ni_traffic_tick + HZ)))
		{
			ni->ni_traffic_tick += HZ / 5;
			
			if (atomic_read(&(ni->ni_packet_number)) >= ni->ni_packet_max_number / 5)
			{
				atomic_sub((int)(ni->ni_packet_max_number / 5), &(ni->ni_packet_number));
			}
			else
			{
				atomic_set(&(ni->ni_packet_number), 0);
			}

			/*ljy--modified begin to separate traffic limit between rx and tx*/
			#if 0
			if (atomic_read(&(ni->ni_packet_number_send)) >= ni->ni_packet_max_number / 5)
			{
				atomic_sub((int)(ni->ni_packet_max_number / 5), &(ni->ni_packet_number_send));
			}
			#endif
			if (atomic_read(&(ni->ni_packet_number_send)) >= ni->ni_packet_max_number_send / 5)
			{
				atomic_sub((int)(ni->ni_packet_max_number_send / 5), &(ni->ni_packet_number_send));
			}
			/*ljy--modified end*/
			else
			{
	...
	else if (ni->ni_vap->every_node_traffic_limit)
	{
		if (ni->ni_traffic_tick == -1)
			ni->ni_traffic_tick = current_tick;
		...
		if (time_after(current_tick, (ni->ni_traffic_tick + HZ)))
		{
			...
			if (atomic_read(&(ni->ni_packet_number)) >= ni->ni_vap->every_node_packet_max_number / 5)
			{
				atomic_sub((int)(ni->ni_vap->every_node_packet_max_number / 5), &(ni->ni_packet_number));
			}
			else
			{
				atomic_set(&(ni->ni_packet_number), 0);
			...
			/*ljy--modified begin to separate traffic limit between rx and tx*/
			#if 0
			if (atomic_read(&(ni->ni_packet_number_send)) >= ni->ni_vap->every_node_packet_max_number / 5)
			{
				atomic_sub((int)(ni->ni_vap->every_node_packet_max_number / 5), &(ni->ni_packet_number_send));
			}
			#endif
			if (atomic_read(&(ni->ni_packet_number_send)) >= ni->ni_vap->every_node_packet_max_number_send / 5)
			{
				atomic_sub((int)(ni->ni_vap->every_node_packet_max_number_send / 5), &(ni->ni_packet_number_send));
			}
			/*ljy--modified end*/
			else
			{
	...
	mod_timer(&(ni->traffic_limit_timer), current_tick + HZ / 5);
}
static int node_setup_timer(struct ieee80211_node * ni)
{

	/*init traffic_limit-timer*/
	init_timer(&(ni->traffic_limit_timer));

	ni->traffic_limit_timer.function = node_traffic_limit_timer_fn;
	ni->traffic_limit_timer.data = (unsigned long) ni;
	ni->traffic_limit_timer.expires = jiffies + HZ / 5;
	/* init the max packet number*/
	ni->ni_packet_max_number = 102400000;

	/*ljy--add begin to separate traffic limit between rx and tx*/
	ni->ni_packet_max_number_send = 102400000;
	/*ljy--add end*/

	/*ljy--add begin to adjust policy of traffic limit*/
	ni->ni_traffic_tick = -1;
	ni->ni_traffic_limit = 0;
	atomic_set(&(ni->ni_packet_number), 0);
	atomic_set(&(ni->ni_packet_number_send), 0);
	/*ljy--add end*/

	add_timer(&(ni->traffic_limit_timer));
	return 0;
...
ath_net80211_node_alloc(struct ieee80211_node_table *nt, struct ieee80211vap *vap, bool tmpnode)
{
	...
	/*zhaoyang1 transplant from 717*/
    /*Begin:Added by duanmingzhe for traffic limit*/
    node_setup_timer(&anode->an_node);
    /*End:Added by duanmingzhe for traffic limit*/
	/*zhaoyang1 transplant end*/
#endif
...
ath_net80211_node_free(struct ieee80211_node *ni)
{
	...
	/*zhaoyang1 transplant from 717*/
    /*Begin:Added by duanmingzhe for traffic limit*/
    del_timer(&(ni->traffic_limit_timer));
    /*End:Added by duanmingzhe for traffic limit*/
#endif
    /*suzhaoyu add for customer online-traffic limit*/
    OS_FREE_TIMER(&ni->online_traffic_timer);
    /*suzhaoyu addend*/
	/*zhaoyang1 transplant end*/
    scn->sc_node_free(ni);
...
ath_net80211_node_getrate(const struct ieee80211_node *ni, u_int8_t type) 
{
    ...
    /*added by chenming*/
    case IEEE80211_LASTRATE_RX:
        return (ATH_NODE_NET80211(ni)->an_lastrxrate) ;
    case IEEE80211_RATECODE_RX:
        return (ATH_NODE_NET80211(ni)->an_rxratecode);
    /*added by chenming*/
    default:
        return 0;
    }
...
/**********************************************
 *author:packet power added by chenming
 *description:calculate the packet power by rssi
************************************************/
u_int16_t
ath_node_get_packet_power(struct ieee80211_node *ni,struct ath_softc *sc)
{
#define REDUCE_MAXPOW_BY_TWO_CHAIN 6
#define REDUCE_MAXPOW_BY_THREE_CHAIN 9
    u_int32_t hal_txchainmask;
    u_int8_t rssi,tx_numchains,index = 0;
    int16_t maxtxpower = ieee80211_node_get_txpower(ni);
    tx_numchains = sc->sc_tx_numchains;
    rssi = ni->ni_rssi;
    switch (rssi)
    {
    ...
    ni->ni_packet_power = ath_packet_power[index].txpower ;
    switch(tx_numchains)
    {
    case 1:
        break;
    case 2:
        maxtxpower -= REDUCE_MAXPOW_BY_TWO_CHAIN ;
        ni->ni_packet_power -= REDUCE_MAXPOW_BY_TWO_CHAIN ;
        maxtxpower = MAX(maxtxpower,0);
        ni->ni_packet_power = MAX(ni->ni_packet_power,0);
        break;
    case 3:
        maxtxpower -= REDUCE_MAXPOW_BY_THREE_CHAIN ;
        ni->ni_packet_power -= REDUCE_MAXPOW_BY_THREE_CHAIN ;
        maxtxpower = MAX(maxtxpower,0);
        ni->ni_packet_power = MAX(ni->ni_packet_power,0);
        break;
    default:
        printk("%s[%d]Unsupported number of chains\n",__func__,__LINE__);
        ASSERT(0);
        break;
    }
    ni->ni_packet_power = MIN(maxtxpower,ni->ni_packet_power);
    return ni->ni_packet_power;
#undef REDUCE_MAXPOW_BY_TWO_CHAIN 
#undef REDUCE_MAXPOW_BY_THREE_CHAIN 
}
/*ended by chenming*/
int
...
/*ended by chenming*/
    ...
    txctl->keytype = keytype;
/*packet power added by chenming */
    if(vap->iv_packet_power_enable && IEEE80211_IS_DATA(wh))
    {
        txctl->txpower = ath_node_get_packet_power(ni,sc);
    }
    else
    {
        txctl->txpower = ieee80211_node_get_txpower(ni);
...
/*ended by chenming*/
	...
	if (type == IEEE80211_FC0_TYPE_DATA && subtype != IEEE80211_FC0_SUBTYPE_NODATA) {
        ...
        if (subtype & IEEE80211_FC0_SUBTYPE_QOS) {
             ...
             * Default all non-QoS traffic to the best-effort queue.
             */
            /*huangtao added for adjust mcast queue*/ 
            if((txctl->ismcast) && (mcast_queue_debug != WME_AC_BE)){
                //printk("[%d] mcast_queue_debug = %d\n",__LINE__,mcast_queue_debug);
                wbuf_set_priority(wbuf, mcast_queue_debug);
                acnum = wbuf_get_priority(wbuf) & 0x03;
            }else{
            /*huangtao added end*/
                wbuf_set_priority(wbuf, WME_AC_BE);
...
extern int IPADDR_PRASE(wbuf_t wbuf, struct ieee80211_node *ni);
...
ath_net80211_rx(ieee80211_handle_t ieee, wbuf_t wbuf, ieee80211_rx_status_t *rx_status, u_int16_t keyix)
{
    ...
    struct ath_softc_net80211 *scn = ATH_SOFTC_NET80211(ic);
	/*zhaoyang1 transplant from 717*/
	/*pengruofeng add start for mgt stats 2011-5-12*/
	struct ath_softc *sc = ATH_DEV_TO_SC(scn->sc_dev);
...
/*<Begin : caizhibang modify for autelan 802.3 tunnel 2012-5-25 */    
    ...
    if (thinap) {
    	/**************************************************************************
    	* add prism2 header
    	* alloc space with prism2 header for each frame 
    	* and add each fame with peism2 header.
    	* 
    	**************************************************************************/
    	...
    	u_int32_t signal;
    	u_int32_t rate;
    	tsf = ath_hal_gettsf32(ATH_DEV_TO_SC(scn->sc_dev)->sc_ah);
    	rssi = 0;
    	signal = 0;
    	rate = 0;
    	ph = (wlan_ng_prism2_header *)wbuf_push(wbuf, sizeof(wlan_ng_prism2_header));
    	*ph = template;
    	ph->hosttime.data = jiffies;
    	ph->mactime.data = tsf;
    	ph->frmlen.data = wbuf->len - sizeof(wlan_ng_prism2_header);
    	/* XXX no way to pass channel flag state */
    	ph->channel.data = ieee80211_chan2ieee(ic, ic->ic_curchan);
    	ph->rssi.data = rssi;
    	ph->signal.data = signal;
    	ph->rate.data = rate;
    	/* add prism2 header finish*/
    	wbuf_pull(wbuf, sizeof(wlan_ng_prism2_header));/*jump the prism2 header to the ieee80211 data*/
    }
#endif	
/*End : caizhibang modify for autelan 802.3 tunnel 2012-5-25 >*/    
    ...
    */
	...
	/* Begin:gengzongjie added for sending arp packets to assoc sta in wifipos function 2014-1-2 */
    if(wifipos_ping_send_enable == 1)
	{
		IPADDR_PRASE(wbuf,ni);
	}
	/* End:gengzongjie added end */
    ...
    */
    ...
    if (status != ATH_RX_CONSUMED) {
		/*zhaoyang1 transplant from 717*/
		/*pengruofeng add start for mgt stats 2011-5-12*/
		if ((wh->i_fc[0] & IEEE80211_FC0_TYPE_MASK) == IEEE80211_FC0_TYPE_MGT && (wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK) != IEEE80211_FC0_SUBTYPE_BEACON)
		{			
...
/* Begin:gengzongjie added for sending arp packets to assoc sta in wifipos function 2014-1-2 */
extern int WIFIPOS_PING_SEND( struct ieee80211vap *vap );
/* End:gengzongjie added end */
...
void
ath_net80211_ps_check(ieee80211_handle_t ieee, struct ieee80211_frame *wh, u_int16_t keyix, bool isr_context)
{
    struct ieee80211com *ic = NET80211_HANDLE(ieee);
    struct ath_softc_net80211 *scn = ATH_SOFTC_NET80211(ic);
    struct ieee80211_node *ni;
    ...
    if (ni == NULL) {
         ...
         */
        if (isr_context) {
            ni = ieee80211_find_rxnode_nolock(ic, (struct ieee80211_frame_min *)wh);
        }
        else {
    ...
    if ((vap->iv_opmode == IEEE80211_M_HOSTAP) && 
        (ni != vap->iv_bss) &&
        !((wh->i_fc[0] & IEEE80211_FC0_TYPE_MASK) == IEEE80211_FC0_TYPE_MGT && (wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK) == IEEE80211_FC0_SUBTYPE_PROBE_REQ))
    {
        if ((wh->i_fc[1] & IEEE80211_FC1_PWR_MGT) ^
            (ni->ni_flags & IEEE80211_NODE_PWR_MGT)) {
			
			if(ni->ni_vap->iv_opmode == IEEE80211_M_HOSTAP)
			    ieee80211_mlme_node_enter_pwrsave_ap(ni, wh->i_fc[1] & IEEE80211_FC1_PWR_MGT); // interrupt context only deal with entering PS issues
        }
    }
...
struct ieee80211_ops net80211_ops = {
	...
	ieee80211_dump_pkt_debug,
	/*zhaoyang modify end*/
	WIFIPOS_PING_SEND,	//gengzongjie added for sending arp packets to assoc sta in wifipos function 2013-12-31 */
    ath_net80211_ps_check,  /* aute_ps_check */ // Add by chenxf for powersave performance 2014-06-05
};
           ...
           struct hal_reg_parm *hal_conf_parm, IEEE80211_REG_PARAMETERS *ieee80211_conf_parm)
{
    ...
    ic->ic_rate_mask = 0x8000FFFF00FF15FFLL;
    ic->ic_rate_mask_bak = ic->ic_rate_mask;
    ic->ic_mcs_mask = 0x80FFFFFF;
	...
	/*zhaoyang1 transplant from 717*/
	/*
     * Default 11.h to start disabled.
     */

    /* yanggs modify for disable 11h by default*/
...
/* Begin:gengzj added for rrm 2013-11-26 */
	ic->ic_iwlist_listen_time_each_chan = 40;
/* End:gengzongjie added end */

	/* Begin:gengzj added for suppressing illegal ap 2013-11-27 */
	/*zhaoyang1 add start for interfering ap*/
	memset(ic->ic_autelan_rogue_ap_channel_list, 
			0, 
			64 * sizeof (ic->ic_autelan_rogue_ap_channel_list[0]));
	u_int8_t i = 0;
	for (; i < 64; i++) {
		spin_lock_init(&(ic->ic_autelan_rogue_ap_channel_list[i].autelan_rogue_ap_lock));
	}
	ic->ic_rogue_ap_restrain = 0;
	/*zhaoyang1 add end*/
...
ath_detach(struct ath_softc_net80211 *scn)
{
	...
	ic->ic_autelan_clr_blacklist(ic); //zhaoyagn1 add for rogue ap
...
ath_suspend(struct ath_softc_net80211 *scn)
{
    ...
    /*lijiyong add begin to debug the bug of ENAEX-13*/
    if (NULL == scn)
    {
        printk(KERN_EMERG "%s: Debug ENAEX-13, scn is null\n", __func__);
        if (kes_debug_print_handle)
        {
            kes_debug_print_handle(KERN_EMERG "%s: Debug ENAEX-13, scn is null\n", __func__);
        }
    }
    /*lijiyong add end*/
