...
int ieee80211_scan_table_iterate(ieee80211_scan_table_t scan_table, ieee80211_scan_iter_func shandler, void *arg) 
{
	...
	/*zhaoyang modify for scan result limit 60*/
	int count = 0;
	/*zhaoyang modify end*/
...
/* Begin: gengzj added for wifipos 2013-11-26 */
		if(iwlist_scan_chan_num)
/* End: gengzj added end */
		{
			if(wlan_channel_ieee(wlan_scan_entry_channel(scan_entry))!=iwlist_scan_chan_num)
			continue;
		}
		/*suzhaoyu add end*/
		/*zhaoyang1 transplant end*/
/* Begin:zhaoyang1 added for rrm result aging 2013-12-26 */
		if ((long unsigned int)wlan_scan_entry_age(scan_entry)> 5000* 60) { 
			wlan_scan_entry_remove(scan_table, scan_entry);
			continue;
    	}
 ...
 */
/* Begin:zhaoyang1 added for rrm result aging 2013-12-26 */
void ieee80211_scan_entry_remove(wlan_scan_table_t scan_table, 
...
int wlan_scan_table_iterate(wlan_if_t vaphandle, ieee80211_scan_iter_func shandler, void *arg)
{
	...
	if (vaphandle->iv_ic->ic_iwlist_scan_chan) {
		iwlist_scan_chan_num = vaphandle->iv_ic->ic_iwlist_scan_chan;
	} else if (vaphandle->iv_ic->ic_iwlist_scan_onebyone) {
		iwlist_scan_chan_num=vaphandle->iwlist_scan_chan_num;
	} else {
		iwlist_scan_chan_num = 0;
	}
...
/* Begin:zhaoyang1 added for rrm result aging 2013-12-26 */
void wlan_scan_entry_remove( wlan_scan_table_t scan_table, 
								wlan_scan_entry_t scan_entry)
{
    ieee80211_scan_entry_remove(scan_table, scan_entry);
}
/* End:zhaoyang1 added end */

#include "ieee80211_openessid.c"
#endif
