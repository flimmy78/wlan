                       ...
                       scan_request_data_t *request_data)
{
	...
	//wlan_scan_table_flush(ss->ss_vap);	//zhaoyang1 added for rrm result aging 2013-12-26 
	if(!ss->ss_vap->iv_ic->ic_iwlist_scan_chan && \
		ss->ss_vap->iv_ic->ic_iwlist_scan_onebyone && \
		vaphandle->iwlist_scanning && \
		ss->ss_vap->iv_opmode == IEEE80211_M_HOSTAP)
    {
	    if(vaphandle->iv_des_mode!=vaphandle->iwlist_scan_lastmode)
	    {
	    	vaphandle->iwlist_scan_current_chan = 0;
		vaphandle->iwlist_scan_lastmode = vaphandle->iv_des_mode;
	    }
	    ss->ss_next=vaphandle->iwlist_scan_current_chan;
    	    //wlan_scan_table_flush(ss->ss_vap);
    }
                                          ...
                                          void                         *arg)
{
    ...
    if (ss->ss_num_handlers >= IEEE80211_MAX_SCAN_EVENT_HANDLERS) {
        IEEE80211_SCAN_PRINTF(ss, IEEE80211_MSG_SCAN, "%s: ERROR: No more space: evhandler=%08p arg=%08p\n",
                              __func__, 
                              evhandler,
                              arg);
        spin_unlock(&ss->ss_lock);
                                            ...
                                            void                         *arg)
{
    ...
    IEEE80211_SCAN_PRINTF(ss, IEEE80211_MSG_SCAN, "%s: Failed to unregister evhandler=%08p arg=%08p\n",
                          __func__, 
                          evhandler,
                          arg);
    return ENXIO;
                                ...
                                IEEE80211_SCAN_PRIORITY  scan_priority)
{
    if ((scan_id != 0) && (ss->ss_scan_id != scan_id)) {
        IEEE80211_SCAN_PRINTF(ss, IEEE80211_MSG_SCAN, "%s: Invalid scan_id=%d ss_scan_id=%d\n",
                              __func__, 
                              scan_id,
                              ss->ss_scan_id);
            ...
            default:     /* Resource Manager not active. */
                ...
                if (ss->ss_vap->iv_bsschan ){
...
#endif
                    {
						...
						//ss->ss_ic->ic_curchan->ic_flags = ss->ss_ic->ic_prevchanflag;
...
static bool scanner_check_endof_scan(ieee80211_scanner_t ss)
{
    ...
    if(!ss->ss_vap->iv_ic->ic_iwlist_scan_chan && \
		ss->ss_vap->iv_ic->ic_iwlist_scan_onebyone && \
		1 == ss->ss_vap->iwlist_scanning && \
		ss->ss_vap->iv_opmode == IEEE80211_M_HOSTAP) {
	/* End:gengzongjie added end */
	
		ss->ss_vap->iwlist_scanning = 0;
		if (ss->ss_next >= ss->ss_nchans) {
		        if (ss->ss_flags & IEEE80211_SCAN_CONTINUOUS) {
		            ...
		            ss->ss_vap->iwlist_scan_current_chan = 0;
		            return true;
		        }

		  } else {
			     if(ss->ss_vap->iwlist_scanning_cancl)
			     {
					ss->ss_vap->iwlist_scanning_cancl=0;
					return true;
			     }
		  }
    }
...
static void scanner_state_foreign_channel_entry(void *context)
{
    ieee80211_scanner_t    ss = context;
    u_int32_t              now = (u_int32_t) CONVERT_SYSTEM_TIME_TO_MS(OS_GET_TIMESTAMP());
	/* Begin:gengzj added for suprressing illegal ap 2013-11-27 */
    ...
    if (!ieee80211_resmgr_active(ss->ss_ic) ||
		...
		/*Begin:Modify by suzhaoyu for AP mode error when open rrm 2012-9-18*/
       // ss->ss_ic->ic_prevchanflag = ss->ss_ic->ic_curchan->ic_flags;
		/*End:Modify by suzhaoyu for AP mode error when open rrm 2012-9-18*/
		/* Begin:gengzj added for suppressing illegal ap 2013-11-27 */
        /*zhaoyang1 add start for interfere ap*/
		if (NULL == ss->ss_chans[ss->ss_next]) {
			printk("%s  %d  ss->ss_next = %d\n", __func__, __LINE__, ss->ss_next);
			ss->ss_next = 0;
		}
		/*zhaoyang1 add end*/
		/* End:gengzj added end */
	...
	if(!ss->ss_vap->iv_ic->ic_iwlist_scan_chan && \
		ss->ss_vap->iv_ic->ic_iwlist_scan_onebyone && \
		ss->ss_vap->iwlist_scanning && \
		ss->ss_vap->iv_opmode == IEEE80211_M_HOSTAP ) {
	/* End:gengzongjie added end */
		ss->ss_vap->iwlist_scan_chan_num =wlan_channel_ieee(ss->ss_chans[ss->ss_next]);
	}
	/*suzhaoyu add end*/
	/*zhaoyang1 transplant end*/
    }
    ...
    if (is_passive_channel(ss->ss_chans[ss->ss_next], ss->ss_flags)) {
		...
		/* Begin:gengzj added for suppressing illegal ap 2013-11-27 */
		/*zhaoyang1 add start interfering ap*/
		if (ic->ic_rogue_ap_restrain) {

			struct autelan_rogue_ap_channel_list *chan_list = 
							&ic->ic_autelan_rogue_ap_channel_list[id(ic->ic_curchan->ic_ieee)];
			spin_lock(&chan_list->autelan_rogue_ap_lock);
			struct autelan_rogue_ap_list *rogue_ap = chan_list->rogue_ap;

			while (rogue_ap) {
				autelan_send_deauth(vap->iv_bss, IEEE80211_REASON_UNSPECIFIED, rogue_ap->bssid);
    ...
    if(!ss->ss_vap->iv_ic->ic_iwlist_scan_chan && \
		...
		ss->ss_vap->iwlist_scanning && \
		ss->ss_vap->iv_opmode == IEEE80211_M_HOSTAP) {
    		ss->ss_vap->iwlist_scanning_cancl = 1;
	    	ss->ss_vap->iwlist_scan_current_chan = ss->ss_next;
    }
	/* End:gengzongjie added end */
    /*suzhaoyu add end */
	/*zhaoyang1 transplant end*/
...
int ieee80211_scan_detach(ieee80211_scanner_t *ss)
{
    ...
    if ((*ss)->ss_num_handlers > 0) {
        IEEE80211_SCAN_PRINTF((*ss), IEEE80211_MSG_SCAN, "%s: Event handler table not empty: %d entries\n",
                              __func__, 
                              (*ss)->ss_num_handlers);
        ...
        for (i = 0; i < IEEE80211_MAX_SCAN_EVENT_HANDLERS; ++i) {
            if ((*ss)->ss_event_handlers[i] != NULL) {
                IEEE80211_SCAN_PRINTF((*ss), IEEE80211_MSG_SCAN, "%s: evhandler=%08p arg=%08p not unregistered\n",
                                      __func__, 
