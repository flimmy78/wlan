...
**
*/
#include <linux/time.h>//AUTELAN-zhaoenjuan transplant for drv_monitor
#include "wlan_opts.h"
...
#include "ratectrl.h"
#include "ath_aow.h"
//AUTELAN-Begin:zhaoenjuan transplant for drv_monitor
#include "osif_private.h"
#include "if_athvar.h"
#include "ar9300/ar9300reg.h"
//AUTELAN-End:zhaoenjuan transplant for drv_monitor
...
#endif
/*AUTELAN-zhaoenjuan transplant for drv_monitor interface up & down*/
#define IS_UP(_dev) \
...
#endif
extern void kernel_restart(char *cmd);//AUTELAN-zhaoenjuan added for kernel restart
...
u_int32_t drv_rx_buffer_dump(struct ath_softc *sc);//AUTELAN-zhaoenjuan transplant for drv_monitor
...
#endif
/*<AUTELAN-Begin:zhaoenjuan added for drv monitor*/
...
//add by zhaoej for ar9300 register print
typedef struct {
	const char*	label;
	u_int		reg;
} HAL_REG;
...
extern int (*kes_debug_print_handle)(const char *fmt, ...);
extern void (*kes_debug_print_flag_handle)(const char *s); //AUTELAN:zhaoenjuan added indicate restart is hot restart
...
u_int32_t soft_tx_buffer_monitor(struct ath_softc *sc)
{
    ...
    static u_int32_t last_tx_from_stack[2] = {0};
	u_int8_t wifi_num = 0;
    ...
    //lutao tag ast_tx_from_stack表示协议栈下来的报文计数，后续添加
    //if(last_tx_from_stack != sc->sc_stats.ast_tx_from_stack)  
    {
        ...
        if(pkts_sum == last_pkts_sum[wifi_num])
        {
            ...
            if(last_nobuffs_sum[wifi_num] != nobuf_sum)
            {
                ...
                printk("packet tx cur=%d,old=%d,nobuf=%d,oldnobuf=%d\n",pkts_sum,last_pkts_sum[wifi_num],sc->sc_stats.ast_txq_nobuf[1],last_nobuffs_sum[wifi_num]);

                /*if ac-tid link has enough packets, but hw queue is full, and 
				can not send any packet success by HW, HW stuck
				not find yet, so undefine below code, lutao tag*/
                #if 0				
                if(hw counter no change)
                {
                    ...
                    last_pkts_sum = pkts_sum;
                    last_nobuffs_sum = sc->sc_stats.ast_txq_nobuf[1];
...
u_int32_t soft_rx_buffer_monitor(struct ath_softc *sc)
{
    ...
    if (atomic_read(&sc->sc_nap_vaps_up)) //check whether any vap is up
    {
        ...
        {    
            rx_intr_last_times[wifi_num] = sc->sc_stats.ast_rx;
        }
        ...
        if (2 < rx_stuck_times[wifi_num]) //check whether the rx stuck times greater than 2
        {   
...
void aute_dump_desc(struct ath_softc *sc,struct ath_buf *bf,int qnum)
{
	struct ath_desc *ds;
	int  i = 0;

	printk("%s:  bf_daddr %08x len %d"
	    "TXDP %08x  \n",__func__, 
...
void aute_dump_tid_buf(struct ath_softc *sc)
{
    ...
    u_int32_t q_num;
	int axq_cnt,j,bufQCount,tidBufcnt,pertidBufcnt;
	struct ath_buf *buf;
...
void aute_tx_node_queue_stats(struct ath_softc *sc)
{
    ...
    TAILQ_FOREACH(ni, &nt->nt_node, ni_list)
    {
        ...
        for (tidno = 0; tidno < WME_NUM_TID; tidno++) {
          ...
          printk("%s: TID: %d  #frames: %d seqstart %d paused %d sched %d pause-period sec:%d,us:%d addtrace(%d|%d|%d|%d|%d|%d|%d|%d)  subtrace(%d|%d|%d|%d|%d|%d|%d|%d) add index=%d,sub index=%d\n"
                  ,__func__, tidno, count, tid->seq_start,tid->paused,tid->sched,tid->paused_period_sec,tid->paused_period_us,
...
void aute_kernel_restart(u_int32_t error_code)
{
#if defined(PC018) || defined(APM82181) 
    {
        ...
        if(kes_debug_print_handle)
        {
            ...
            kes_debug_print_handle(KERN_EMERG "Autelan drv monitor:watchdog Restating system...\n");
        }
        OS_DELAY(1000000);
...
#ifndef APM82181 
    {
        ...
        OS_DELAY(1000000);
        kes_debug_print_flag_handle("E");
        kernel_restart("Autelan drv monitor: Restating system...\n");
    }
#endif
}
...
OS_TIMER_FUNC(autelan_drv_monitor)
{
...

	{
        ...
        sc->sc_reset_type = ATH_RESET_DEFAULT;
        sc->sc_stats.ast_resetOnError++;
        printk("reset radio by manual.\n");
		autelan_drv_monitor_enable = (autelan_drv_monitor_enable & 0x0f);
        if(kes_debug_print_handle)
        {
            kes_debug_print_handle(KERN_EMERG "reset radio by manual.\n");
        }
    }
    ...
    do{
         ...
         if((loop_num[i] % 180)  == 0) //per 3 min check
         {
		 ...
		 if((loop_num[i] % 5)  == 0)//per 5 sec check
    	 {
...
handle:
    ...
    switch (error_code)
    {
        ...
        case AUTE_DRV_HW_STUCK_ERR:
            reboot_flag = drv_hw_buffer_dump(sc);//if the monitor need not reboot the system, set the flag to zero
            break;
    ...
    if(autelan_drv_monitor_reboot > 0)
    {
        ...
        {
            if(loop_num[i] > 180) //60*3*1000/autelan_drv_monitor_period, 3minutes
            {
                if(loop_num[i] < last_error)
                    last_error = 0;
                
                if((loop_num[i] - last_error) <= 18)
                {
                    aute_kernel_restart(error_code);
                }
                else 
                    last_error = loop_num[i];
            }
        }
...
/*AUTELAN-End:zhaoenjuan added for drv monitor >*/

/* Begin:gengzongjie added for sending arp packets to assoc sta in wifipos function 2013-12-31 */
u_int32_t wifipos_ping_send_period = 1000;//ms

u_int32_t wifipos_ping_send(struct ath_softc *sc)
{
    struct ieee80211vap *vap = NULL;
    struct ieee80211com *ic =NULL;
    ic = (struct ieee80211com *)(sc->sc_ieee);
    ...
    if((wifipos_ping_send_enable == 1))
    {
        TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next)
        { 
            if(NULL != sc->sc_ieee_ops->WIFIPOS_PING_SEND){
                sc->sc_ieee_ops->WIFIPOS_PING_SEND(vap);
                }
        }
    }
	return 0;
...
static
OS_TIMER_FUNC(ping_send_monitor)
{
    struct ath_softc    *sc;
    OS_GET_TIMER_ARG(sc, struct ath_softc *);
    struct ieee80211com *ic =NULL;
    ...
    else
    {
        /*there are associated users, add monitor fun here*/
       do{
            wifipos_ping_send(sc);	//per sec
        } while(0);
    }

    OS_SET_TIMER(&sc->sc_wifipos_ping_send, wifipos_ping_send_period);
...
ath_calibrate(void *arg)
{
    ...
    if (! sc->sc_scanning && !atomic_read(&sc->sc_in_reset) 
        ...
        /*Begin: Added by wangjia, for disable NF. */
        if(longcal) {
            longcal = sc->DisableNF ? AH_FALSE : AH_TRUE;
            // printk("XY DEBUG: longcal %d\n", longcal);
        }
        /*End: Added by wangjia, for disable NF. */
...
**
**  \param dev      Void pointer to ATH object passed from protocol layer
**  \param enable   ndicating whether to enable ForcePPM
...
static int
ath_vap_stopping_lmac(ath_dev_t dev, int if_id)//AUTELAN-zhaoenjuan transplant for drv monitor change ath_vap_stopping to ath_vap_stopping_lmac
{
...
static int
ath_vap_down_lmac(ath_dev_t dev, int if_id, u_int flags)//AUTELAN-zhaoenjuan transplant for drv monitor change ath_vap_down to ath_vap_down_lmac
{
    struct ath_softc *sc = ATH_DEV_TO_SC(dev);
    struct ath_hal *ah = sc->sc_ah;
    struct ath_vap *avp;
...
static int
ath_suspend_lmac(ath_dev_t dev)//AUTELAN-zhaoenjuan transplant for drv monitor change ath_suspend to ath_suspend_lmac
{
    struct ath_softc *sc = ATH_DEV_TO_SC(dev);
    struct ath_hal   *ah = sc->sc_ah;
        ...
        if (status & HAL_INT_FATAL) {
			...
			/*AUTELAN-End:zhaoenjuan transplant for drv monitor stats*/
                ...
                if (status & HAL_INT_RX) {
					sc->rx_intr++;	/*AUTELAN-zhaoenjuan transplant for drv monitor stats*/
        	...
        	/*AUTELAN-End:zhaoenjuan transplant for drv monitor stats*/
                ...
                if (status & HAL_INT_RX) {
                    ...
                    ath_rx_proc_descfast(dev);
#endif
					sc->rx_intr_noedma++;			/*AUTELAN-zhaoenjuan transplant for drv monitor stats*/
                    sched = ATH_ISR_SCHED;
                }
            }
...
ath_handle_intr(ath_dev_t dev)
{
    ...
    int i;
	/*AUTELAN-Added-Begin:Added by duanmingzhe for EIFS*/
	struct ath_hal *ah = sc->sc_ah;
	/*AUTELAN-Added-End:Added by duanmingzhe for EIFS*/
    ...
    do {
        ...
        if (status & HAL_INT_FATAL) {
			...
			/*AUTELAN-Added-Begin:Added by duanmingzhe for EIFS*/
			if(set_eifs) //edit by duanmingzhe
			{
				OS_REG_WRITE(ah,0x10b0,10 );
			}
			/*AUTELAN-Added-End:Added by duanmingzhe for EIFS*/
			
			/*AUTELAN-Added-Begin:Added by duanmingzhe for set 0 to reg(9e24) */
			if(set_reg9e24)
			{
				OS_REG_WRITE(ah,0x00009e24,OS_REG_READ(ah,0x00009e24)&0xFFFFFFFE );
			}	
			/*AUTELAN-Added-End:Added by duanmingzhe for  set 0 to reg(9e24)*/
            /*
...
#define    N(a)    (sizeof(a)/sizeof(a[0]))
    ...
    *ops = &sc->sc_ath_ops;
    ...
    //printk("%s:Related to axq_totalqueued sc->sc_cabq is %d\n",__func__,sc->sc_cabq);
    if(kes_debug_print_handle)
    {
      kes_debug_print_handle(KERN_EMERG "%s:Related to axq_totalqueued sc->sc_cabq is %d\n",__func__,sc->sc_cabq);  
    }
   ...
   if (error) goto bad2;
	...
	/* Begin:gengzongjie transplant for the bug of unable to send packets 2013-5-9 */
     //zhaoenjuan print axq_totalqueued 
    printk("%s:Related to axq_totalqueued sc->sc_xrtxq is %d\n",__func__,sc->sc_xrtxq);
    if(kes_debug_print_handle)
    {
      kes_debug_print_handle(KERN_EMERG "%s:Related to axq_totalqueued sc->sc_xrtxq is %d\n",__func__,sc->sc_xrtxq);  
    }
	/* End: gengzongjie transplant end */
...
OS_INIT_TIMER(sc->sc_osdev, &sc->sc_wifipos_ping_send, ping_send_monitor, sc);
OS_SET_TIMER(&sc->sc_wifipos_ping_send, wifipos_ping_send_period);
/* End:gengzongjie added end */
    ...
    if (sc->sc_hasbmask) {
		...
		else if ((new_dispatch_mac == 1) || (new_dispatch_mac == 2)) {
...
#endif
    OS_CANCEL_TIMER(&sc->sc_drv_monitor);//AUTELAN-zhaoenjuan added for drv monitor



/* Begin:gengzongjie added for sending arp packets to assoc sta in wifipos function 2014-1-2 */
OS_CANCEL_TIMER(&sc->sc_wifipos_ping_send);
/* End:gengzongjie added end */
...
#endif
{
    ...
    struct ath_vap *avp;
    int rx_mitigation;
    u_int32_t error_code;  //AUTELAN:zhaoenjuan transplant for drv_monitor,interface id not create,restart the ap 
...
#else
   /*AUTELAN-Begin:zhaoenjuan transplant for drv_monitor,interface id not create,restart the ap */ 
    if (if_id >= ATH_BCBUF ) {
   ...
   /*AUTELAN-ENd:zhaoenjuan transplant for drv_monitor,interface id not create,restart the ap*/ 
#endif    
...
/*zhaoyang1 transplant from 717*/
/*modified by chenming to get values of all available txpower-issue num PCAPVXN-78*/
void ath_get_maxtxpower(ath_dev_t dev, u_int32_t *txpow)
{
    struct ath_softc *sc = ATH_DEV_TO_SC(dev);
    ath_hal_getmaxtxpower(sc->sc_ah, txpow);
    *txpow = *txpow/2;
}

EXPORT_SYMBOL(ath_get_maxtxpower);
/*end-chenming*/
/*zhaoyang1 transplant end*/
void ath_read_from_eeprom(ath_dev_t dev, u_int16_t address, u_int32_t len, u_int16_t **value, u_int32_t *bytesread)
{
