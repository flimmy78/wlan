...
/*pengruofeng add start for management frame stats 2011-5-12*/
#include "ath_internal.h"
#include "if_athvar.h"
/*pengruofeng add end 2011-5-12*/
...
ieee80211_send_mgmt(struct ieee80211vap *vap,struct ieee80211_node *ni, wbuf_t wbuf, bool force_send)
{
	...
	vap->iv_stats.is_tx_mgmt++;
//	printk("%s (11-5-18-05-111) vap->iv_stats.is_tx_mgmtbytes : %llu\n",__func__,vap->iv_stats.is_tx_mgmtbytes);
//	printk("%s (11-5-18-05-222) wbuf->len : %d \n",__func__,(int)wbuf->len);
	vap->iv_stats.is_tx_mgmtbytes += wbuf->len;
//	printk("%s vap->iv_stats.is_tx_mgmt : %d\n",__func__,vap->iv_stats.is_tx_mgmt);
//	printk("%s (11-5-18-05-333) vap->iv_stats.is_tx_mgmtbytes : %llu\n",__func__,vap->iv_stats.is_tx_mgmtbytes);
//	printk("%s (11-5-18-05-444) wbuf->len : %d \n",__func__,(int)wbuf->len);
	/*pengruofeng add end 2011-5-11*/
    vap->iv_lastdata = OS_GET_TIMESTAMP();
	/*zhaoyang modify for tunnel count CHINAMOBILE-271*/
	IEEE80211_NODE_STAT(ni, tx_mgmt);
	IEEE80211_NODE_STAT_ADD(ni, tx_mgmt, wbuf->len);
	/*zhaoyang modify end*/
	/*zhaoyang1 transplant end*/
    ...
    if (!ieee80211_vap_active_is_set(vap)) {
        ...
        ieee80211_sta_power_tx_end(vap->iv_pwrsave_sta);
        return -ENOMEM; //AUTELAN-Added:duanmingzhe for tid stuck,if drop this frame notified the caller.
    }
	...
	/*zhaoyang1 transplant from 717*/
	/*pengruofeng add start for management frame stats 2011-5-12*/
	vap->iv_stats.is_tx_signaling++;
	sc->sc_stats.ast_tx_mgmt++;
	/*pengruofeng add end 2011-5-12*/
	/*zhaoyang1 transplant end*/

	/* AUTELAN-Added-Begin : &tuqiang for wifiX statistics 2014-07-04 */

	sc->sc_stats.ast_tx_mgmt_bytes += wbuf_get_pktlen(wbuf);
...
ieee80211_send_nulldata(struct ieee80211_node *ni, int pwr_save)
{
    ...
    if(thinap)
    {
        wbuf = wbuf_alloc(ic->ic_osdev, WBUF_TX_MGMT, sizeof(struct ieee80211_frame) + sizeof(wlan_ng_prism2_header));
    }
    else
    {
    ...
    IEEE80211_DPRINTF(vap, IEEE80211_MSG_DEBUG | IEEE80211_MSG_DUMPPKTS,
                      "[%s]%s: send null data frame on channel %u, pwr mgt %s\n",
    ...
    const size_t          optielen)
{
    ...
    if(thinap)
    {
        wbuf = wbuf_alloc(ic->ic_osdev, WBUF_TX_MGMT, MAX_TX_RX_PACKET_SIZE + sizeof(wlan_ng_prism2_header));
    }
    else
    {
        wbuf = wbuf_alloc(ic->ic_osdev, WBUF_TX_MGMT, MAX_TX_RX_PACKET_SIZE);
    ...
    frm = ieee80211_add_rates(frm, &vap->iv_op_rates[mode], ic);
    frm = ieee80211_add_xrates(frm, &vap->iv_op_rates[mode], ic);
    ...
    )
{
	...
	IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
	  	" <FAIL> [Step 02 - SEND AUTH] %s: thinap = %d, vap->iv_traffic_balance = %d\n", 
	  	__func__, thinap, vap->iv_traffic_balance);
		return 0;	
    }
...
int
autelan_send_deauth(struct ieee80211_node *ni, u_int16_t reason, u_int8_t *ap_bssid)
{
    struct ieee80211vap *vap = ni->ni_vap;
    struct ieee80211com *ic = ni->ni_ic;
    wbuf_t wbuf;
    struct ieee80211_frame *wh;
    u_int8_t *frm;
	u_int8_t sta_mac[] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};

	//vap->iv_stats.is_deauth_state++;
    ...
    /* setup the wireless header */
    wh = (struct ieee80211_frame *)wbuf_header(wbuf);
    autelan_send_setup(vap, ni, wh,
                         IEEE80211_FC0_TYPE_MGT | IEEE80211_FC0_SUBTYPE_DEAUTH,
                         ap_bssid, sta_mac, ap_bssid);
    ...
    *(u_int16_t *)frm = htole16(reason);
    frm += 2;
	//memcpy(frm, extra_byte, sizeof(extra_byte));
	//frm += 10;
...
ieee80211_send_disassoc(struct ieee80211_node *ni, u_int16_t reason)
{
	...
	vap->iv_stats.is_disassoc_state++;
    ...
    case IEEE80211_ACTION_CAT_BA: {
        ...
        case IEEE80211_ACTION_BA_ADDBA_REQUEST:
            ...
            //               __func__, tidno, baparamset.buffersize);
							 "%s: ADDBA request action mgt frame. TID %d, buffer size %d startseqno %d",
							 __func__, tidno, baparamset.buffersize,basequencectrl.startseqnum);
			/*zhaoyang modify end*/
            break;
...
#define BEACON_INFO_ELEMENT_OFFSET    (offsetof(struct ieee80211_beacon_frame, info_elements))
    ...
    if (!((IEEE80211_BINTVAL_MIN <= scan_entry_parameters->bintval) &&
          ...
          IEEE80211_DISCARD(vap, IEEE80211_MSG_SCAN,
                            wh, "beacon", "%s: invalid beacon interval (%u)", __func__, 
                    ...
                    struct ieee80211_rx_status *rs)
{
           ...
           if(IEEE80211_ADDR_EQ(wh->i_addr1, tmpvap->iv_myaddr) &&
              (tmpvap->iv_opmode == IEEE80211_M_HOSTAP)) {
               ...
               if (ni != vap->iv_bss) {
                  IEEE80211_NOTE(vap, IEEE80211_MSG_MLME, ni,
                            "%s: %s", __func__, "Removing the node from the station node list\n");
		     IEEE80211_NOTE_MAC_MGMT_DEBUG(vap, wh->i_addr2, 
			 	" <INFO> [Step 02 - RECV AUTH] %s: station exist in other vap, delete station\n", __func__);
				  ...
				  ni->ni_subtype = 2;
			IEEE80211_NOTE_MAC_MGMT_DEBUG(vap, wh->i_addr2, 
				  	" <INFO> [Step 02 - RECV AUTH] %s: station send auth between different vap(0x%02X%04X), report delete station\n", 
	...
	IEEE80211_NOTE_MAC_MGMT_DEBUG(vap, wh->i_addr2, 
		" <INFO> [Step 02 - RECV AUTH] %s: ALGORITHM: 0x0000-OPEN, 0x0001-SHARED, 0x0080-LEAP\n", 
		__func__);
	IEEE80211_NOTE_MAC_MGMT_DEBUG(vap, wh->i_addr2, 
		 " <INFO> [Step 02 - RECV AUTH] %s: ALGORITHM - OPEN, SEQ_NUM: 1-REQUEST, 2-RESPONSE\n", 
		 __func__);		   	
	IEEE80211_NOTE_MAC_MGMT_DEBUG(vap, wh->i_addr2, 
		 " <INFO> [Step 02 - RECV AUTH] %s: ALGORITHM - SHARED, SEQ_NUM: 1-REQUEST, 2-CHALLENGE, 3-RESPONSE, 4-PASS\n", 
		 __func__);		   
...
ieee80211_recv_deauth(struct ieee80211_node *ni, wbuf_t wbuf, int subtype)
{
	...
	/*zhaoyang1 transplant from 717*/
	/*pengruofeng--add begin for wids 2011-5-30*/
	if ((vap->iv_opmode == IEEE80211_M_HOSTAP) && (vap->spoof_detect))
	{
		if (IEEE80211_IS_MULTICAST(wh->i_addr1) || IEEE80211_IS_MULTICAST(wh->i_addr2) || IEEE80211_IS_MULTICAST(wh->i_addr3))
		{
			match = next = NULL;
			...
			LIST_FOREACH(next, &vap->iv_sta.attacker, attacker) {
				if (!memcmp(next->mac, wh->i_addr2, IEEE80211_ADDR_LEN)) {
					match = next;
					match->rssi=0;//pengruofeng modify rssi for 0
					break;
				}
			}
    ...
    if ((vap->iv_opmode == IEEE80211_M_STA) && 
        ...
        IEEE80211_DISCARD(vap, IEEE80211_MSG_INPUT,
                          ...
                          "%s: %s", __func__, "deauth frame is not encrypted");
	...
	/*zhaoyang1 transplant from 717*/
	/*pengruofeng--add for wids 2011-5-30*/ 
	if (vap->spoof_detect && ni != vap->iv_bss && 
   		(vap->iv_opmode == IEEE80211_M_HOSTAP ||
   		 vap->iv_opmode == IEEE80211_M_BTAMP)) {
	   if ((ni->ni_rxseqs[0] >> IEEE80211_SEQ_SEQ_SHIFT) != ((ni->ni_rxseqs_last[0] >> IEEE80211_SEQ_SEQ_SHIFT) + 1))
	   {
		   match = next = NULL;
		   WIDS_LOCK_IRQ(&(vap->iv_sta));
...
ieee80211_recv_disassoc(struct ieee80211_node *ni, wbuf_t wbuf, int subtype)
{
	...
	/*zhaoyang1 transplant from 717*/
	/*pengruofeng--add for wids 2010-5-30*/
	if ((vap->iv_opmode == IEEE80211_M_HOSTAP) && (vap->spoof_detect))
	{
		if (IEEE80211_IS_MULTICAST(wh->i_addr1) || IEEE80211_IS_MULTICAST(wh->i_addr2) || IEEE80211_IS_MULTICAST(wh->i_addr3))
		{
			...
			//printk("%s: (m)disassoc spoofing.\n", __func__);
			return EOK;
		}
	}
	/*pengruofeng--add end 2010-5-32*/
	/*zhaoyang1 transplant end*/
    ...
    if ((vap->iv_opmode == IEEE80211_M_STA) && 
        ...
        IEEE80211_DISCARD(vap, IEEE80211_MSG_INPUT,
                          wh, ieee80211_mgt_subtype_name[
                              subtype >> IEEE80211_FC0_SUBTYPE_SHIFT],
                          "%s: %s", __func__, "disassoc frame is not encrypted");
...
ieee80211_recv_action(struct ieee80211_node *ni, wbuf_t wbuf, int subtype, struct ieee80211_rx_status *rs)
{
    ...
    if (!(IEEE80211_ADDR_EQ(wh->i_addr3, ni->ni_bssid)) &&
        ...
        IEEE80211_DISCARD(vap, IEEE80211_MSG_INPUT,
                          ...
                          "%s: %s", __func__, "action frame not in same BSS");
        return -EINVAL;
    }
    ...
    case IEEE80211_ACTION_CAT_HT: {
		...
		case IEEE80211_ACTION_HT_CSI:
            ...
            frm += sizeof(struct ieee80211_action_ht_CSI);
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_ANY,
                "%s: **%s CSI_Report_Len = %d **\n", __func__, 
                __FUNCTION__, CSI_Report_Len);
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_ANY,"%s: MIMO_Control 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x \n",
			__func__, 
...
#define SCANER_NETLINK_PID   (0x103)
#define DETECTOR_NETLINK_PID (0x101)
#define NLMSGHDR_LEN         (sizeof(struct nlmsghdr))
#define ATNL_HDR_LEN         (sizeof(struct atnetlink_header))
#define ATTR_HDR_LEN         (sizeof(struct attribute_header))
#define NLSNED_LEN           (sizeof(struct rrm_scan_sta_result))
...
//have include the size of nlmsghdr
...
void ieee80211_scan_locate_nlsend(struct ieee80211_node *ni, wbuf_t wbuf, int subtype, struct ieee80211_rx_status *rs)
{
	wbuf_t nlwbuf;
    void *nlh;
	u_int16_t msg_alloc_size;
    int type = -1,subtype_tmp = 0, dir = 0;
	struct atnetlink_header *atnl_hdr;
	struct attribute_header *attr_hdr;
	struct	rrm_scan_sta_result *scan;
	...
	if(NULL == ni || NULL == wbuf || NULL == rs){
		printk("%s,param is NULL",__func__);
		return;
	}
	
	type = wh->i_fc[0] & IEEE80211_FC0_TYPE_MASK;
	subtype_tmp = wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK;
    dir = wh->i_fc[1] & IEEE80211_FC1_DIR_MASK;


    if((type == IEEE80211_FC0_TYPE_MGT)&&((subtype_tmp == IEEE80211_FC0_SUBTYPE_ASSOC_REQ)||\
		(subtype_tmp == IEEE80211_FC0_SUBTYPE_PROBE_REQ))) {
		;
	}else if ((type == IEEE80211_FC0_TYPE_DATA) &&(dir == IEEE80211_FC1_DIR_TODS)) {
		;
	}
	else {
		return ;
	...
	msg_alloc_size =  OS_NLMSG_SPACE(ATNL_HDR_LEN + ATTR_HDR_LEN+ NLSNED_LEN);
	nlwbuf = wbuf_alloc(NULL,WBUF_MAX_TYPE,msg_alloc_size);
	if (nlwbuf ==NULL)    {
		  printk("netlink wbuf_alloc failed! \n");
	      return;
    }

	wbuf_append(nlwbuf,msg_alloc_size); //skb_put
	nlh = wbuf_raw_data(nlwbuf);

	OS_SET_NETLINK_HEADER_LOCATE(nlh, NLMSG_LENGTH(ATNL_HDR_LEN + ATTR_HDR_LEN+ NLSNED_LEN),
                     0,0,0,SCANER_NETLINK_PID) ;
    wbuf_set_netlink_pid(nlwbuf,DETECTOR_NETLINK_PID);
    wbuf_set_netlink_dst_group(nlwbuf,0);

	atnl_hdr = (struct atnetlink_header *)OS_NLMSG_DATA(nlh);
	attr_hdr = (struct attribute_header *)((char *)atnl_hdr + ATNL_HDR_LEN);

	scan     = (struct rrm_scan_sta_result *)((char *)attr_hdr + ATTR_HDR_LEN);

    memset(atnl_hdr,0x0,ATNL_HDR_LEN);
	atnl_hdr->attr_count = 1;
	attr_hdr->funct_type = 0;
	attr_hdr->funct_length = sizeof(attr_hdr->func_type);
	attr_hdr->func_type = 2;
	attr_hdr->funcsubt_type = 1;
	attr_hdr->funcsubt_length =  sizeof(attr_hdr->func_subtype);
	attr_hdr->func_subtype = 2;
	attr_hdr->version_type = 2;
	attr_hdr->version_length = sizeof(attr_hdr->version);
	attr_hdr->version  = 0;
	attr_hdr->msglen_type =  3;
	attr_hdr->mgslen_length = 2;
	attr_hdr->mgslen = NLSNED_LEN ;

	//time
    do_gettimeofday(&timesend);
    scan->time_type = 0;
    scan->time_length = sizeof(scan->time_s) + sizeof(scan->time_us);
    scan->time_s = timesend.tv_sec;
	scan->time_us = timesend.tv_usec;
	
	//mac
	scan->src_mac_type = 1;
	scan->src_mac_length = IEEE80211_ADDR_LEN;
	if(dir == IEEE80211_FC1_DIR_DSTODS){
		...
		IEEE80211_ADDR_COPY(scan->src_mac, ((struct ieee80211_frame_addr4*)wbuf_header(wbuf))->i_addr4);	
	}
	else {
		IEEE80211_ADDR_COPY(scan->src_mac,wh->i_addr2);
	}

	//rssi
	scan->rssi_type = 2;
	scan->rssi_length = sizeof(scan->rssi);
	//scan->rssi = ni->ni_rssi;
	scan->rssi = (u_int8_t)rs->rs_rssi; /* gengzongjie modified ni->ni_rss to rs->rs_rssi */
	

	// sta channel
	scan->sta_chan_type = 3;
	scan->sta_chan_length = sizeof(scan->sta_chan);
	scan->sta_chan = curchan->ic_ieee;

	//radio id
	scan->radioid_type = 4;
	scan->radioid_length = sizeof(scan->radioid);
	scan->radioid = vap->iv_radioid;

	if(vap->iv_sl_debug){
		printk("wh->i_fc[0] = 0x%02x,wh->i_fc[1] = 0x%02x,wh->i_dur[0] = 0x%02x,wh->i_dur[1] = 0x%02x \n",wh->i_fc[0],wh->i_fc[1],wh->i_dur[0],wh->i_dur[1]);
		printk("wh:%02x:%02x:%02x:%02x:%02x:%02x\n",wh->i_addr2[0],wh->i_addr2[1],wh->i_addr2[2],wh->i_addr2[3],wh->i_addr2[4],wh->i_addr2[5]);
		printk("scan:%02x:%02x:%02x:%02x:%02x:%02x\n",scan->src_mac[0],scan->src_mac[1],scan->src_mac[2],scan->src_mac[3],scan->src_mac[4],scan->src_mac[5]);
		printk("driver: radio=%d, type= %d ,subtype = %d, channel = %d ,rssi = %d \n",scan->radioid,type,subtype_tmp,scan->sta_chan,scan->rssi);
	}
	OS_NETLINK_UCAST(iv_scan_notlocate.sl_sock, nlwbuf,DETECTOR_NETLINK_PID, 1);
                    ...
                    struct ieee80211_rx_status *rs)
{
    ...
    struct ieee80211_frame *wh;
	/*zhaoyang1 transplant from 717*/
	struct wids_detect_attacker_entry *match, *next; /*pengruofeng add for wids 2011-5-31*/
	/*zhaoyang1 transplant end*/
    int    is_bcast;
	int retval = 0;//Add by linrongqi for recv_mgmt before netif_rx 2012-11-7
    wh = (struct ieee80211_frame *) wbuf_header(wbuf);

	/*zhaoyang1 transplant from 717*/
	/*pengruofeng add for wids 2011-5-31*/
	match = next = NULL;
    ...
    if (IEEE80211_IS_MFP_FRAME(wh)) {
       ...
       IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME, "%s: Received MFP frame with Subtype 0x%2X\n", __func__, subtype);
       if (NULL == ieee80211_crypto_decap(ni, wbuf, 
                    ieee80211_hdrspace(ic, wbuf_header(wbuf)), rs)) {
            mac_stats->ims_rx_decryptcrc++;
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME, "%s: Decrypt Error MFP frame with Subtype 0x%2X\n", __func__, subtype);
            return -1;
        } else {
    ...
    case IEEE80211_FC0_SUBTYPE_BEACON:
        retval = ieee80211_recv_beacon(ni, wbuf, subtype, rs);//Add by linrongqi for recv_mgmt before netif_rx 2012-11-7
        break;

    case IEEE80211_FC0_SUBTYPE_PROBE_REQ:
        retval = ieee80211_recv_probereq(ni, wbuf, subtype);//Add by linrongqi for recv_mgmt before netif_rx 2012-11-7
        break;

    case IEEE80211_FC0_SUBTYPE_AUTH:
		/*zhaoyang1 transplant from 717*/
		/*pengruofeng add start for management frame stats 2011-5-9*/
		vap->iv_stats.is_rx_auth_try++;
		/*pengruofeng add end 2011-5-9*/
		/*zhaoyang1 transplant end*/
        IEEE80211_NOTE_MAC_MGMT_DEBUG(vap, wh->i_addr2, " <RECV> [Step 02 - RECV AUTH] %s\n", __func__);
        retval = ieee80211_recv_auth(ni, wbuf, subtype, rs );//Add by linrongqi for recv_mgmt before netif_rx 2012-11-7
        break;

    case IEEE80211_FC0_SUBTYPE_ASSOC_RESP:
    case IEEE80211_FC0_SUBTYPE_REASSOC_RESP:
	  if (subtype == 0x10)
	  	IEEE80211_NOTE_MGMT_DEBUG(vap, ni, " <RECV> [Step 03 - RECV ASSOC RESP] %s\n", __func__);
	  else
	  	IEEE80211_NOTE_MGMT_DEBUG(vap, ni, " <RECV> [Step 04 - RECV REASSOC RESP] %s\n", __func__);		
        retval = ieee80211_recv_asresp(ni, wbuf, subtype);//Add by linrongqi for recv_mgmt before netif_rx 2012-11-7
        break;

    case IEEE80211_FC0_SUBTYPE_ASSOC_REQ:
		/*zhaoyang1 transplant from 717*/
		/*pengruofeng add start for management frame stats 2011-5-9*/
		vap->iv_stats.is_rx_assoc_try++;
		/*pengruofeng add end 2011-5-9*/
		/*zhaoyang1 transplant end*/
    case IEEE80211_FC0_SUBTYPE_REASSOC_REQ:
	  if (subtype == 0x00)
	  	IEEE80211_NOTE_MGMT_DEBUG(vap, ni, " <RECV> [Step 03 - RECV ASSOC REQ] %s\n", __func__);
	  else
	  	IEEE80211_NOTE_MGMT_DEBUG(vap, ni, " <RECV> [Step 04 - RECV REASSOC REQ] %s\n", __func__);		
        retval = ieee80211_recv_asreq(ni, wbuf, subtype);//Add by linrongqi for recv_mgmt before netif_rx 2012-11-7
        break;
        
    case IEEE80211_FC0_SUBTYPE_DEAUTH:
	 IEEE80211_NOTE_MGMT_DEBUG(vap, ni, " <RECV> [Step 05 - RECV DEAUTH] %s\n", __func__);
        retval = ieee80211_recv_deauth(ni, wbuf, subtype);//Add by linrongqi for recv_mgmt before netif_rx 2012-11-7
        break;

    case IEEE80211_FC0_SUBTYPE_DISASSOC:
	 IEEE80211_NOTE_MGMT_DEBUG(vap, ni, " <RECV> [Step 06 - RECV DISASSOC] %s\n", __func__);
        retval = ieee80211_recv_disassoc(ni, wbuf, subtype);//Add by linrongqi for recv_mgmt before netif_rx 2012-11-7
        break;
        
    case IEEE80211_FC0_SUBTYPE_ACTION:
    case IEEE80211_FCO_SUBTYPE_ACTION_NO_ACK:
        retval = ieee80211_recv_action(ni, wbuf, subtype, rs);//Add by linrongqi for recv_mgmt before netif_rx 2012-11-7
        break;
    ...
    {	
        ...
        vap->iv_stats.is_rx_mgmtbytes +=wbuf->len;
//		printk("%s vap->iv_stats.is_rx_mgmt :%d vap->iv_stats.is_rx_mgmtbytes : %d\n",__func__,vap->iv_stats.is_rx_mgmt++,vap->iv_stats.is_rx_mgmtbytes);
		/*zhaoyang modify for tunnel count CHINAMOBILE-271*/
		IEEE80211_NODE_STAT(ni, rx_mgmt);
		IEEE80211_NODE_STAT_ADD(ni, rx_mgmt, wbuf->len);
		/*zhaoyang modify end*/
     }
	/*pengruofeng add end 2011-5-9*/
	/*zhaoyang1 transplant end*/

    return retval;//Add by linrongqi for recv_mgmt before netif_rx 2012-11-7
}
