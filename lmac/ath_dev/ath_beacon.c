...
extern u_int8_t rate_index[3][IEEE80211_RATE_XRATE_SIZE + 1];    //added by zhanghu

/* Begin:gengzongjie transplant for the bug of unable to send packets 2013-5-9 */
extern int (*kes_debug_print_handle)(const char *fmt, ...);//added by zhaoenjuan
...
static u_int8_t ath_rate_findrix(const HAL_RATE_TABLE *rt , u_int8_t ieee_rate)
{
    u_int8_t i, rix = 0;
...
ath_beacon_setup(struct ath_softc *sc, struct ath_vap *avp, struct ath_buf *bf)
{
    ...
    struct ath_hal *ah = sc->sc_ah;    
    struct ieee80211vap *vap = (struct ieee80211vap *)avp->av_if_data; //added by zhanghu
    struct ieee80211com *ic = vap->iv_ic;  //added by zhanghu
    struct ath_desc *ds;
    int flags;
    ...
    struct ieee80211vap *iv = (struct ieee80211vap *)avp->av_if_data;
    /*
    ** Code Begins
    */
    ...
    rix = sc->sc_minrateix;
    rt = sc->sc_currates;
#if 0 //Added by duanmingzhe for divided Data rate & Mgmt rate contrl 
    /*Start: Added by zhanghu for increasing rate*/
#ifndef  FOR_BIT
#define FOR_BIT(parm)    (sizeof(parm)*8)
#endif

    if(ic->ic_rate_mask & ((u_int64_t)1 << (FOR_BIT(ic->ic_rate_mask) - 1))){
        u_int8_t i = 0;
        u_int8_t max = 0;
        u_int8_t index = 0;        
        ...
        if(i < max && (rt->info[rix].dot11Rate & IEEE80211_RATE_VAL) < rate_index[index][i]){
            rix = ath_rate_findrix(rt, rate_index[index][i]);
        }        
    ...
    if(vap->iv_manager_rate & 
        (IEEE80211_MANAGER_RATE_S_ALL | IEEE80211_MANAGER_RATE_S_BEA) ){
        ...
        if(IEEE80211_RATE_XRATE_SIZE == i){
            ...
            rix = ath_rate_findrix(rt, rate_index[2][i]);
        }        
    }    
    ...
    if(iv->iv_first_beacon )
    {
        series[0].TxPowerCap = 40;
        iv->iv_first_beacon = 0;
    }
    else
    {
...
ath_beacon_generate(struct ath_softc *sc, int if_id, int *needmark)
{
    ...
    struct ieee80211vap *vap = NULL;	/*Begin:Added by duanmingzhe for auto-transmit*/
	...
	/*zhaoyang1 transplant from 717*/
	/*Begin:Added by duanmingzhe for stop send beacon when auto-transmit enable*/
    vap = (struct ieee80211vap *)avp->av_if_data;
    KASSERT(vap != NULL,("vap is null"));	/*lyb add for value protect*/
    if (vap->iv_transmit_switch)
	return NULL;
    /*End:Added by duanmingzhe for stop send beacon when auto-transmit enable*/
	/*zhaoyang1 transplant end*/
...
#endif
 //AUTELAN-zhaoenjuan transplant for drv monitor stuck check  
int bb_hang = 0;
int mac_hang = 0;
/*
...
ath_beacon_tasklet(struct ath_softc *sc, int *needmark)
{
    ...
    if (bfaddr != 0)
    {
        ...
        sc->sc_stats.ast_tx_mgmt += bc;

		sc->sc_stats.ast_be_xmit_bytes += wbuf_get_pktlen(bf->bf_mpdu);
...
**  \return N/A
*/
 /*AUTELAN-Begin:zhaoenjuan transplant for drv monitor stuck check */ 
extern int bb_hang;
extern int mac_hang;
...
ath_bstuck_tasklet(struct ath_softc *sc)
{
    ...
    struct ieee80211vap *tempIv;  
    bstuck_check_num++;   
#ifdef ATH_SUPPORT_DFS
    sc->sc_dfs_hang.total_stuck++;
#endif
    if (!ATH_HANG_DETECTED(sc)) {
        TAILQ_FOREACH(tempIv, &(ic)->ic_vaps, iv_next)
            tempIv->iv_first_beacon = 1;
        //printk("atheros--%s: stuck beacon by QCU; resetting (bmiss count %u),bb_hang=%d,mac_hang=%d,bstuck_check_num=%u,beacon_complete=%u\n",
    ...
    else
    {
        TAILQ_FOREACH(tempIv, &(ic)->ic_vaps, iv_next)
            tempIv->iv_first_beacon = 1;
        //printk("atheros--%s: stuck beacon by MAC detect; resetting (bmiss count %u, reason 0x%x),bb_hang=%d,mac_hang=%d,bstuck_check_num=%u,beacon_complete=%u\n",
    ...
    if(bstuck_enable)
    {
    	...
    	/*zhaoyang1 transplant from 717*/
    	/*xiaruixin modify for beacon miss rest */
 ...
 /*AUTELAN-End:zhaoenjuan transplant for drv monitor stuck check */ 
...
static void ath_beacon_config_tsf_offset(struct ath_softc *sc, int if_id )
{
    ...
    if (sc->sc_stagbeacons)
    {
        ...
        /*lijiyong modify begin to resolve user reauth frequently in powersave mode*/
