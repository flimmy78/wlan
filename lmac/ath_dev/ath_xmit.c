...
/*added by zhanghu*/
u_int8_t rate_index[3][IEEE80211_RATE_XRATE_SIZE + 1] = {
            {12,18,24,36,48,72,96,108,0,0,0,0,8},
            {2,4,11,22,0,0,0,0,0,0,0,0,4},
            {2,4,11,22,12,18,24,36,48,72,96,108,12}
    };
/*added by zhanghu*/
#ifndef ARRAY_LEN
...
static u_int8_t ath_rate_findrix(const HAL_RATE_TABLE *rt , u_int8_t ieee_rate); //added by zhanghu
...
#if 1 
void
ath_dump_txquedepth(struct ath_softc * sc);
#endif
/*zhaoyang modify end*/
...
/* Begin:gengzongjie added for sending arp packets to assoc sta in wifipos function 2014-1-2 */
int IPADDR_PRASE(wbuf_t wbuf, struct ieee80211_node *ni)
{
    struct icmphdr *icmpheader = NULL;
    struct iphdr * iphdr = NULL;
    struct llc *llc = NULL;
    u_int8_t frm_type = 0;
    u_int8_t frm_subtype1 = 0;
    u_int8_t icmptype = 0;
    u_int16_t seq = 0;
    u_int32_t i = 0;
    u_int16_t pkt_len = wbuf->len;

    struct ieee80211_frame * wh = (struct ieee80211_frame *)wbuf_header(wbuf);
    if(NULL != wh)
    {
        if(pkt_len > (roundup(sizeof(struct ieee80211_frame), 4) + sizeof(struct llc) + sizeof(struct iphdr) + sizeof(struct icmphdr)))  
        {
            frm_type = wh->i_fc[0] & IEEE80211_FC0_TYPE_MASK;    
            frm_subtype1 = wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK;
            if( frm_type  & IEEE80211_FC0_TYPE_DATA)
            {
                if(frm_subtype1 & IEEE80211_FC0_SUBTYPE_QOS)
                {
                    llc = (struct llc *)((u_int8_t *)wh + roundup(sizeof(struct ieee80211_qosframe), 4));
                    iphdr = (struct iphdr *)((u_int8_t *)wh + roundup(sizeof(struct ieee80211_qosframe), 4) + sizeof(struct llc));
                }
                else
                {
                    llc = (struct llc *)((u_int8_t *)wh + roundup(sizeof(struct ieee80211_frame), 4));
                    iphdr = (struct iphdr *)((u_int8_t *)wh + roundup(sizeof(struct ieee80211_frame), 4) + sizeof(struct llc));
                }
                if( llc->llc_snap.ether_type == htons(ETHERTYPE_IP) )
                {
                    if(iphdr->protocol == 1)
                    {
                        icmpheader = (struct icmphdr *)((u_int8_t *)iphdr + sizeof(struct iphdr)); 
                        icmptype = icmpheader->type;
                        seq = icmpheader->un.echo.sequence;
                    }
                    ...
                    {   
                        ...
                        {   
                            ...
                            if(iphdr->daddr == 0)
                            {
                                printk("WIFIPOS: Fatal error iphdr=0x%x,ni=0x%x\n",iphdr,ni);
                            }
                            else
                            {
                                ni->saddr = iphdr->daddr;
                                ni->daddr = iphdr->saddr;
                                memcpy(ni->ni_srcmac, wh->i_addr3, IEEE80211_ADDR_LEN);
								ni->addr_flag ++;
                            }
                        }
                    }
...
static u_int8_t 
ath_control_Mframe_rate_enable(struct ieee80211vap *vap , struct ieee80211_frame *wh)
{
    u_int8_t enable = 0;
    u_int8_t frm_type = wh->i_fc[0] & IEEE80211_FC0_TYPE_MASK;
    u_int8_t frm_subtype = wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK;
    
    if(frm_type != IEEE80211_FC0_TYPE_MGT){
        return 0;
    }
        
    if(vap->iv_manager_rate & IEEE80211_MANAGER_RATE_S_ALL){
        return 1;
    }
    
    switch(frm_subtype){
        case IEEE80211_FC0_SUBTYPE_PROBE_REQ:
            if(vap->iv_manager_rate & IEEE80211_MANAGER_RATE_S_PREQ){
                enable = 1;
            }
        break;

        case IEEE80211_FC0_SUBTYPE_PROBE_RESP:
             if(vap->iv_manager_rate & IEEE80211_MANAGER_RATE_S_PRES){
                enable = 1;
            }
        break;
        
        case IEEE80211_FC0_SUBTYPE_AUTH:
             if(vap->iv_manager_rate & IEEE80211_MANAGER_RATE_S_AUTH){
                enable = 1;
            }
        break;
        ...
        case IEEE80211_FC0_SUBTYPE_REASSOC_REQ:
            if(vap->iv_manager_rate & IEEE80211_MANAGER_RATE_S_AREQ){
                enable = 1;
            }
        break;
        ...
        case IEEE80211_FC0_SUBTYPE_REASSOC_RESP:
            if(vap->iv_manager_rate & IEEE80211_MANAGER_RATE_S_ARES){
                enable = 1;
            }
        break;

        case IEEE80211_FC0_SUBTYPE_DISASSOC:
            if(vap->iv_manager_rate & IEEE80211_MANAGER_RATE_S_DISASS){
                enable = 1;
            }
        break;

        case IEEE80211_FC0_SUBTYPE_DEAUTH:
            if(vap->iv_manager_rate & IEEE80211_MANAGER_RATE_S_DEAHTH){
                enable = 1;
            }
        break;
...
#define    N(a)    (sizeof(a)/sizeof(a[0]))
    ...
    if (!ATH_TXQ_SETUP(sc, qnum)) {
		...
		/* Begin:gengzongjie transplant for the bug of unable to send packets 2013-5-9 */
		//zhaoenjuan print axq_totalqueued		 
        printk("%s :the axq_totalqueued is zero(%d)\n",__func__,txq->axq_totalqueued);
        if(kes_debug_print_handle)
        {	 	 
          kes_debug_print_handle(KERN_EMERG "%s :the axq_totalqueued is zero(%d)\n",__func__,txq->axq_totalqueued);
        }
...
#define DESC2PA(_sc, _va)	\
    ...
    if (sc->sc_enhanceddmasupport) {
		...
		/*pengruofeng add start for frame stats 2011-5-9*/
...
__ath_tx_prepare(struct ath_softc *sc, wbuf_t wbuf, ieee80211_tx_control_t *txctl)
{
    ...
    int subtype;
	/*zhaoyang1 transplant from 717*/
    struct ieee80211_node *ni = NULL;	/*Begin:Added by duanmingzhe for auto-transmit*/
	/*zhaoyang1 transplant end*/
	struct ieee80211com *ic = NULL;     /*Added by zhanghu*/
    txctl->dev = sc;
	...
	/*zhaoyang1 transplant from 717*/
    ni = (struct ieee80211_node *)(an->an_node);	/*Begin:Added by duanmingzhe for auto-transmit*/
	/*zhaoyang1 transplant end*/
	ic = ni->ni_ic;                                 /*Added by zhanghu*/
    /*
    ...
    ni->ni_tx_cur_dot11Rate = 0;
    ni->ni_tx_cur_rateCode = 0;
    /*End: Add by zhanghu to statistics of every frame by the rate*/


	/*zhaoyang1 transplant from 717*/
	/*AUTELAN-Added-Begin:Modified by duanmingzhe for Null-DATA frame to MGMT tid issue*/
    subtype = wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK;
    //if (txctl->isdata && subtype != IEEE80211_FC0_SUBTYPE_NODATA) {
    if (txctl->isdata && !txctl->use_minrate && subtype != IEEE80211_FC0_SUBTYPE_NODATA) {
	/*AUTELAN-Added-End:Modified by duanmingzhe for Null-DATA frame to MGMT tid issue*/
        if (txctl->ismcast && ni->ni_vap->iv_transmit_switch == 0) { //Added by duanmingzhe for auto-transmit
    /*zhaoyang1 transplant end*/
...
/*Autelan-Added:duanmingzhe for patch tx-stuck patchs from case 01146407*/
                if (
#ifdef ATH_RIFS
                txctl->ht && (sc->sc_txaggr || sc->sc_txrifs)
#else
...
#define FOR_BIT(parm)    (sizeof(parm)*8)
#endif
        if((ic->ic_rate_mask & ((u_int64_t)1 << (FOR_BIT(ic->ic_rate_mask) - 1))) && txctl->min_rate == 0){
            u_int8_t i = 0;
            u_int8_t max = 0;
            u_int8_t index = 0;
            u_int32_t   ic_rate_mask = 0;

            index = ((ic->ic_rate_mask >> (FOR_BIT(ic->ic_rate_mask) - 8)) & 0x07)/2;         
            max = rate_index[index][IEEE80211_RATE_XRATE_SIZE];
            while(i < 2*max) ic_rate_mask |= 1 << (i++);
            ic_rate_mask &= (u_int32_t)(ic->ic_rate_mask >> (index*16));
            i = 0;
            while( i < max && !((ic_rate_mask & (1 << i)) && (ic_rate_mask & (1 << (i+max)))) ){
                i++;
            }
            if(max == i){
                printk(KERN_DEBUG "%s:%u--can't find the vaild rate\n", __func__, __LINE__);
            }
            if(i < max && \
                (rt->info[rcs[0].rix].dot11Rate & IEEE80211_RATE_VAL) < rate_index[index][i]){
                rcs[0].rix = ath_rate_findrix(rt, rate_index[index][i]);
            }       
        }            
    ...
    /*Start: Add by zhanghu to statistics of every frame by the rate*/
    ni->ni_tx_cur_dot11Rate = rt->info[rcs[0].rix].dot11Rate & ~0x80;
    ni->ni_tx_cur_rateCode = rt->info[rcs[0].rix].rateCode;
    /*End: Add by zhanghu to statistics of every frame by the rate*/

    /*Start: added by zhanghu to control the manager frame*/
    {
        struct ath_vap *avp = sc->sc_vaps[txctl->if_id];
        struct ieee80211vap *vap = avp->av_if_data;
        
        u_int8_t enable = ath_control_Mframe_rate_enable(vap, wh);
        
        if(enable){
            u_int16_t    index = vap->iv_manager_rate & 0x0FFF;
            u_int8_t    i = 0;
            
            while( !((index >> i) & 0x01) && i < IEEE80211_RATE_XRATE_SIZE){
                i++;
            }
            
            if(IEEE80211_RATE_XRATE_SIZE == i){
                printk("%s: %d more than IEEE80211_RATE_XRATE_SIZE\n", __func__, i);
            }else{
                rcs[0].rix = ath_rate_findrix(rt, rate_index[2][i]);
            }
        }
    }
	...
	/*AUTELAN-Added-Begin:Deleted by duanmingzhe for Null-DATA frame to MGMT tid issue*/
    //subtype = wh->i_fc[0] & IEEE80211_FC0_SUBTYPE_MASK;
	/*AUTELAN-Added-End:Deleted by duanmingzhe for Null-DATA frame to MGMT tid issue*/
#if ATH_SUPPORT_LED
    if (txctl->isdata &&
...
ath_tx_start_dma(wbuf_t wbuf, sg_t *sg, u_int32_t n_sg, void *arg)
{
    ...
    if (txctl->ismcast) {
        ...
        /*Start: added by zhanghu for killing the bug that multicast frame is  disorder */
        else if(cabq->axq_depth && (vap->iv_mcast_debug & IEEE80211_MCAST_DEBUG_DISORDER) ){
    ...
    if (firstbf) {
		...
		/* Begin:gengzongjie transplant for the bug of unable to send packets 2013-7-2 */
		 sc->sc_stats.ast_txq_noaggr_packets[txq->axq_qnum]++;//AUTELAN-zhaoenjuan transplant for drv monitor stats
		/* End:gengzongjie transplant end */
        }
...
ath_tx_update_stats(struct ath_softc *sc, struct ath_buf *bf, u_int qnum, struct ath_tx_status *ts)
{
	...
	if(retrycount > 0){
		...
		if(NULL != vap){
			wbuf = bf->bf_mpdu;			
			if(wbuf->frametype_flag == 1)//data frame 
			{
			...
			else if(wbuf->frametype_flag == 2)//mgmt frame 
			{
				vap->iv_unicast_stats.ims_tx_retry_mgmt_packets += retrycount;
				vap->iv_unicast_stats.ims_tx_retry_mgmt_bytes += wbuf->len * retrycount;							
			}			
		}
	}
	/*zhaoyang modify end*/
 ...
 */
extern u_int32_t beacon_complete;/*AUTELAN-zhaoenjuan transplant for drv monitor beacon stuck check*/
void
...
/*AUTELAN-zhaoenjuan transplant for drv monitor beacon stuck check*/
...
/*packet power added by chenming */
    ...
    ni = (struct ieee80211_node *)an->an_node;
    iv = (struct ieee80211vap *)ni->ni_vap;
/*ended by chenming*/
    /*
	...
	/* gengzongjie added for the bug that ctsrate=0 when cix=0xff 2013-4-28 */
	if(cix == 0xff)
		cix = 0;
...
/*packet power added by chenming */
    if(iv->iv_packet_power_enable && bf->bf_isdata )
        txpower = bf->bf_reftxpower;  
/*ended by chenming*/
    for (i = 0; i < 4; i++) {
...
{   /* non training packet get values from node rate set*/
     ...
     */
    /*zhaoyang1 transplant from 717*/
    /*zhaoyang modify for ctrl frame count PCAPVXN-130*/
	if(flags & (HAL_TXDESC_RTSENA | HAL_TXDESC_CTSENA))
	{
		ni = (struct ieee80211_node *)an->an_node;//sheyanbin modify for stack dump
		if(NULL != ni){
			if(flags & HAL_TXDESC_RTSENA)
			{
				IEEE80211_NODE_STAT(ni, tx_ctrl);
				IEEE80211_NODE_STAT(ni, rx_ctrl);
					
			}else if(flags & HAL_TXDESC_CTSENA)
			{
				IEEE80211_NODE_STAT(ni, tx_ctrl);
				if(sc->sc_protmode == PROT_M_RTSCTS)
					IEEE80211_NODE_STAT(ni, rx_ctrl);
...
ath_dump_txquedepth(struct ath_softc *sc)
{
	...
	struct ath_atx_ac *ac;
	struct ath_atx_tid *tid;
	int axq_cnt,j,bufQCount,tidBufCount,pertidBufCount;
	struct ath_buf *buf;
	ATH_TXBUF_LOCK(sc);
	...
	for (i = 0; i < HAL_NUM_TX_QUEUES;i++) {
		if (ATH_TXQ_SETUP(sc, i)) {
			...
			TAILQ_FOREACH(ac, &txq->axq_acq, ac_qelem) {
				TAILQ_FOREACH(tid, &ac->tid_q, tid_qelem) {
					pertidBufCount = 0;
					printk("tid %p pause %d : sched_normal %d send_normal %d : ", tid, tid->paused,tid->seq_sched_normal,tid->seq_send_normal);
					...
					TAILQ_FOREACH(buf, &tid->buf_q, bf_list) {
						if(pertidBufCount ==0)
							{
							printk("\n ---------------------------------------");
							sc->sc_ieee_ops->dump_pkt((const u_int8_t *) buf->bf_vdata);
							printk("\n ---------------------------------------");
							}
						printk(" %d",buf->bf_seqno);
						bufQCount ++;
						pertidBufCount++;
						}
					printk("\n");
					...
					printk(" node macaddr %s  \n",ether_sprintf(sc->sc_ieee_ops->get_macaddr(tid->an->an_node)));
			...
			printk("\nsc_txq[%d] : depth  is %d \n aggr_depth is %d tidBufCount %d  bufQCount %d axq_cnt %d\n  ", txq->axq_qnum, txq->axq_depth, txq->axq_aggr_depth, tidBufCount, bufQCount, axq_cnt);
			//            ATH_TXQ_UNLOCK(txq);
			}
		}
	ATH_TXBUF_UNLOCK(sc);
...
EXPORT_SYMBOL(ath_dump_txquedepth);
#endif
/*zhaoyang modify end*/
