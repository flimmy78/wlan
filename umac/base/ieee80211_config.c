...
/*added by chenming -issue num PCAPVXN-78*/
#include "if_athvar.h"

extern void ath_get_maxtxpower(ath_dev_t dev, u_int32_t *txpow);
/*end-chenming*/
...
wlan_set_param(wlan_if_t vaphandle, ieee80211_param param, u_int32_t val)
{
	...
	case IEEE80211_TO_ENODE_MEDEBUG:
		if (vap->iv_me) {
			vap->iv_me->me_to_Enode_enable = val & 0x01;
		}
		break;
...
wlan_get_param(wlan_if_t vaphandle, ieee80211_param param)
{
    struct ieee80211vap *vap = vaphandle;
	/*zhaoyang1 transplant from 717*/
    struct ath_softc_net80211 *scn = ATH_SOFTC_NET80211(vap->iv_ic); /*added by chenming-issue num PCAPVXN-78*/
	/*zhaoyang1 transplant end*/
    struct ieee80211com *ic = vap->iv_ic;
	struct ieee80211_node_table *nt = &ic->ic_sta;
    u_int32_t val = 0;
...
/*zhaoyang1 transplant from 717*/
/*added by chenming-issue num PCAPVXN-78*/
    case IEEE80211_MAX_TXPOWER:   	
        ath_get_maxtxpower(scn->sc_dev, &val);
        break;
/*end-chenming*/
/*zhaoyang1 transplant end*/
    case IEEE80211_MULTI_DOMAIN:
	...
	case IEEE80211_TO_ENODE_MEDEBUG:
		if (vap->iv_me) {
			val = vap->iv_me->me_to_Enode_enable & 0x01;
		}
		break;
	/*End:Add by zhanghu for converting multiply frame to umcast frame for every node 2012-12-24*/
	/* gengzongjie transplant end */
