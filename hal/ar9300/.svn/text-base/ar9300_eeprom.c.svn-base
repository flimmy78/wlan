 ...
 */
    ...
    if(0x31 == ah->ah_devid)
    {
        OS_REG_WRITE(ah,0x16284,0x553f000); //added by duanmingzhe for xpabias
    }
...
/*zhaoyang1 transplant from 717*/
/*added by chenming to obtain max txpower from each mode-issue num PCAPVXN-78*/
     k = ALL_TARGET_LEGACY_6_24;         /* legacy */
    if (IS_CHAN_HT40(chan))
    {
        k = ALL_TARGET_HT40_0_8_16;     /* ht40 */
    }
    else if (IS_CHAN_HT20(chan))
    {
       k = ALL_TARGET_HT20_0_8_16;     /* ht20 */
    }
    AH_PRIVATE(ah)->ah_maxPower = targetPowerValT2[k];
    switch (ar9300_get_ntxchains(ahp->ah_txchainmask))
    {
        case 1:
            break;
        case 2:
            AH_PRIVATE(ah)->ah_maxPower += INCREASE_MAXPOW_BY_TWO_CHAIN;
            break;
        case 3:
            AH_PRIVATE(ah)->ah_maxPower += INCREASE_MAXPOW_BY_THREE_CHAIN;
            break;
        default:
            HALASSERT(0); /* Unsupported number of chains */
    }
...
/*zhaoyang1 transplant end*/
    ...
    /*zhaoyang1 transplant from 717*/
    twiceMaxRegulatoryPower = 60; //Added by duanmingzhe for fix a bug of channel 5180 txpower is 17dbm
    /*zhaoyang1 transplant end*/
    ...
    */
    ...
    AH_PRIVATE(ah)->ah_maxPowerLevel = targetPowerValT2[i];
    switch (ar9300_get_ntxchains(ahp->ah_txchainmask))
    {
        case 1:
            break;
...
ar9300EepromRestoreInternal(struct ath_hal *ah, ar9300_eeprom_t *mptr, int mdataSize)
{
	...
	if ((AH9300(ah)->CalibrationDataTry==CalibrationDataNone || AH9300(ah)->CalibrationDataTry==CalibrationDataNand) && AH9300(ah)->TryNand && nptr<0)
	{
		...
		AH9300(ah)->CalibrationDataSourceAddress=BaseAddress_Nand;
...
/*Begin:Transmitted by zhaoyang1 2013-01-22*/

/*
 * Begin, Added by wangjia, calibrate txpower by software, 2012-11-06.  
 * Used for getting V5 cost-down boards hardware version, 
 * to distinguish between indoors and outdoors boards.  
 *
...
#define AP_V5_UNKNOWN           1
#define AP_V5_INDOOR            5
#define AP_V5_OUTDOOR           6

unsigned char get_hw_version()
{
#if defined ( PC018 ) && !defined ( XH9344 ) && !defined ( XJ9344 )
#define HW_OFFSET 64
#define HW_BUF_LEN 13
    struct file *filp = filp_open("/dev/mtdblock7", O_RDONLY, 0);
#else
#define HW_OFFSET 64
#define HW_BUF_LEN 10
    struct file *filp = filp_open("/dev/mtdblock5", O_RDONLY, 0);
#endif

#define AP_V5_HW_VER_LEN 3

    int  retval = 0;
    char buff[HW_OFFSET + HW_BUF_LEN];
    
    char *ap_v5_indoor_hw_ver  = "5.0";
    char *ap_v5_outdoor_hw_ver = "6.0";

    memset(buff, 0, sizeof(buff));

    if(IS_ERR(filp)) 
    {
        printk("calibrate txpower, failed to open file.\r\n");  
        /**
         * If error occured when we determine the hw version, 
         * we just think it's an indoor AP, because it's txpower is lower.
         */
        return AP_V5_INDOOR;  
    }
    ...
    {  
        orgfs = get_fs();  
        set_fs(KERNEL_DS);
        retval = filp->f_op->read(filp, buff, HW_OFFSET + HW_BUF_LEN, &filp->f_pos); 
        set_fs(orgfs); 
        if (retval < 0)  
        {   
            filp_close(filp, NULL);
            printk("calibrate txpower, failed to read file.\r\n"); 
    ...
    if(memcmp(buff + HW_OFFSET, ap_v5_indoor_hw_ver, AP_V5_HW_VER_LEN) == 0)
    {
        return AP_V5_INDOOR;
    }
    else if(memcmp(buff + HW_OFFSET, ap_v5_outdoor_hw_ver, AP_V5_HW_VER_LEN) == 0)
    {
        return AP_V5_OUTDOOR;
    }
    
    return AP_V5_UNKNOWN;
}
/*End, Added by wangjia, calibrate txpower by software, 2012-11-06. */
/*End:Transmitted by zhaoyang1 2013-01-22*/
...
ar9300EepromRestore(struct ath_hal *ah)
{
	...
	/*Begin, Added by wangjia, calibrate txpower by software, 2012-11-06. */
	int is2GHz = 0;
	unsigned char ap_v5_hw_ver = AP_V5_UNKNOWN;

	/* Use this condition to determine whether its calibrate info is incorrect or not. */
	if(5500 == FBIN2FREQ(mptr->calFreqPier5G[1], is2GHz) && 
		5500 == FBIN2FREQ(mptr->calFreqPier5G[3], is2GHz)) {
		ap_v5_hw_ver = get_hw_version();
		printk("Applying txpower calibration, %s, ...", 
			(AP_V5_INDOOR == ap_v5_hw_ver) ? "indoor" : 
			(AP_V5_OUTDOOR == ap_v5_hw_ver) ? "outdoor" : "unknown");
		if(AP_V5_INDOOR == ap_v5_hw_ver) {
			// Chain 0
			mptr->calPierData5G[0][3].refPower = mptr->calPierData5G[0][1].refPower;   // 5500 <= 5500
			mptr->calPierData5G[0][4].refPower = mptr->calPierData5G[0][0].refPower+2; // 5620 <= 5185+2
			mptr->calPierData5G[0][5].refPower = mptr->calPierData5G[0][0].refPower-2; // 5700 <= 5185-2
			mptr->calPierData5G[0][6].refPower = mptr->calPierData5G[0][2].refPower+4; // 5745 <= 5785+4
			mptr->calPierData5G[0][7].refPower = mptr->calPierData5G[0][2].refPower-2; // 5825 <= 5785-2
			// Chain 1
			mptr->calPierData5G[1][3].refPower = mptr->calPierData5G[1][1].refPower;   // 5500 <= 5500
			mptr->calPierData5G[1][4].refPower = mptr->calPierData5G[1][0].refPower+2; // 5620 <= 5185+2
			mptr->calPierData5G[1][5].refPower = mptr->calPierData5G[1][0].refPower-4; // 5700 <= 5185-4
			mptr->calPierData5G[1][6].refPower = mptr->calPierData5G[1][2].refPower+4; // 5745 <= 5785+4
			mptr->calPierData5G[1][7].refPower = mptr->calPierData5G[1][2].refPower-2; // 5825 <= 5785-2
			// Chain 2, NO need to change.				   
		}
		else if(AP_V5_OUTDOOR == ap_v5_hw_ver) {
			// Chain 0
			mptr->calPierData5G[0][3].refPower = mptr->calPierData5G[0][1].refPower;   // 5500 <= 5500
			mptr->calPierData5G[0][4].refPower = mptr->calPierData5G[0][1].refPower+4; // 5620 <= 5500+4
			mptr->calPierData5G[0][5].refPower = mptr->calPierData5G[0][2].refPower+8; // 5700 <= 5785+8
			mptr->calPierData5G[0][6].refPower = mptr->calPierData5G[0][2].refPower+8; // 5745 <= 5785+8
			mptr->calPierData5G[0][7].refPower = mptr->calPierData5G[0][2].refPower+8; // 5825 <= 5785+8
			// Chain 1
			mptr->calPierData5G[1][3].refPower = mptr->calPierData5G[1][1].refPower;   // 5500 <= 5500
			mptr->calPierData5G[1][4].refPower = mptr->calPierData5G[1][1].refPower+2; // 5620 <= 5500+2
			mptr->calPierData5G[1][5].refPower = mptr->calPierData5G[1][2].refPower+6; // 5700 <= 5785+6
			mptr->calPierData5G[1][6].refPower = mptr->calPierData5G[1][2].refPower+6; // 5745 <= 5785+6
			mptr->calPierData5G[1][7].refPower = mptr->calPierData5G[1][2].refPower+6; // 5825 <= 5785+6
			// Chain 2, NO need to change.
		}
		printk("Done.\r\n");
	}
...
ar9300EepromSetBoardValues(struct ath_hal *ah, HAL_CHANNEL_INTERNAL *chan)
{
    ...
    /*Begin: Added by wangjia, for change noise floor. Refered to ar9300SetCcaThreshold. */
    if(ah->ah_noise_level > 0) {
        // Chain 0
        OS_REG_RMW_FIELD(ah, AR_PHY_CCA_0, AR_PHY_CCA_THRESH62, ah->ah_noise_level);
        
        // Chain 1
        OS_REG_RMW_FIELD(ah, AR_PHY_CCA_1, AR_PHY_CCA_THRESH62, ah->ah_noise_level);
		
#ifdef APM82181
		// Chain 2
        OS_REG_RMW_FIELD(ah, AR_PHY_CCA_2, AR_PHY_CCA_THRESH62, ah->ah_noise_level);
#endif 
    }
    /*End: Added by wangjia, for change noise floor. */
