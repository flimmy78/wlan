...
#endif
/*zhaoyang1 transplant from 717*/
/*ljy--add start for redefine the creating rule of vap mac*/
#define NEW_ATH_SET_VAP_BSSID_MASK(bssid_mask, dispatch_mac)				\
		do {								\
			if (dispatch_mac == 1) {		\
				(bssid_mask)[3] &= 0;		\
...
#define ATH_TXQ_MOVE_MCASTQ(_tqs,_tqd) do {                         \
	...
	/*printk("%s:ATH_TXQ_MOVE_MCASTQ the axq_totalqueued is %d\n",__func__,(_tqd)->axq_totalqueued);*/	\
	/*if(kes_debug_print_handle){*/	\
  	/*kes_debug_print_handle(KERN_EMERG "%s :the axq_totalqueued is %d\n",__func__,(_tqd)->axq_totalqueued);}*/	\
...
//End PAPRD vars
    ...
    os_timer_t              sc_drv_monitor;//AUTELAN:zhaoenjuan transplant for drv_monitor

	/* Begin:gengzongjie added for sending arp packets to assoc sta in wifipos function 2014-1-2 */
    os_timer_t              sc_wifipos_ping_send;
...
/* End: gengzj added end */
   /* Autelan-begin:zhaoenjuan transplant (peiwh) for drv_monitor wifi1 rx stuck check 2014-05-07 */
    u_int32_t rx_hp_cnt;
    u_int32_t rx_lp_cnt;
  ...
  /* Autelan-End:zhaoenjuan transplant (peiwh) for drv_monitor wifi1 rx stuck check 2014-05-07 */
};
...
u_int16_t ath_node_get_packet_power(struct ieee80211_node *ni,struct ath_softc *sc);
/*ended by chenming*/
/*
 * Receive interfaces
 */
...
/*zhaoyang1 transplant from 717*/
/*added by chenming to get maxtxpower-issue num PCAPVXN-78*/
#define ath_hal_getmaxtxpower(_ah, _ppow) \
...
#define ath_hal_txcalcretrycount(_ah, _ds, _ts) \
    ((*(_ah)->ah_calcTxRetryCount)((_ah), (_ds), (_ts)))  
/*zhaoyang modify end*/
/*zhaoyang1 transplant end*/
...
#define ath_hal_setCcaThreshold(_ah, _thresh62) \
    ((*(_ah)->ah_setCcaThreshold)((_ah), (_thresh62)))
#define ath_hal_getCcaThreshold(_ah) \
    ((*(_ah)->ah_getCcaThreshold)((_ah)))
/*lijiyong add end*/

/*lijiyong add begin for debug noise immunity level*/
#define ath_hal_setOfdmNoiseImmunityLevel(_ah, _level) \
    ((*(_ah)->ah_setOfdmNoiseImmunityLevel)((_ah), (_level)))
#define ath_hal_setCckNoiseImmunityLevel(_ah, _level) \
    ((*(_ah)->ah_setCckNoiseImmunityLevel)((_ah), (_level)))
/*lijiyong add end*/
