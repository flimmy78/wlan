...
#include "if_athvar.h" /* pengruofeng add for rx rate 2011-6-2*/
...
/*added by chenming for rate display*/
...
#define NETDEV_TO_VAP(_dev)  (((osif_dev *)(_dev)->priv)->os_if)
#else
#define NETDEV_TO_VAP(_dev) (((osif_dev *)netdev_priv(_dev))->os_if)
#endif 
/*xiaruixin modify end*/
#define NETDEV_TO_COM(_dev) (((osif_dev *)netdev_priv(_dev))->os_devhandle) //Added by duanmingzhe for get com from netdev
...
extern void get_vap_openessid_info(struct ieee80211vap *vap);
extern void init_whitelist(struct ieee80211vap *vap);

extern void ath_dump_txquedepth(struct ath_softc *sc);
static void autelan_queue_info(struct ieee80211com *ic)
{
	...
	TAILQ_FOREACH(ni, &nt->nt_node, ni_list)
	{
		...
		ATH_TXBUF_LOCK(sc);
		printk("\nnode[%u] %02x:%02x:%02x:%02x:%02x:%02x txbuf_free(%d) an=%p\n",nodeno,ni->ni_macaddr[0],ni->ni_macaddr[1],ni->ni_macaddr[2],ni->ni_macaddr[3],ni->ni_macaddr[4],ni->ni_macaddr[5],sc->sc_txbuf_free,an);

		//printk("tid_paused = %d ,tid_resumed = %d\n",sc->sc_stats.ast_11n_stats.tx_tidpaused,sc->sc_stats.ast_11n_stats.tx_tidresumed);
		printk("\n-------------------TID info------------------------\n");
		...
		for (tidno = 0; tidno < 17; tidno++)
		{
			...
			{  
				...
				if(tidbufcnt[tidno])
					printk("tid[%d]:addr = %p, tidbufcnt = %u, tid_pausd = %d, tid_sched = %u\n",tidno,tid,tidbufcnt[tidno],tid->paused,tid->sched);
			}else{
		...
		for (acno = 0; acno < WME_NUM_AC; acno++)
		{
			...
			if (ac!=NULL)
			{
				...
				if(acno!=0 && acno%2==0)
					printk("\n");
				printk("ac[%d]:addr = %p, ac_tidcnt = %u\t",acno,ac,ac_tidcnt[acno]);
			}else{
				printk("ac[%d] is NULL \t",acno);
			}
		}
		printk("\n\n");
		ATH_TXBUF_UNLOCK(sc);
...
static int clear_illegaldev_history(struct ieee80211vap *vap )
{
	struct wids_detect_attacker_entry *next;
	next = NULL;
	
	WIDS_LOCK_IRQ(&(vap->iv_sta));
	if (vap->iv_opmode == IEEE80211_M_HOSTAP && vap->flood_detect){
...
static int clear_attack_history(struct ieee80211vap *vap )
{
	struct wids_detect_attacker_entry *next;
	next = NULL;
	...
	if (vap->iv_opmode == IEEE80211_M_HOSTAP && vap->flood_detect){
		...
		(vap->iv_sta).attacker_list_node_cnt=0; //modified by zhaoyang1 for wids 2013-7-30
	}
	WIDS_UNLOCK_IRQ(&(vap->iv_sta));
...
static int clear_attack_stat_history(struct ieee80211vap *vap )
{
	if (vap->iv_opmode == IEEE80211_M_HOSTAP&&vap->flood_detect){
...
#endif    
/* Begin:gengzj added for suppressing illegal ap 2013-11-27 */
/*zhaoyang1 modify start for interfere ap*/
//static void
void
/*zhaoyang1 modify end*/
/* End:gengzj added end */
    ...
    struct iw_param *rrq, char *extra)
{
	...
	/*Begin: gengzongjie transplant from service provider 2013-5-7 */
	/*Begin: added by zhanghu for the bug of  TDDEVA-274 2013-05-06*/
    rrq->value = (rrq->value/100000) * 100000;
    /*End: added by zhanghu for the bug of  TDDEVA-274 2013-05-06*/
	/* End: gengzongjie transplant end */
    ...
    ieee80211_iw_getstats(struct net_device *dev)
{
    ...
    avgrssi = ieee80211_getrssi(vap->iv_ic,-1,IEEE80211_RSSI_RX);

    set_quality(&is->qual, avgrssi);
...
/*added by chenming for rate display*/
static int 
ieee80211_get_htrate(struct ieee80211com * ic, u_int32_t * rates, u_int32_t * nrates)
{
    int i;
    struct ath_softc_net80211   *scn = ATH_SOFTC_NET80211(ic);
    struct ath_softc *sc = scn->sc_dev;

    const HAL_RATE_TABLE    *currate = sc->sc_currates;
    if (currate->rateCount > IEEE80211_RATE_MAXSIZE)
        return -EINVAL;
    for (i = 0; i < currate->rateCount; i++) 
    {
        if(currate->info[i].rateCode & 0x80 )
        {
            if (IS_CHAN_HT40(sc->sc_curchan)) 
            {
                if (ieee80211com_has_htflags(ic, IEEE80211_HTF_SHORTGI40))
                    rates[i] = (currate->info[i].rateKbps * 30) / 13;
                else 
                    rates[i]= (currate->info[i].rateKbps * 27) / 13;
            } 
            else if(IS_CHAN_HT20(sc->sc_curchan))
            {
                if (ieee80211com_has_htflags(ic, IEEE80211_HTF_SHORTGI20)) 
                    rates[i] = (currate->info[i].rateKbps * 10) / 9;
            }
        }
        else if (currate->info[i].rateCode <= 0x1b) 
            rates[i] = currate->info[i].rateKbps;
    }
/*
*if our products support tripple stream in the future
*the nrate should modified properly.
*/
    *nrates = currate->rateCount -8;
    return 0;
...
/*zhaoyang1 transplant from 717*/
/*modified by chenming for rate display*/
    struct ath_softc_net80211   *scn = ATH_SOFTC_NET80211(ic);
    struct ath_softc *sc = scn->sc_dev;
    ...
    u_int32_t  rates[IEEE80211_RATE_MAXSIZE];
    int i,ret;
/*ended by chenming*/
/*modified by chenming to get maxtxpower-issue num PCAPVXN-78*/
    //int step = 0;
    //u_int32_t txpowlimit;
    u_int32_t maxtxpower;
 ...
 */
    maxtxpower = wlan_get_param(vap, IEEE80211_MAX_TXPOWER);
	maxtxpower = MIN(30, maxtxpower); //Modify by wangzhicong for AF4000 maxtxpower 2012-10-23 WBS-34
    range->num_txpower = maxtxpower +1;
    memset(range->txpower, 0, sizeof(range->txpower));
    for(i =0;i<maxtxpower +1;i++){
        range->txpower[i] = i;
     }
/*end-chenming*/
/*zhaoyang1 transplant end*/
...
/*zhaoyang1 transplant from 717*/
/* modified by chenming for rate display */
    if (IS_CHAN_HT(sc->sc_curchan))
    	ret = ieee80211_get_htrate(ic, rates, &nrates);
    else
    	ret = wlan_get_bss_rates(vap, rates, IEEE80211_RATE_MAXSIZE, &nrates) ;
    if (ret ==0 ){
    ...
    __u32 *mode, char *extra)
{
    ...
    printk(" %s: imr.ifm_active=%x, new mode=%d, valid=%d \n",
...
static int 
ieee80211_ioctl_giwVapRateFrame(struct net_device *dev, 
    struct iw_request_info *info, 
    struct iw_point *data, char *extra)
{
    u_int8_t    i = 0;
    ...
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    struct iw_vap_rate_frame_req    *req = (struct iw_vap_rate_frame_req *)extra;

    struct mac_rate_frame_info *req_rx_data = &(req->vap_rx_data);
    struct mac_rate_frame_info *req_tx_data = &(req->vap_tx_data);

    u_int64_t   *iv_rx_mcs = vap->iv_rx_mcs_count;
    u_int64_t   *iv_tx_mcs = vap->iv_tx_mcs_count;

    struct rate_count   *iv_rx_rate = vap->iv_rx_rate_index;
    struct rate_count   *iv_tx_rate = vap->iv_tx_rate_index;
    
    for(; i < 12; i++){
        req_rx_data->rate[i].count = iv_rx_rate[i].count;
        req_rx_data->rate[i].dot11Rate = iv_rx_rate[i].dot11Rate;
        req_rx_data->mcs[i] = iv_rx_mcs[i];

        req_tx_data->rate[i].count = iv_tx_rate[i].count;
        req_tx_data->rate[i].dot11Rate = iv_tx_rate[i].dot11Rate;
        req_tx_data->mcs[i] = iv_tx_mcs[i];        
    }
    for(; i < 23; i++){
        req_rx_data->mcs[i] = iv_rx_mcs[i];   
        req_tx_data->mcs[i] = iv_tx_mcs[i];   
...
static int 
ieee80211_ioctl_giwNiRateFrame(struct net_device *dev, 
    struct iw_request_info *info, 
    struct iw_point *data, char *extra)
{

    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    struct iw_ni_rate_frame_req    *req = (struct iw_ni_rate_frame_req *)extra;

    ieee80211_iterate_node_for_req(req, vap);
...
/*End:gengzongjie added end */
    ...
    */
    ...
    case IEEE80211_M_HOSTAP:
		...
		/* Begin:gengzongjie added for rrm 2013-11-26 */
		...
		switch (vap->iv_des_mode)
		{
			...
			case IEEE80211_MODE_11NA_HT40MINUS:
			case IEEE80211_MODE_11NA_HT40:
				//scan_params->flags = IEEE80211_SCAN_PASSIVE | IEEE80211_SCAN_5GHZ; 
				scan_params->flags = IEEE80211_SCAN_5GHZ;
				/*
				if(255 != iwlist_scan_current_chan_na)
				{
					vap->iwlist_scan_current_chan=iwlist_scan_current_chan_na;
					iwlist_scan_current_chan_na = 255;
				}
				*/
				break;
			...
			case IEEE80211_MODE_11NG_HT40:
				...
				/*
				if(255 != iwlist_scan_current_chan_ng)
				{
					vap->iwlist_scan_current_chan=iwlist_scan_current_chan_ng;
					iwlist_scan_current_chan_ng = 255;
				}
				*/
				break;
		...
		if (ic->ic_iwlist_scan_mode & 0x01)
			scan_params->flags |= IEEE80211_SCAN_PASSIVE;
		if (ic->ic_iwlist_scan_mode & 0x02) 
			scan_params->flags |= IEEE80211_SCAN_ACTIVE;
		
		if (ic->ic_iwlist_scan_chan) {
			scan_params->num_channels = 1;
			scan_params->chan_list = OS_MALLOC(osifp->os_handle, scan_params->num_channels, GFP_KERNEL);
			scan_params->chan_list[0] = ic->ic_iwlist_scan_chan;
		} else if (ic->ic_iwlist_scan_onebyone) {
	  		vap->iwlist_scanning=1;
	 		vap->iwlist_scanning_cancl=0;
        }
		
		if (ic->ic_iwlist_listen_time_each_chan) {
			scan_params->min_dwell_time_passive = ic->ic_iwlist_listen_time_each_chan * 2 / 3;
    	    scan_params->max_dwell_time_passive = ic->ic_iwlist_listen_time_each_chan;
		} else {
...
domlme(void *arg, wlan_node_t node)
{
    ...
    case IEEE80211_MLME_DISASSOC:
		...
		node->ni_vap->iv_stats.is_disassoc_ioctl++;
    ...
    void *w, char *extra)
{
    ...
    struct ieee80211_node *ni = NULL;
#else
	struct ieee80211_node *ni = NULL;//Add by linrongqi for delete unnecessary netlink 2012-11-15
#endif
    ...
    IEEE80211_NOTE_MAC_MGMT_DEBUG(vap, mlme->im_macaddr,
		" <INFO> [Step ** - IOCTL] %s: dev = %s, ioctl = 0x%04X, name = %s\n", 
		__func__, dev->name, IEEE80211_IOCTL_SETMLME, "SETMLME");
    ...
    case IEEE80211_MLME_DEAUTH:
        ...
        if (!IEEE80211_ADDR_EQ(mlme->im_macaddr, ieee80211broadcastaddr)) {
				...
				if(ioctl_del_sta){
					...
					{
						ni->ni_maintype = 2;
						ni->ni_subtype = (mlme->im_op == 2) ? 2 : 1;
						IEEE80211_NOTE_MAC_MGMT_DEBUG(vap, mlme->im_macaddr,
		  					" <INFO> [Step ** - IOCTL] %s: ioctl %s to the station(0x%02X%04X), report delete station\n", 
		  					__func__, (mlme->im_op == 2) ? "disassoc" : "deauth", ni->ni_maintype, ni->ni_subtype);
...
#if AUTELAN_SOLUTION2
				IEEE80211_NOTE_MAC_MGMT_DEBUG(vap, mlme->im_macaddr,
		  				" <INFO> [Step ** - IOCTL] %s: ioctl %s to all station(0x%02X%04X), report delete all station\n", 
		  				__func__, (mlme->im_op == 2) ? "disassoc" : "deauth", 2, 3);
				ieee80211_allsta_leave_send_event(vap, 2, 3);
#endif
				/*suzhaoyu add end*/
    ...
    case IEEE80211_MLME_UNAUTHORIZE:
        ...
        {
			/*Begin:Add by linrongqi for AUTHORIZE without node return fail 2012-11-7*/
        	if( 0 != wlan_node_authorize(vap, 1, mlme->im_macaddr))
				return -EINVAL;
			/*End:Add by linrongqi for AUTHORIZE without node return fail 2012-11-7*/
#if UMAC_SUPPORT_SMARTANTENNA
            ni = ieee80211_find_node(&vap->iv_ic->ic_sta, mlme->im_macaddr);
            if (ni)
            {
...
static void wlan_set_can_locate_probe(wbuf_t msg)
{
	return ;
}

/*save the socket ID for the radio forward the service packets*/
struct scan_locate iv_scan_notlocate={
NULL,0,0
};

static int wlan_set_scan_locate_start(struct net_device *dev)
{
    osif_dev  *osifp = ath_netdev_priv(dev);
    wlan_if_t vap = osifp->os_if;
	...
	//init  scan locate struct
    if(iv_scan_notlocate.sl_sock !=NULL &&  vap->iv_locate !=0){
		printk("scan locate function has start!\n");
		return 1;
	}

      
	#if 0
	vap->iv_scan_locate = (struct scan_locate * )OS_MALLOC(ic->ic_osdev,sizeof(struct scan_locate), GFP_KERNEL);
	if(vap->iv_scan_locate == NULL){
		printk("scan locate malloc error!\n");
	...
	#endif
    if(iv_scan_notlocate.sl_sock == NULL){
		iv_scan_notlocate.sl_sock = OS_NETLINK_CREATE(NETLINK_DETECTOR_SCANNER, 
	                        1, (void *)&wlan_set_can_locate_probe , THIS_MODULE);
       
		if (iv_scan_notlocate.sl_sock == NULL) {
			iv_scan_notlocate.sl_sock =
				OS_NETLINK_CREATE(NETLINK_DETECTOR_SCANNER + 1, 1,
						  (void *)
						  &wlan_set_can_locate_probe,
						  THIS_MODULE);
		}
		if(iv_scan_notlocate.sl_sock == NULL){
			printk("scan locate malloc sock error\n");
			
			return 1;
		}else{
			printk("scan locate malloc sock ok\n");
		}
		iv_scan_notlocate.sl_flag = 1; 
    }
 
 	vap->iv_locate = 1; 

	//set rx to Promiscuous mode
	struct net_device *comdev;
	comdev = ((osif_dev *)osifp)->os_comdev;
    scn = ath_netdev_priv(comdev);
    sc = ATH_DEV_TO_SC(scn->sc_dev);
	
    sc->sc_locate = 1;
	
 //   vap->iv_radioid = comdev->name[4] - 48;
	return 0;
}

static int wlan_set_scan_locate_end(struct net_device *dev)
{
    osif_dev  *osifp = ath_netdev_priv(dev);
    wlan_if_t vap = osifp->os_if;
    ...
    void *w, char *extra)
{
    ...
    switch (param)
    {
	case IEEE80211_OPENESSID_INFO:
		if(value == 0){
			vap->iv_whitelist_duty = 0;
		}else{

			init_whitelist(vap);
			vap->iv_whitelist_duty = 1;
		}
		break;

/* Begin: gengzj added for wifipos 2013-11-26 */
	/*AUTELAN-Added-begin:Added by pengdecai for wifi scan locate function*/	
	case IEEE80211_PARAM_SCAN_LOCATE:
		if(value == 0)
			wlan_set_scan_locate_end(dev);
		else 
			wlan_set_scan_locate_start(dev);
		break;

	case IEEE80211_PARAM_SL_DEG:
       	 vap->iv_sl_debug = value;
		break;
	case IEEE80211_PARAM_SCAN_ASINFO:
		vap->iv_sl_asinfo = value;
		break;
	/*AUTELAN-Added-end:Added by pengdecai for wifi scan locate function*/
/* End: gengzj added end */
    /*zhaoyang1 transplant from 717*/
    ...
    case IEEE80211_PARAM_AUTHMODE:
        ...
        */
        if ((value != IEEE80211_AUTH_OPEN) && (value != IEEE80211_AUTH_SHARED) 
                && (value != IEEE80211_AUTH_8021X) && (value != IEEE80211_AUTH_AUTO)) 
        {
            if (osifp->m_count)
    ...
    */
    ...
    case IEEE80211_PARAM_BLOCKDFSCHAN:
		...
		case IEEE80211_PARAM_SCAN_WITH_WEIGHT:
			if(value <= 1)
				vap->iv_select_with_weight = value; 
			else
				printk("Error argument!\n");
			break;
			
		case IEEE80211_PARAM_SCAN_WITH_NEAR_RSSI:
			if(value <= 1)
				vap->iv_select_with_near_rssi = value;	
			else
				printk("Error argument!\n");
			break;
			
		case IEEE80211_PARAM_SCAN_WITH_CHANNEL:
			if(value <= 200)
				vap->iv_select_with_channel = value;	
			else
				printk("Error argument!\n");
			break;
		
		case IEEE80211_PARAM_AP_ATTACK_MAX_CNT:
			if(value <= 50000)
				vap->attack_max_cnt= value; 
			else
				printk("Error argument!\n");
			break;
#if 0
		case IEEE80211_PARAM_AP_STA_INTERFERENCE:
			if(value <= 50000)
				vap->sta_interferece_detect = value;	
			else
				printk("Error argument!\n");
			break;
		case IEEE80211_PARAM_AP_STA_INTERFERENCE_COUNT:
			if(value <= 50000 && value > 0)
				vap->sta_interferece_count = value; 
			else
				printk("Error argument!\n");
			break;
#endif 
		case IEEE80211_PARAM_AP_PROBE_ATTACK_MAX_CNT:
			if(value <= 50000)
				vap->probe_attack_max_cnt= value;	
			else
				printk("Error argument!\n");
			break;
			
		case IEEE80211_PARAM_AP_FLOOD_DETECT:
			if(value <= 1)
				vap->flood_detect= value;	
			else
				printk("Error argument!\n");
			break;	
			
		case IEEE80211_PARAM_AP_SPOOF_DETECT:
			if(value <= 1)
				vap->spoof_detect= value;
			else
				printk("Error argument!\n");
			break;	
			
		case IEEE80211_PARAM_AP_WEAKIV_DETECT:
			if(value <= 1)
				vap->weakiv_detect= value;	
			else
				printk("Error argument!\n");
			break;	
			
		case IEEE80211_PARAM_AP_CLEAR_ILLDEV_HISTORY:
			if(value <= 1)
			{
				vap->clear_illegaldev_history= value;
				if(value == 1)
					clear_illegaldev_history(vap);
			}
			else
				printk("Error argument!\n");
			break;	
			
		case IEEE80211_PARAM_AP_CLEAR_ATTACK_HISTORY:
			if(value <= 1)
			{
				vap->clear_attack_history= value;
				if(value == 1)
					clear_attack_history(vap);
			}
			else
				printk("Error argument!\n");
			break;	
			
		case IEEE80211_PARAM_AP_ATTACK_STATISTICS:
			if(value <= 1)
			{
				vap->clear_attack_statistics= value;
				if(value == 1)
					clear_attack_stat_history(vap);
			}
			else
				printk("Error argument!\n");
			break;	
		
		case IEEE80211_PARAM_AP_WEAKIV_THRESHOLD:
			if(value)
				vap->iv_weakiv_threshold = value;	
			else
				printk("Error argument!\n");
			break;
		
		case IEEE80211_PARAM_AP_WIDS_INTERVAL:
			if(value)
				vap->wids_interval = value; 
			else
				printk("Error argument!\n");
			break;
		/*pengruofeng--add end 2011-5-27*/

		/*pengruofeng add for debug_code 2011-6-23*/
		case IEEE80211_PARAM_AP_DEBUG_CODE:
			if(value >= 0)
				vap->iv_debug_code= value;
			else
				printk("Error debug_code argument!\n");
			break;
		/*pengruofeng add end for debug_code 2011-6-23*/
		/*suzhaoyu add for customer online-traffic limit*/
 		case IEEE80211_PARAM_LOWEST_TRA_TIMELENGTH:
			if(value >0)
			{
				vap->lowest_traffic_limit_timelength=value;
				if(vap->lowest_traffic_limit_switch)
					OS_SET_TIMER(&vap->online_traffic_timer, 0);
			}
			else
				printk("Error time length !\n");
			break;
		case IEEE80211_PARAM_LOWEST_TRA_THESHOLD:
			if(value >=0)
				vap->lowest_traffic_limit_threshold=value;
			else
				printk("Error traffic threshold value !\n");
			break;
		case IEEE80211_PARAM_LOWEST_TRA_LIM:
			if(value == 1 || value == 0)
			{	
				vap->lowest_traffic_limit_switch=value;
				if(value)
					OS_SET_TIMER(&vap->online_traffic_timer, 0);
			}
			else
				printk("Error lowest traffic limit switch value!only 1 or 0.\n");
			break;
		/*suzhaoyu addend*/
		/*zhaoyang add for noisefloor switch when channel 0*/
		case IEEE80211_PARAM_NOISEFLOOR_ENABLE:
			if(value == 1 || value == 0)
			{
				vap->noisefloor_enable = value;
				printk("set noisefloor %d\n",value);	
			}				
			else 
				printk("Error argument!,only 1 or 0.\n");
			break;
		/*zhaoyang add end*/		
		/*zhaoyang1 transplant end*/
        case IEEE80211_PARAM_RSSITHRE_DATA_FRAME: //added by chenming for rssithre
            vap->iv_rssi_threshold = (((value > 95)?95:value)<0)?0:value;
            break;
/*packet power added by chenming */
        case IEEE80211_PARAM_PACKET_POWER:
            vap->iv_packet_power_enable = value;
            break; 
/*ended by chenming*/
#if ATH_SUPPORT_WAPI
	...
	case IEEE80211_PARAM_RD_TRAP:		
		if(value == 1 || value == 0)
		{
			vap->rd_trap = value;
			printk("set trap switch %d\n",value);//zhaoyang modify for trap switch CHINAMOBILE-116
		}				
		else 
			printk("Error argument!,only 1 or 0.\n");
		break;
...
*/
	...
	/* gengzongjie transplant the modifcation by zhanghu for converting multiply frame to umcast frame for every node 2013-4-28 */	
	/*Begin:Add by zhanghu for converting multiply frame to umcast frame for every node 2012-12-24*/
	case IEEE80211_PARAM_TO_ENODE_MEDEBUG:
		wlan_set_param(vap, IEEE80211_TO_ENODE_MEDEBUG, value);
		break;
	/*End:Add by zhanghu for converting multiply frame to umcast frame for every node 2012-12-24*/
	/* gengzongjie transplant end */
...
/*yanggs add for thinap wds*/
    case IEEE80211_PARAM_VAP_WDS:
         if (value == 1 || value == 0)
         {
                vap->vap_wds= value;
                printk("set vap_wds %d\n",value);
         }
         else
                printk("Error argument!,only 1 or 0.\n");
         break;
...
#undef IBSSDFS_RECOVER_MASK              
    ...
    /*Start: added by zhanghu for killing the bug that multicast frame is  disorder */
    case IEEE80211_PARAM_MCAST_DBG_LVL:
		vap->iv_mcast_debug = (value & 0xFFFFFFFF);	
	    break;
    /*End: added by zhanghu for killing the bug that multicast frame is  disorder */

    
    /*Start: added by zhanghu to control the manager frame*/
    case IEEE80211_PARAM_CONTROL_MANAGER_FRAME:
		vap->iv_manager_rate = (value & 0xFFFFFFFF);	
	    break;    
    /*End: added by zhanghu to control the manager frame*/

	/*ljy--add begin for ac roaming eap at splitmac on 2013-1-15*/
	case IEEE80211_PARAM_EAP:
		vap->iv_eap = value;
		break;
	/*ljy--add end*/

    /* add by zhaohan@autelan.com. Control the sending freq of deauth frames, 2013-12-12 */
    case IEEE80211_PARAM_DEAUTH_SEND_INTERVAL:
        if (value >= 0 && value <= IEEE80211_DEAUTH_INTERVAL_MAX)
            vap->iv_deauth_send_interval= value*HZ;
        else
            retv = EINVAL;
        break;
    /* add by zhaohan@autelan.com end */
...
/* Begin: gengzj added for wifipos 2013-11-26 */
	/*AUTELAN-Added-begin:Added by pengdecai for wifi scan locate function*/	
	case IEEE80211_PARAM_SCAN_LOCATE:
		param[0] = vap->iv_locate;
		break;
	case IEEE80211_PARAM_SL_DEG:
		param[0] = vap->iv_sl_debug;
		break;
	case IEEE80211_PARAM_SCAN_ASINFO:
		param[0] = vap->iv_sl_asinfo;
		break;
	case IEEE80211_PARAM_SL_PCK:
		if(iv_scan_notlocate.sl_flag == 1){
			printk("all radio scan packets = %d\n",iv_scan_notlocate.sl_pckcnt);
		}
		param[0] = vap->iv_sl_pckcnt;
		break;
	/*AUTELAN-Added-end:Added by pengdecai for wifi scan locate function*/	
/* End: gengzj added end */
    ...
    */
	...
	/* gengzongjie transplant the modification by zhanghu for converting multiply frame to umcast frame for every node 2013-4-28 */
	/*Begin:Add by zhanghu for converting multiply frame to umcast frame for every node 2012-12-24*/
	case IEEE80211_PARAM_TO_ENODE_MEDEBUG:
		param[0] = wlan_get_param(vap, IEEE80211_TO_ENODE_MEDEBUG);
		break;
	/*End:Add by zhanghu for converting multiply frame to umcast frame for every node 2012-12-24*/
	/* gengzongjie transplant end */
	...
	/*zhaoyang add for noisefloor switch when channel 0*/
	case IEEE80211_PARAM_NOISEFLOOR_ENABLE:
		param[0]=vap->noisefloor_enable;
		break;
	/*zhaoyang add end*/	
    case IEEE80211_PARAM_AP_ATTACK_MAX_CNT:
	param[0] = vap->attack_max_cnt; 
	break;
    case IEEE80211_PARAM_RSSITHRE_DATA_FRAME: //added by chenming for rssithre
        param[0] = vap->iv_rssi_threshold;
        break;
/*packet power added by chenming */
    case IEEE80211_PARAM_PACKET_POWER: 
        param[0] = vap->iv_packet_power_enable;
        break;
/*ended by chenming*/
#if 0
	case IEEE80211_PARAM_AP_STA_INTERFERENCE:
		param[0] = vap->sta_interferece_detect;	
		break;
	case IEEE80211_PARAM_AP_STA_INTERFERENCE_COUNT:
		param[0] = vap->sta_interferece_count;	
		break;
#endif
    case IEEE80211_PARAM_AP_PROBE_ATTACK_MAX_CNT:
	param[0] = vap->probe_attack_max_cnt;	
	break;

    case IEEE80211_PARAM_AP_FLOOD_DETECT:
	param[0] = vap->flood_detect;	
	break;	

    case IEEE80211_PARAM_AP_SPOOF_DETECT:
	param[0] = vap->spoof_detect;	
	break;	

    case IEEE80211_PARAM_AP_WEAKIV_DETECT:
	param[0] =	vap->weakiv_detect; 
	break;	

    case IEEE80211_PARAM_AP_CLEAR_ILLDEV_HISTORY:
	param[0] =	vap->clear_illegaldev_history;	
	break;	

    case IEEE80211_PARAM_AP_CLEAR_ATTACK_HISTORY:
	param[0] =	vap->clear_attack_history;	
	break;	

    case IEEE80211_PARAM_AP_ATTACK_STATISTICS:
	param[0] =	vap->clear_attack_statistics;	
	break;	

    case IEEE80211_PARAM_SCAN_WITH_WEIGHT:
	param[0] = vap->iv_select_with_weight;
	break;

    case IEEE80211_PARAM_SCAN_WITH_NEAR_RSSI:
	param[0] = vap->iv_select_with_near_rssi;
	break;

    case IEEE80211_PARAM_SCAN_WITH_CHANNEL:
	param[0] = vap->iv_select_with_channel;
	break;
			
    case IEEE80211_PARAM_AP_WEAKIV_THRESHOLD:
	param[0] =	vap->iv_weakiv_threshold;
	break;
	
    case IEEE80211_PARAM_AP_WIDS_INTERVAL:
	param[0] =	vap->wids_interval;
	break;
	/*pengruofeng--add end 2011-5-27*/
	...
	/*pengruofeng add for debug_code 2011-6-23*/
	case IEEE80211_PARAM_AP_DEBUG_CODE:
	param[0] =	vap->iv_debug_code;
	break;
	/*pengruofeng add end for debug_code 2011-6-23*/
    /*suzhaoyu add for customer online-traffic limit*/
    case IEEE80211_PARAM_LOWEST_TRA_LIM:
	param[0] =	vap->lowest_traffic_limit_switch;
	break;
    case IEEE80211_PARAM_LOWEST_TRA_TIMELENGTH:
	param[0] =	vap->lowest_traffic_limit_timelength;
	break;
    case IEEE80211_PARAM_LOWEST_TRA_THESHOLD:
	param[0] =	vap->lowest_traffic_limit_threshold;
	break;
    /*suzhaoyu addend*/
	/*zhaoyang1 transplant end*/
    ...
    */
	...
	/*zhaoyang1 add start for  probe request REQUIREMENTS-340*/
	case IEEE80211_PARAM_PROBE_REQUEST:
		param[0] = vap->iv_probe_request;
		break;
	/*zhaoyang1 add end*/
    
    /*Start: added by zhanghu for killing the bug that multicast frame is  disorder */
    case IEEE80211_PARAM_MCAST_DBG_LVL:
        param[0] = (vap->iv_mcast_debug & 0xFFFFFFFF);
		break;
    /*End: added by zhanghu for killing the bug that multicast frame is  disorder */

    /*Start: added by zhanghu to control the manager frame*/
    case IEEE80211_PARAM_CONTROL_MANAGER_FRAME:
        param[0] = (vap->iv_manager_rate & 0xFFFFFFFF);
	    break;    
    /*End: added by zhanghu to control the manager frame*/
    ...
    /* add by zhaohan@autelan.com. Control the sending freq of deauth frames, 2013-12-12 */
    case IEEE80211_PARAM_DEAUTH_SEND_INTERVAL:
        param[0] = vap->iv_deauth_send_interval/HZ;
        break;
    /* add by zhaohan@autelan.com end */


	case IEEE80211_PARAM_QUEUE_INFO:
		autelan_queue_info(ic);
		break;
		/* AUTELAN-Added-Begin : &tuqiang for openessid */

	case IEEE80211_OPENESSID_INFO:
		get_vap_openessid_info(vap);
		break;
		/* AUTELAN-Added-End : tuqiang@autelan.com */
...
/*zhaoyang1 transplant from 717*/
/*zhaoyang modify for XJPB44V2 compile error*/
...
ieee80211_ioctl_getstastats(struct net_device *dev, struct iwreq *iwr)
{
	...
	/*zhaoyang1 transplant from 717*/
	/*xiaruixin modify for report enable reset*/
	wlan_if_t vap = NETDEV_TO_VAP(dev);
	/*xiaruixin modify end */
	/*zhaoyang1 transplant end*/
    struct ieee80211com *ic = vap->iv_ic;
...
/* Begin:zhaoyang1 added for rrm result aging 2013-12-26 */
//#define AGING_TIMEOUT_SEC 60
    //if ((long unsigned int)wlan_scan_entry_age(se)> 1000* AGING_TIMEOUT_SEC){ 
		//return 0;
    //}
/* End:zhaoyang1 added end */
...
//#define AGING_TIMEOUT_SEC 60
	/*zhaoyang1 transplant from 717*/
    /*suzhaoyu modified for rogue ap num unnormal began    key:PCAPVXN-57 */
    //if ((long unsigned int)wlan_scan_entry_age(se)> 5000* AGING_TIMEOUT_SEC){ 
       ///return 0;
   //}
/* End:zhaoyang1 added end */
    /*suzhaoyu modified for rogue ap num unnormal end key:PCAPVXN-57*/
	/*zhaoyang1 transplant end*/
...
get_sta_info(void *arg, wlan_node_t node)
{
    ...
    si = req->si;
	/*zhaoyang1 transplant from 717*/
    si->isi_assoc_time = jiffies - wlan_node_get_assocuptime(node);/*pengruofeng--modified for sta association time 2011-6-2*/
	/*zhaoyang1 transplant end*/
    si->isi_len = len;
    si->isi_ie_len = ielen;
    ...
    if (wlan_node_txrate_info(node, &rinfo) == 0) {
		/* gengzongjie modified to reduce redundant code 2013-4-26 */
        //si->isi_txratekbps = rinfo.rate; 
		/* gengzongjie modified end */
		
		/* gengzongjie transplant from service provider 2013-4-26 */
        /*Begin: Modified by zhanghu to get the last frame tx rate in wlanconfig ath0 list 2013-01-28*/
        si->isi_txratekbps = rinfo.lastrate;
        /*End:  Modified by zhanghu to get the last frame tx rate in wlanconfig athX list 2013-01-28*/
		/* gengzongjie transplant end */
    }
    ...
    if (wlan_node_rxrate_info(node, &rinfo) == 0) {
		...
		/* gengzongjie transplant from service provider 2013-4-26 */
        /*Begin: Modified by zhanghu to get the last frame rx rate in wlanconfig athX list 2013-01-28*/
        si->isi_rxratekbps = rinfo.lastrate;
        /*End:  Modified by zhanghu to get the last frame rx rate in wlanconfig athX list 2013-01-28*/
		/* gengzongjie transplant end */
    }
    /*ended by chenming*/
	...
	/* AUTELAN-Added-Begin : &tuqiang for stations' seqnum checking func. 2014-07-04 */
	si->isi_refcnt = ieee80211_node_refcnt(node);
	/* AUTELAN-Added-End : tuqiang@autelan.com */
...
static const struct iw_priv_args ieee80211_priv_args[] = {
    ...
    */
	...
	/*zhaoyang1 transplant from 717*/
    /*pengruofeng add for wids 2011-5-30*/
    { IEEE80211_PARAM_AP_ATTACK_MAX_CNT,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_attack_cnt"},
    { IEEE80211_PARAM_AP_ATTACK_MAX_CNT,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "attack_cnt"},
#if 0    
	/*lyb add start for sta interferce test*/	  
	{ IEEE80211_PARAM_AP_STA_INTERFERENCE,
	  0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_sta_interfer"},
	{ IEEE80211_PARAM_AP_STA_INTERFERENCE,
	  IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "sta_interfer"},

	{ IEEE80211_PARAM_AP_STA_INTERFERENCE_COUNT,
	  0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_interfer_count"},
	{ IEEE80211_PARAM_AP_STA_INTERFERENCE_COUNT,
	  IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "interfer_count"},
	/*lyb add end*/
#endif
    { IEEE80211_PARAM_AP_PROBE_ATTACK_MAX_CNT,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_prb_atk_cnt"},
    { IEEE80211_PARAM_AP_PROBE_ATTACK_MAX_CNT,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "prb_atk_cnt"},
    { IEEE80211_PARAM_AP_FLOOD_DETECT,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_flood_detect"},
    { IEEE80211_PARAM_AP_FLOOD_DETECT,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "flood_detect"},
    { IEEE80211_PARAM_AP_SPOOF_DETECT,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_spoof_detect"},
    { IEEE80211_PARAM_AP_SPOOF_DETECT,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "spoof_detect"},
    { IEEE80211_PARAM_AP_WEAKIV_DETECT,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_wk_iv_detect"},
    { IEEE80211_PARAM_AP_WEAKIV_DETECT,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "wk_iv_detect"},
    { IEEE80211_PARAM_AP_CLEAR_ILLDEV_HISTORY,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_clr_illdev"},
    { IEEE80211_PARAM_AP_CLEAR_ILLDEV_HISTORY,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "clr_illdev_his"},
    { IEEE80211_PARAM_AP_CLEAR_ATTACK_HISTORY,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_clr_atk_his"},
    { IEEE80211_PARAM_AP_CLEAR_ATTACK_HISTORY,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "clr_atk_his"},
    { IEEE80211_PARAM_AP_ATTACK_STATISTICS,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_clr_atk_stat"},
    { IEEE80211_PARAM_AP_ATTACK_STATISTICS,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "clr_atk_stat"},
    { IEEE80211_PARAM_SCAN_WITH_WEIGHT,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "select_weight"},
    { IEEE80211_PARAM_SCAN_WITH_WEIGHT,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_selectweight"},
    { IEEE80211_PARAM_SCAN_WITH_NEAR_RSSI,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "near_rssi"},
    { IEEE80211_PARAM_SCAN_WITH_NEAR_RSSI,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_near_rssi"},
    { IEEE80211_PARAM_SCAN_WITH_CHANNEL,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "select_chan"},
    { IEEE80211_PARAM_SCAN_WITH_CHANNEL,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_select_chan"},
    { IEEE80211_PARAM_AP_WEAKIV_THRESHOLD,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_wk_iv_thr"},
    { IEEE80211_PARAM_AP_WEAKIV_THRESHOLD,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "wk_iv_thr"},
    { IEEE80211_PARAM_AP_WIDS_INTERVAL,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_wids_intval"},
    { IEEE80211_PARAM_AP_WIDS_INTERVAL,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "wids_intval"},
    /*pengruofeng--add end 2011-5-30*/
	
	/*pengruofeng add for debug_code 2011-6-23*/
    { IEEE80211_PARAM_AP_DEBUG_CODE,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_debug_code"},
    { IEEE80211_PARAM_AP_DEBUG_CODE,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "debug_code"},
	/*pengruofeng add end for debug_code 2011-6-23*/
	
	/*zhaoyang add for noisefloor switch when channel 0*/
	{ IEEE80211_PARAM_NOISEFLOOR_ENABLE,
    ...
    */
	...
	/* gengzongjie transplant the modification by zhanghu for converting multiply frame to umcast frame for every node 2013-4-28 */
    /*Begin:Add by zhanghu for converting multiply frame to umcast frame for every node 2012-12-24*/
    { IEEE80211_PARAM_TO_ENODE_MEDEBUG,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "me_enode_en" },
    { IEEE80211_PARAM_TO_ENODE_MEDEBUG, 0,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_me_enode_en"},
    /*End:Add by zhanghu for converting multiply frame to umcast frame for every node 2012-12-24*/
	/* gengzongjie transplant end */
    ...
    */
	...
	/*zhaoyang1 transplant from 717*/
	/*zhaoyang modify for add rd_trap switch*/
	{ IEEE80211_PARAM_RD_TRAP,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "trap_enable" },//zhaoyang modify for trap switch CHINAMOBILE-116
    { IEEE80211_PARAM_RD_TRAP, 0,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_trap_enable" }, //zhaoyang modify for trap switch CHINAMOBILE-116
	/*zhaoyang modify end*/
#endif /*ATH_SUPPORT_WAPI*/
    /*suzhaoyu add for customer online-traffic limit*/
    ...
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "set_lowtraf_time" },
    { IEEE80211_PARAM_LOWEST_TRA_TIMELENGTH, 0,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_lowtraf_time" }, 
    { IEEE80211_PARAM_LOWEST_TRA_THESHOLD,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "set_lowtraf_thre" },
    { IEEE80211_PARAM_LOWEST_TRA_THESHOLD, 0,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_lowtraf_thre" }, 
    /*suzhaoyu addend*/
	/*zhaoyang1 transplant end*/
 /*ATH_SUPPORT_WAPI*/
/*<begin : transplant by caizhibang from apv5*/
/*yanggs add for thinap wds*/
{ IEEE80211_PARAM_VAP_WDS,
IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "vap_wds" },
{ IEEE80211_PARAM_VAP_WDS, 0,
IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_vap_wds" },
/*yanggs add end*/
/*end : transplant by caizhibang from apv5 >*/
#ifdef ATH_WPS_IE
    { IEEE80211_PARAM_WPS,
   ...
   /* Video debug parameters */   
    ...
    /*Start: added by zhanghu for killing the bug that multicast frame is  disorder */
    { IEEE80211_PARAM_MCAST_DBG_LVL,
        IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "mcast_dbgLVL" },
    { IEEE80211_PARAM_MCAST_DBG_LVL,
        0,IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_mcast_dbgLVL" },
    /*End: added by zhanghu for killing the bug that multicast frame is  disorder */


    /*Start: added by zhanghu to control the manager frame*/
    { IEEE80211_PARAM_CONTROL_MANAGER_FRAME,
        IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "set_Mrate" },
    { IEEE80211_PARAM_CONTROL_MANAGER_FRAME,
        0,IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_Mrate" },
    /*End: added by zhanghu to control the manager frame*/

    /*packet power added by chenming */
    { IEEE80211_PARAM_PACKET_POWER,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "PktPowEnable" },
    { IEEE80211_PARAM_PACKET_POWER,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "getPktPowEnable" },
    /*ended by chenming*/

    /*ljy--add begin for ac roaming eap at splitmac on 2013-1-15*/
    { IEEE80211_PARAM_EAP,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "eap"},
    { IEEE80211_PARAM_EAP,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_eap"},
    /*ljy--add end*/

    /* add by zhaohan@autelan.com. Control the sending freq of deauth frames, 2013-12-12 */
    { IEEE80211_PARAM_DEAUTH_SEND_INTERVAL,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "DeauthIntval"},
    { IEEE80211_PARAM_DEAUTH_SEND_INTERVAL,
    0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "getDeauthIntval"},
    /* add by zhaohan@autelan.com end */

	/* AUTELAN-Added-Begin : &tuqiang for openessid */

    { IEEE80211_PARAM_QUEUE_INFO,
		0,IW_PRIV_TYPE_INT|IW_PRIV_SIZE_FIXED|1,"get_queueinfo"},

    { IEEE80211_OPENESSID_INFO,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "whitelistduty"},

    { IEEE80211_OPENESSID_INFO,
		0,IW_PRIV_TYPE_INT|IW_PRIV_SIZE_FIXED|1,"getopenessid"},

	/* AUTELAN-Added-End : tuqiang@autelan.com */

/* Begin: gengzj added for wifipos 2013-11-26 */
    /*AUTELAN-Added-begin:Added by pengdecai for wifi scan locate function*/	
    { IEEE80211_PARAM_SCAN_LOCATE,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "locate"},
    { IEEE80211_PARAM_SCAN_LOCATE,0,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_locate"}, 
      
    { IEEE80211_PARAM_SL_DEG,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "locate_dbug"},
    { IEEE80211_PARAM_SL_DEG,0,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_locate_dbug"}, 

    { IEEE80211_PARAM_SL_PCK,0,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_locate_pkt"}, 
      
    { IEEE80211_PARAM_SCAN_ASINFO,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "locate_assoc"},  
    { IEEE80211_PARAM_SCAN_ASINFO,0,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_locate_assoc"}, 
...
static const iw_handler ieee80211_handlers[] = {
    ...
    /*Start: Add by zhanghu to statistics of every frame by the rate*/
    (iw_handler) NULL,  /*SIOCSIWPMKS   PMKSA cache operation */
    (iw_handler) NULL,  /*SIOCGIWPMKS   PMKSA cache operation */

    (iw_handler) NULL,                             /* SIOCSIWVAPRATEFRAME */
    (iw_handler) ieee80211_ioctl_giwVapRateFrame,   /* SIOCGIWVAPRATEFRAME */

    (iw_handler) NULL,                             /* SIOCSIWNIRATEFRAME */
    (iw_handler) ieee80211_ioctl_giwNiRateFrame,    /* SIOCGIWNIRATEFRAME */
...
static int
ieee80211_ioctl_autelan_80211_cache(struct net_device *dev, struct iwreq *iwr)
{
	struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
	struct ieee80211com *ic = vap->iv_ic;
	struct ieee80211_autelan_80211_cache ik;
	struct ieee80211_node_table *nt = &ic->ic_sta; 
	struct ieee80211_node * ni = NULL;	
	...
	switch (ik.type) {
		...
		case SET_ENABLE:
			cache_enable = ik.arg1;
			printk("%s cache_enable = %d\n",__func__,cache_enable);
			break;
		case GET_ENABLE:
			printk("Autelan 80211_cache get_cache_enable %d\n",cache_enable);
			break;		
		case SET_CACHE_ENABLE:
			vap->vap_cache.c_cache_enable= ik.arg1;
			printk("%s vap->vap_cache.c_cache_enable = %d\n",__func__,vap->vap_cache.c_cache_enable);
			break;
		case GET_CACHE_ENABLE:
			copy_to_user(iwr->u.data.pointer, &(vap->vap_cache.c_cache_enable), sizeof(vap->vap_cache.c_cache_enable));
			break;		
		case SET_CACHE_LIMIT:
			vap->vap_cache.c_limit= ik.arg1;
			printk("%s vap->vap_cache.c_protocol = %d\n",__func__,vap->vap_cache.c_limit);
			break;	
		case GET_CACHE_LIMIT:
			copy_to_user(iwr->u.data.pointer, &(vap->vap_cache.c_limit), sizeof(vap->vap_cache.c_limit));
			break;	
		case SET_CACHE_THRESHOLD_B:
			if(ik.arg1 <= vap->vap_cache.c_queue.cq_max_len_b)
			{
				vap->vap_cache.c_send_threshold_b= ik.arg1;
				printk("%s vap->vap_cache.c_send_threshold_b = %d\n",__func__,vap->vap_cache.c_send_threshold_b);
			}
			else
			{
				printk("Error:queue_maxlen must be more than send_threshold_b.\n");					
			}
			break;
		case GET_CACHE_THRESHOLD_B:
			copy_to_user(iwr->u.data.pointer, &(vap->vap_cache.c_send_threshold_b), sizeof(vap->vap_cache.c_send_threshold_b));
			break;
		case SET_CACHE_THRESHOLD_G:
			if(ik.arg1 <= vap->vap_cache.c_queue.cq_max_len_g)
			{
				vap->vap_cache.c_send_threshold_g= ik.arg1;
				printk("%s vap->vap_cache.c_send_threshold_g = %d\n",__func__,vap->vap_cache.c_send_threshold_g);
			}
			else
			{
				printk("Error:queue_maxlen must be more than send_threshold_g.\n");					
			}
			break;
		case GET_CACHE_THRESHOLD_G:
			copy_to_user(iwr->u.data.pointer, &(vap->vap_cache.c_send_threshold_g), sizeof(vap->vap_cache.c_send_threshold_g));
			break;
		case SET_CACHE_THRESHOLD_N:
			if(ik.arg1 <= vap->vap_cache.c_queue.cq_max_len_n)
			{
				vap->vap_cache.c_send_threshold_n= ik.arg1;
				printk("%s vap->vap_cache.c_send_threshold_n = %d\n",__func__,vap->vap_cache.c_send_threshold_n);
			}
			else
			{
				printk("Error:queue_maxlen must be more than send_threshold_n.\n");					
			}
			break;
		case GET_CACHE_THRESHOLD_N:
			copy_to_user(iwr->u.data.pointer, &(vap->vap_cache.c_send_threshold_n), sizeof(vap->vap_cache.c_send_threshold_n));
			break;
		case SET_CACHE_PRIORITY_B:
			vap->vap_cache.c_priority_b= ik.arg1;
			printk("%s vap->vap_cache.c_priority_b = %d\n",__func__,vap->vap_cache.c_priority_b);
			break;		
		case SET_CACHE_PRIORITY_G:
			vap->vap_cache.c_priority_g= ik.arg1;
			printk("%s vap->vap_cache.c_priority_g = %d\n",__func__,vap->vap_cache.c_priority_g);
			break;		
		case SET_CACHE_PRIORITY_N:
			vap->vap_cache.c_priority_n= ik.arg1;
			printk("%s vap->vap_cache.c_priority_n = %d\n",__func__,vap->vap_cache.c_priority_n);
			break;
		case GET_CACHE_PRIORITY_B:			
			copy_to_user(iwr->u.data.pointer, &(vap->vap_cache.c_priority_b), sizeof(vap->vap_cache.c_priority_b));
			break;
		case GET_CACHE_PRIORITY_G:			
			copy_to_user(iwr->u.data.pointer, &(vap->vap_cache.c_priority_g), sizeof(vap->vap_cache.c_priority_g));
			break;
		case GET_CACHE_PRIORITY_N:			
			copy_to_user(iwr->u.data.pointer, &(vap->vap_cache.c_priority_n), sizeof(vap->vap_cache.c_priority_n));
			break;
		case SET_CACHE_QUEUE_MAXLEN_B:
			vap->vap_cache.c_queue.cq_max_len_b= ik.arg1;
			printk("%s vap->vap_cache.c_queue.cq_max_len_b = %d\n",__func__,vap->vap_cache.c_queue.cq_max_len_b);
			break;
		case GET_CACHE_QUEUE_MAXLEN_B:			
			copy_to_user(iwr->u.data.pointer, &(vap->vap_cache.c_queue.cq_max_len_b), sizeof(vap->vap_cache.c_queue.cq_max_len_b));
			break;
		case SET_CACHE_QUEUE_MAXLEN_G:
			vap->vap_cache.c_queue.cq_max_len_g= ik.arg1;
			printk("%s vap->vap_cache.c_queue.cq_max_len_g = %d\n",__func__,vap->vap_cache.c_queue.cq_max_len_g);
			break;
		case GET_CACHE_QUEUE_MAXLEN_G:			
			copy_to_user(iwr->u.data.pointer, &(vap->vap_cache.c_queue.cq_max_len_g), sizeof(vap->vap_cache.c_queue.cq_max_len_g));
			break;		
		case SET_CACHE_QUEUE_MAXLEN_N:
			vap->vap_cache.c_queue.cq_max_len_n= ik.arg1;
			printk("%s vap->vap_cache.c_queue.cq_max_len_n = %d\n",__func__,vap->vap_cache.c_queue.cq_max_len_n);
			break;
		case GET_CACHE_QUEUE_MAXLEN_N:			
			copy_to_user(iwr->u.data.pointer, &(vap->vap_cache.c_queue.cq_max_len_n), sizeof(vap->vap_cache.c_queue.cq_max_len_n));
			break;
		case GET_CACHE_SEND_COUNT:
			
			printf("----------------------------------\r\n");

			printf("interface input all rxpkt = %d\n",all_pkt);			
			printf("input all cached rxpkt = %d\n",cache_all_pkt);

			printf("schedule the task counter = %d\n",cache_schedule_taskcount);
			printf("not schedule the task counter due to timeslice control= %d\n",cache_not_schedule_taskcount);
			printf("size < %d enqueue packet counter = %d\n",cache_pass_factor,cache_pass_count);
			printf("wbuf size > %d && size < %d counter = %d\n",cache_min_size,cache_max_size,cache_size_count);
			printf("----------------------------------\r\n");
			printf("11b current cache status is as below:\r\n");
			printf("enqueue success packet counter = %d\n",cache_11b_all_pkt);
			printf("enqueue drop packet counter due to rate limit = %d\n",cache_11b_enqueue_drop);
			printf("enqueue drop packet counter due to bucket full = %d\n",cache_11b_drop);
			printf("dequeue packet success counter = %d\n",cache_11b_sendcount);
			printf("dequeue overrun the thresh counter = %d\n",cache_11b_send_overrun);
			printf("dequeue not overrun the thresh counter = %d\n",cache_11b_send_not_overrun);
			printf("dequeue bucket deep = %d\n",dequeue_cache_11b_deep);
			

			
			printf("----------------------------------\r\n");
			printf("11g current cache status is as below:\r\n");
			printf("enqueue success packet counter = %d\n",cache_11g_all_pkt);
			printf("enqueue drop packet counter due to rate limit = %d\n",cache_11g_enqueue_drop);
			printf("enqueue drop packet counter due to bucket full = %d\n",cache_11g_drop);
			printf("dequeue packet success counter = %d\n",cache_11g_sendcount);
			printf("dequeue overrun the thresh tasklet counter = %d\n",cache_11g_send_overrun);
			printf("dequeue not overrun the thresh tasklet counter = %d\n",cache_11g_send_not_overrun);
			printf("dequeue bucket deep = %d\n",dequeue_cache_11g_deep);


			printf("----------------------------------\r\n");
			printf("11n current cache status is as below:\r\n");
			printf("enqueue success packet counter = %d\n",cache_11n_all_pkt);
			...
			printf("dequeue bucket deep = %d\n",dequeue_cache_11n_deep);



			printf("------debug information is as below:-------\r\n");
			printf("inter packet 11b last timer usec = %d\n",timerb_last_usec);
			printf("inter packet current timer usec = %d\n",timer_current_usec);

			printf("inter packet 11g last timer usec = %d\n",timerg_last_usec);
			printf("inter packet current timer usec = %d\n",timer_current_usec);

			printf("inter packet 11n last timer usec = %d\n",timern_last_usec);
			printf("inter packet current timer usec = %d\n",timer_current_usec);			
...
static int
ieee80211_ioctl_autelan_wmm(struct net_device *dev, struct iwreq *iwr)
{
	struct ieee80211vap *vap = NETDEV_TO_VAP(dev); //Modified by duanmingzhe device reset when report enable
	...
	switch (ik.type) {
		...
		case GET_TOS_FLAG :
			copy_to_user(iwr->u.data.pointer, &((vap->priv_wmm).tos_flag), sizeof((vap->priv_wmm).tos_flag));
			break;
		...
		case GET_TOS_INGRESS :
			copy_to_user(iwr->u.data.pointer, (vap->priv_wmm).tos_ingress_priority, sizeof((vap->priv_wmm).tos_ingress_priority));
			break;
		...
		case GET_TOS_EGRESS :
			copy_to_user(iwr->u.data.pointer, (vap->priv_wmm).tos_egress_priority, sizeof((vap->priv_wmm).tos_egress_priority));
			break;
		...
		case GET_VLAN_FLAG :
			copy_to_user(iwr->u.data.pointer, &((vap->priv_wmm).vlan_flag), sizeof((vap->priv_wmm).vlan_flag));
			break;
		...
		case GET_VLAN_INGRESS :
			copy_to_user(iwr->u.data.pointer, (vap->priv_wmm).vlan_ingress_priority, sizeof((vap->priv_wmm).vlan_ingress_priority));
			break;
		...
		case GET_VLAN_EGRESS:
			copy_to_user(iwr->u.data.pointer, (vap->priv_wmm).vlan_egress_priority, sizeof((vap->priv_wmm).vlan_egress_priority));
			break;
		...
		case GET_BE_FLAG :
			copy_to_user(iwr->u.data.pointer, &((vap->priv_wmm).be_flag), sizeof((vap->priv_wmm).be_flag));
			break;
		/*ljy--add end*/
...
/* Begin: Added by wangjia, for traffic limit debug. 2012-12-02. */
extern u_int32_t tl_tasklet_timeslice;
extern u_int32_t tl_dequeue_threshold;
extern u_int32_t tl_debug_flag;
/* End: Added by wangjia, for traffic limit debug. 2012-12-02. */

/* Begin: Modified by wangjia, for traffic limit. 2012-12-02. */
static int
...
/* End: Modified by wangjia, for traffic limit. 2012-12-02. */

static int
ieee80211_ioctl_autelan_quick_roaming(struct net_device *dev, struct iwreq *iwr)
{
#if 0
	struct ieee80211vap *vap = NETDEV_TO_VAP(dev); //Modified by duanmingzhe device reset when report enable
	//struct ieee80211com *ic = vap->iv_ic;
	//struct ieee80211_node *ni = NULL ;
	struct ieee80211_autelan_quick_roaming ik;
	...
	switch (ik.type) {
		...
		case SET_FLAG:
			vap->quick_roaming = ik.arg1;
			break;

		case GET_FLAG:
			copy_to_user(iwr->u.data.pointer, &(vap->quick_roaming), sizeof(vap->quick_roaming));
			break;
...
static int
ieee80211_ioctl_autelan_niflag(struct net_device *dev, struct iwreq *iwr)
{
	...
	switch (ik.type) {
		...
		case SET_NIFLAG:
			...
			ni = ieee80211_find_node(&(vap->iv_ic->ic_sta), ik.ni_macaddr);
			if(NULL == ni) {				
				printk("MAC error!\n");
				return -EFAULT;
			}
		
		    ni->ni_localflags = ik.ni_localflags;		    
			ni->ni_portal_ipaddr[0] = ik.ni_portal_ipaddr[0];
			ni->ni_portal_ipaddr[1] = ik.ni_portal_ipaddr[1];
			ni->ni_portal_ipaddr[2] = ik.ni_portal_ipaddr[2];
			ni->ni_portal_ipaddr[3] = ik.ni_portal_ipaddr[3];
		...
		case GET_NIFLAG:
			...
			ik.ni_localflags = ni->ni_localflags;		    
			ik.ni_portal_ipaddr[0] = ni->ni_portal_ipaddr[0];
			ik.ni_portal_ipaddr[1] = ni->ni_portal_ipaddr[1];
			ik.ni_portal_ipaddr[2] = ni->ni_portal_ipaddr[2];
			ik.ni_portal_ipaddr[3] = ni->ni_portal_ipaddr[3];
...
#if 1
static int
ieee80211_ioctl_autelan_mac_binding(struct net_device *dev, struct iwreq *iwr)
{
	struct ieee80211vap *vap = NETDEV_TO_VAP(dev); //Modified by duanmingzhe device reset when report enable
	struct ieee80211com *ic = vap->iv_ic;
	struct ieee80211_node *ni = NULL;
	...
	switch (ik.type) {
		...
		case SET_DHCP_ENABLE:
			vap->vap_dhcp_enable = ik.arg1;
			break;

		case GET_DHCP_ENABLE:
			copy_to_user(iwr->u.data.pointer, &(vap->vap_dhcp_enable), sizeof(vap->vap_dhcp_enable));
			break;

		case SET_IP_AUTO_LEARNING:
			vap->vap_ip_auto_learning = ik.arg1;
			break;

		case GET_IP_AUTO_LEARNING:
			copy_to_user(iwr->u.data.pointer, &(vap->vap_ip_auto_learning), sizeof(vap->vap_ip_auto_learning));
			break;
		...
		case SET_IP:
			...
			if (ni == NULL)
				return -EINVAL;	
			memcpy(ni->ni_ipaddr, ik.ipaddr, sizeof(ni->ni_ipaddr));
			ieee80211_free_node(ni);
			break;
		...
		case GET_IP:
			...
			if (ni == NULL)
				return -EINVAL;	
			memcpy(ik.ipaddr, ni->ni_ipaddr, sizeof(ni->ni_ipaddr));
		...
		case  GET_VLAN:
			...
			if (ni == NULL)
				return -EINVAL;	
			copy_to_user(iwr->u.data.pointer, &(ni->ni_vid), sizeof(ni->ni_vid));
			ieee80211_free_node(ni);
			break;

		//sheyanbin--add to through the pppoe frame
		case SET_PPPOE_ENABLE:
			vap->vap_pppoe_enable = ik.arg1;
			break;

		case GET_PPPOE_ENABLE:
			copy_to_user(iwr->u.data.pointer, &(vap->vap_pppoe_enable), sizeof(vap->vap_pppoe_enable));
			break;
		//sheyanbin--add end
...
/* Begin:gengzj added for suppressing illegal ap 2013-12-27 */
/* zhangyang1 added for rogue ap 2013-12-27 */
static int 
ieee80211_ioctl_autelan_rogue_ap_blacklist(struct net_device *dev, struct iwreq *iwr)
{
	struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
	struct ieee80211com *ic = vap->iv_ic;
	struct ieee80211_autelan_rogue_ap_blacklist ik;
	int i = 0;
	...
	switch (ik.type) {
		case DISPLAY_BLACKLIST:
			ic->ic_autelan_display_blacklist(ic , &ik);
			copy_to_user(iwr->u.data.pointer, &ik, sizeof(ik));
			break;
			
		case ADD_MAC:
			ic->ic_autelan_add_to_blacklist(ic, &ik);
			break;
				
		case DEL_MAC:
			ic->ic_autelan_remove_from_blacklist(ic, &ik);
			break;
			
		case CLR_BLACKLIST:
			ic->ic_autelan_clr_blacklist(ic);
			break;
...
static int ieee80211_ioctl_autelan_display_restrain_table(struct net_device *dev, struct iwreq *iwr)
{
#if 0
	struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
	struct ieee80211com *ic = vap->iv_ic;
	//struct ieee80211_node *ni = NULL;
	struct ieee80211_get_scanning_table ik;
	struct ieee80211_chan_scan_cache *chan = NULL;
	struct rouge_ap_cache *ap = NULL;
	struct rouge_sta_cache *sta = NULL;
	int i = 0;
	u_int8_t *ptr = NULL;
	...
	if (0 == ic->ic_interfere_ap_channel_num) {
		*(int16_t *)ptr = -1;
		return 0;
	}
	else {
		*(int16_t *)ptr = ic->ic_interfere_ap_channel_num;
		ptr += 2;
		for (i=0; i<ic->ic_interfere_ap_channel_num; i++) {
			chan = ic->ic_chan_scan_cache + i;
			*(int16_t *)ptr = chan->chan_num;
			ptr += 2;
			ap = chan->ap_head.next;
			memcpy(ptr, (u_int8_t *)&chan->ap_count, 2);
			ptr += 2;
			while (ap != NULL) {
				memcpy(ptr, ap->ap_bssid, 6);
				ptr += 6;
				memcpy(ptr, (u_int8_t *)&ap->sta_count, 2);
				ptr += 2;
				sta = ap->sta_head.next;
				while (sta != NULL) {
					memcpy(ptr, sta->sta_bssid, 6);
					ptr += 6;
					sta = sta->next;
				}
				ap = ap->next;
...
ieee80211_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
    ...
    case SIOCG80211STATS:
		/*zhaoyang1 transplant from 717*/
		/*pengruofeng add start for management frame stats 2011-5-11*/
		vap->iv_stats.is_rx_err_pack = vap->iv_stats.is_rx_tooshort
...
/* temporary to be removed later, until _getstastats() is available */
	...
	/*zhaoyang1 transplant from 717*/
    /*Begin:Added by pengruofeng for autelan ioctl 2011-6-1*/
    case IEEE80211_IOCTL_AUTELAN_WMM:
       return ieee80211_ioctl_autelan_wmm(dev, (struct iwreq *)ifr);
    case IEEE80211_IOCTL_TRAFFIC_LIMIT:
   {
    ...
    //case IEEE80211_IOCTL_QUICK_ROAMING:
    //    return ieee80211_ioctl_autelan_quick_roaming(dev, (struct iwreq *)ifr);
    case IEEE80211_IOCTL_MAC_BINDING:
        return ieee80211_ioctl_autelan_mac_binding(dev, (struct iwreq *)ifr);
    /*End:Added by pengruofeng for autelan ioctl 2011-6-1*/
	/*zhaoyang1 transplant end*/
    /*Begin:added by duanmingzhe for 80211 cache*/
    case IEEE80211_IOCTL_80211_CACHE:
	return ieee80211_ioctl_autelan_80211_cache(dev, (struct iwreq *)ifr);
    /*End:added by duanmingzhe for 80211 cache*/
	/*zhaoyang add for data forward base users*/
	case IEEE80211_IOCTL_SET_NIFLAG:		
		return ieee80211_ioctl_autelan_niflag(dev, (struct iwreq *)ifr);
	/*zhaoyang modify end*/
    /* Begin: Added by dongzw for mgmt debug */
	...
	/* Begin:gengzj added for suppressing illegal ap 2013-11-27 */
	/*zhaoyang1 add start for interfering ap*/
	case IEEE80211_IOCTL_ROGUE_AP_BLACKLIST:
		return ieee80211_ioctl_autelan_rogue_ap_blacklist(dev, (struct iwreq *)ifr);
	//case IEEE80211_IOCTL_DISPLAY_RESTRAIN_TABLE:
		//return ieee80211_ioctl_autelan_display_restrain_table(dev, (struct iwreq *)ifr);
	/*zhaoyang1 add end*/
	/* End:gengzj added end */
...
//zhengkun modify the function interface
#if  0
...
#endif
static int
ATH_SYSCTL_DECL(ieee80211_sysctl_vap_splitmac, ctl, write, filp, buffer,
	lenp, ppos)
{
...
/*Begin:Modified by zhaoyang1 for optimizing traffic balance based wlan 2013-06-19*/
static int
ATH_SYSCTL_DECL(ieee80211_sysctl_traffic_balance, ctl, write, filp, buffer,
	lenp, ppos)
{
	struct ieee80211vap *vap = ctl->extra1;
	u_int val;
	...
	if (write) {
		ret = ATH_SYSCTL_PROC_DOINTVEC(ctl, write, filp, buffer,
			lenp, ppos);
		if (ret == 0)
			vap->iv_traffic_balance= val;
	} else {
		val = vap->iv_traffic_balance;
		ret = ATH_SYSCTL_PROC_DOINTVEC(ctl, write, filp, buffer,
			lenp, ppos);
	}
	return ret;
}
/*End:Modified by zhaoyang1 for optimizing traffic balance based wlan 2013-06-19*/

/*pengruofeng add for spoofing attack of data frame 2011-5-30*/
static int
ATH_SYSCTL_DECL(ieee80211_sysctl_seq_detect_num, ctl, write, filp, buffer,
       lenp, ppos)
{
       struct ieee80211vap *vap = ctl->extra1;
       ...
       if (write)
		...
		ret = ATH_SYSCTL_PROC_DOINTVEC(ctl, write, filp, buffer,
			lenp, ppos);
		if (ret == 0)
			vap->iv_seq_detect_num= val;
       	  }
    	else
   		 {
              val = vap->iv_seq_detect_num;
...
static int
ATH_SYSCTL_DECL(ieee80211_sysctl_transmit_switch, ctl, write, filp, buffer,
	lenp, ppos)
{
	struct ieee80211vap *vap = ctl->extra1;
	u_int val;
	...
	if (write) {
		ret = ATH_SYSCTL_PROC_DOINTVEC(ctl, write, filp, buffer,
			lenp, ppos);
		if (ret == 0)
			vap->iv_transmit_switch= val;
	} else {
		val = vap->iv_transmit_switch;
		ret = ATH_SYSCTL_PROC_DOINTVEC(ctl, write, filp, buffer,
			lenp, ppos);
	}
	return ret;
...
static int
ATH_SYSCTL_DECL(ieee80211_sysctl_transmit_rate, ctl, write, filp, buffer,
	lenp, ppos)
{
	struct ieee80211vap *vap = ctl->extra1;
	u_int val;
	...
	if (write) {
		ret = ATH_SYSCTL_PROC_DOINTVEC(ctl, write, filp, buffer,
			lenp, ppos);
		if (ret == 0)
			vap->iv_transmit_rate= val;
	} else {
		val = vap->iv_transmit_rate;
		ret = ATH_SYSCTL_PROC_DOINTVEC(ctl, write, filp, buffer,
			lenp, ppos);
	}
	return ret;
...
/*Begin:Added by pengdecai for set maxstaion number 2012-03-06*/
static int
ATH_SYSCTL_DECL(ieee80211_sysctl_set_maxaid, ctl, write, filp, buffer,
	lenp, ppos)
{
	struct ieee80211vap *vap = ctl->extra1;
	u_int val;
	...
	if (write) {
		...
		if (ret == 0) {	
            ...
            if (vap->iv_opmode == IEEE80211_M_HOSTAP) {
	            ...
	            if (val < vap->iv_sta_assoc) {
	                printk("%d station associated with the vap! refuse this request\n", 
	                            vap->iv_sta_assoc);
	                return EINVAL;
	            }

	            vap->iv_max_aid = val + 1;
	            /* The interface is up, we may need to reallocation bitmap(tim, aid) */
	            if (IS_UP(dev)) {
	                error = ieee80211_power_alloc_tim_bitmap(vap);
	                if(!error)
		...
		val = vap->iv_max_aid - 1;
		ret = ATH_SYSCTL_PROC_DOINTVEC(ctl, write, filp, buffer,
			lenp, ppos);
	}
	return ret;
}
/*End:Added by pengdecai for set maxstaion number 2012-03-06*/
...
static const ctl_table ieee80211_sysctl_template[] = {
	...
	{ .ctl_name = CTL_AUTO,
	  .procname = "maxaid",
	  .mode	  = 0644,
	  .proc_handler = ieee80211_sysctl_set_maxaid
	},
	/*End:Added by pengdecai for set maxstaion number 2012-03-06*/
	...
	{ .ctl_name = CTL_AUTO,
	  ...
	  .mode	    = 0644,
	  .proc_handler = ieee80211_sysctl_traffic_balance
	},
	/*End:Modified by zhaoyang1 for optimizing traffic balance based wlan 2013-06-19*/
...
/*Begin:Added by duanmingzhe for capwap*/
struct ieee80211vap * get_vap_from_netdev(struct net_device * dev)  
{
	return NETDEV_TO_VAP(dev);	
}
EXPORT_SYMBOL(get_vap_from_netdev);
struct ieee80211com * get_com_from_netdev(struct net_device * dev)  
{
	return NETDEV_TO_COM(dev);	
}
EXPORT_SYMBOL(get_com_from_netdev);
/*End:Added by duanmingzhe for capwap*/
...
#undef IS_CHAN_HT        
#undef CHANNEL_HT20       
#undef CHANNEL_HT40PLUS   
#undef CHANNEL_HT40MINUS  
/*ended by chenming*/
/*zhaoyang1 transplant end*/
