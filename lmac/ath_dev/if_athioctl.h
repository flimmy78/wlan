...
struct ath_11n_stats {
	...
	u_int32_t   aute_tx_schedule;    /*tx schedule do something*/ //zhaoenjuan transplant for drv_monitor
...
#define ATH_STATS_MAX_INTR_BKT  512
	...
	u_int64_t   ast_be_xmit_bytes;
	u_int64_t   ast_tx_mgmt_bytes;
	...
	/* Begin:gengzongjie transplant for the bug of unalbe to send packets 2013-5-9 */
    u_int32_t   ast_txq_packets[16];            /* perq aggr packets sent on the interface for each category */
	u_int32_t   ast_txq_noaggr_packets[16];     /* perq no-aggr packets sent on the interface for each category */  //zhaoenjuan transplant for drv_monitor
...
typedef enum {
    ...
    ATH_PARAM_RATE_MASK_CAP = 85,
    ATH_PARAM_RATE_MASK_G = 86,
    ATH_PARAM_RATE_MASK_B = 87,
    ATH_PARAM_RATE_MASK_A = 88,
    ATH_PARAM_MCS_MASK = 89,    
    ATH_PARAM_MCS_CAP = 90,
    /*End: Added by zhanghu for increasing rate*/

    /*Begin: Transplant from v5 cmcc, wangjia. And change their values. */
    /*lijiyong add begin for debug cca*/
    ATH_PARAM_CCA_THRESHOLD             = 91,
    ATH_PARAM_CTL_NF                    = 92,
    ATH_PARAM_EXT_NF                    = 93,
    /*lijiyong add end*/

    /*lijiyong add begin for debug noise immunity level*/
    ATH_PARAM_OFDM_NOISE_IMMUNITY_LEVEL = 94,
    ATH_PARAM_CCK_NOISE_IMMUNITY_LEVEL  = 95,
    /*lijiyong add end*/
    /*End: Transplant from v5 cmcc, wangjia.*/
	
    /*Begin: Added by wangjia, for disable NF. */
    ATH_PARAM_DEBUG_NF                  = 96,
    /*End: Added by wangjia, for disable NF. */

	/* AUTELAN-Added-Begin : &tuqiang for openessid */

	ATH_OPENESSID_PARAM                 = 97,
...
#define ATH_OPENESSID_SETBLACKLIST                 (SIOCIWFIRSTPRIV+6)
#define ATH_OPENESSID_DISTROYBLACKLIST             (SIOCIWFIRSTPRIV+7)
#define ATH_OPENESSID_SETWHITELIST                 (SIOCIWFIRSTPRIV+8)
#define ATH_OPENESSID_DISTROYWHITELIST             (SIOCIWFIRSTPRIV+9)
