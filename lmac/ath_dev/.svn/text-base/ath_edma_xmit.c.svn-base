...
ath_tx_edma_tasklet(ath_dev_t dev)
{
    ...
    for (;;) {
        ...
        if (ts.queue_id == sc->sc_bhalq) {
            beacon_complete++;//AUTELAN-zhaoenjuan transplant for drv monitor beacon stuck check
         ...
         * 2. swretry is enabled
         * 3. not aggregation
         * Otherwise, complete the TU
        */
        if (txok || (ath_check_swretry_req(sc, bf) == AH_FALSE) || bf->bf_isaggr || !bf->bf_isdata) {
        ...
        if (an != NULL) {
                ...
                (ts.ts_status || bf->bf_isswretry) && (bf->bf_status & ATH_BUFSTATUS_MARKEDSWRETRY))
            {
                ...
                DPRINTF(sc, ATH_DEBUG_SWR,
                        ...
                        __func__, (*(u_int16_t *)&wh->i_seq[0]) >> 4, ts.ts_ratecode,
                ...
                DPRINTF(sc, ATH_DEBUG_SWR, "%s: SeqNo%d --> status %08X\n",
                        __func__, (*(u_int16_t *)&wh->i_seq[0]) >> 4, ts.ts_status);
...
#endif
                ath_tx_mpdu_resend(sc, txq, &bf_head, ts);
