...
ar9300Set11nRegs(struct ath_hal *ah, HAL_CHANNEL *chan, HAL_HT_MACMODE macmode)
{
	...
	/* EV 62881/64991 - turn off Green Field detection for Apple STA beta */
...
/*begin:wangyu add for xj9344 and xh9344 gpio display abnormal*/
#if !defined ( XJ9344 ) && !defined ( XH9344 )
        REG_WRITE(ATH_GPIO_OE, (REG_READ(ATH_GPIO_OE) | (0x1 << 13)));
#endif
/*end:wangyu add for xj9344 and xh9344 gpio display abnormal*/
...
{
#ifndef AR9300_EMULATION_BB
	/*Beging:Modified by zhaoyang1 for Coverity check 2013-07-01*/
	#if 0
	struct ath_hal_9300 *ahp = AH9300(ah);
    struct ath_hal_private *ahpriv = AH_PRIVATE(ah);
	...
	#endif
	struct ath_hal_9300 *ahp = NULL;
    struct ath_hal_private *ahpriv = NULL;
...
#endif
    /*Begin:Added by duanmingzhe for fix segment error*/
	#if 0
	if(AH_NULL == chan || AH_NULL == ah)
    {		
        return AH_FALSE;
    }
	#endif
    ...
    /*End:Added by duanmingzhe for fix segment error*/
	/*zhaoyang1 transplant end*/
	
	ahp = AH9300(ah);
    ahpriv = AH_PRIVATE(ah);
    ichan = ahpriv->ah_curchan;
    chan = (HAL_CHANNEL *)ichan;
	if (AH_NULL == chan) {
		return AH_FALSE;
	}
	/*End:Modified by zhaoyang1 for Coverity check 2013-07-01*/
...
/*begin:wangyu add for xj9344 and  xh9344 gpio display abnormal*/
#if defined ( XJ9344 ) || defined ( XH9344 )
       REG_WRITE(ATH_GPIO_OUT_FUNCTION3, ( REG_READ(ATH_GPIO_OUT_FUNCTION3) & (~(0xff << 8))));
#else
