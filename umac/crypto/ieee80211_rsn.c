...
ieee80211_set_key(struct ieee80211vap *vap, struct ieee80211_key *k, ieee80211_keyval *kval)
{
	...
	/*Begin:Added by duanmingzhe for authorize ni*/
	if(thinap)
	{
		if(!IEEE80211_IS_MULTICAST(kval->macaddr))
		{
			wlan_node_authorize(vap,1,kval->macaddr);
		}
	}
	/*End:Added by duanmingzhe for authorize ni*/
...
ieee80211_set_keymapping_key(struct ieee80211vap *vap, ieee80211_keyval *kval)
{
    ...
    if (ni == NULL) {
		...
		/*Begin:Modified by zhaoyang1 for fixing the bug of sometime 
		*station can not assoc to AP, in IEEE80211_VERIFY_ADDR ni_bssid 
		*is NULL 2013-07-19
		*/
		IEEE80211_ADDR_COPY(ni->ni_bssid, vap->iv_bss->ni_bssid);
		/*End:Modified by zhaoyang1 for fixing the bug of sometime 
		*station can not assoc to AP, in IEEE80211_VERIFY_ADDR ni_bssid 
		*is NULL 2013-07-19
		*/
    }
