...
/*Begin: Modified by WangJia, for traffic limit. 2012-11-02.*/
/*
...
static void autelan_help () 
{
	...
	/* Begin:gengzj added for suppressing illegal ap 2013-11-27 */
	/*zhaoyang1 add start for interfering ap*/
	printf("\trogue_ap_blacklist\t... ...\n");
	//printf("\tdisplay_restrain_table\t... ...\n");
	/*zhaoyang1 add end*/
	/* End:gengzj added end */
	printf("\n");
	//return 0;
...
/* Begin:gengzj added for suppressing illegal ap 2013-11-27 */
/*zhaoyang1 add start for interfering ap*/
static void autelan_rogue_ap_blacklist_help () 
{
	printf("\nusage:: autelan  rogue_ap_blacklist COMMAND [OPTION] ... \n");
	printf("OPTIONS: \n");	
	printf("\t[interface]\t\tdisplay\t[channel number]\n");
	printf("\t[interface]\t\tclr\n");
	printf("\t[interface]\t\tadd\t\t[mac]\t[channel number]\n");
	printf("\t[interface]\t\tdel\t\t[mac]\t[channel number]\n");
	printf("\n");
}

int display_blacklist (int argc, char** argv)
{
	int i = 0;
	
	if (argc < 5) {
		autelan_rogue_ap_blacklist_help () ;
		return -1;
	} else {

		static u_int32_t buf[1024];
		struct ieee80211_autelan_rogue_ap_blacklist a;
		a.type = DISPLAY_BLACKLIST;
		a.arg1 = atoi(argv[4]);
		memset(buf, 0x00, 1024);
		memcpy(buf, &a, sizeof(struct ieee80211_autelan_rogue_ap_blacklist));
		...
		iwr.u.data.pointer = (void *) buf;
		iwr.u.data.length = sizeof(buf);
		autelan_ioctl(&iwr, IEEE80211_IOCTL_ROGUE_AP_BLACKLIST);
		memcpy(&a, buf, sizeof(struct ieee80211_autelan_rogue_ap_blacklist));

		if (a.arg1 != 0) {
			for (; i<a.arg1; i++) {
				printf("%02X:%02X:%02X:%02X:%02X:%02X\n", 
					a.arg2[i*6], a.arg2[i*6+1], 
					a.arg2[i*6+2], a.arg2[i*6+3], 
					a.arg2[i*6+4], a.arg2[i*6+5]);
			}
		} else {
			printf("Blacklist is empty now\n");
		}
	}
	return 0;
...
int clr_blacklist(int argc, char **argv)
{
		int i = 0;
...
int add_mac(int argc, char** argv)
{
	int n = 0;
	int i = 0;
	...
	if (argc < 6) {
		...
		n = sscanf(argv[4], "%02hhX:%02hhX:%02hhX:%02hhX:%02hhX:%02hhX", a.arg2, a.arg2+1, a.arg2+2, a.arg2+3, a.arg2+4, a.arg2+5);
		if ((n != 6) || (strlen(argv[4]) != 17)) {
			printf("Format of macaddr is wrong\n");
			return 0;
		}
		memset(buf, 0x00, 1024);
		memcpy(buf, &a, sizeof(struct ieee80211_autelan_rogue_ap_blacklist));
...
int del_mac(int argc, char** argv)
{
	int n = 0;
...
int autelan_rogue_ap_blacklist(int argc, char** argv)
{
	...
	if (strncmp (argv[3], "clr", strlen("clr")) == 0) {

		clr_blacklist(argc, argv);

	} else if (strncmp (argv[3], "display", strlen("display")) == 0){

		display_blacklist(argc, argv);

	} else if (strncmp (argv[3], "add", strlen("add")) == 0) { 
			
		add_mac(argc, argv);
		
	} else if (strncmp (argv[3], "del", strlen("del")) == 0) {
...
static void autelan_display_restrain_table_help () 
{
	printf("\nusage:: autelan display_restrain_table  COMMAND [OPTION] ... \n");
	printf("OPTIONS: \n");	
	printf("\t[interface]\t\t\n");
	printf("\n");
}
...
/* Begin:gengzj added for suppressing illegal ap 2013-11-27 */
	/*zhaoyang1 add start for interfering ap*/
	} else if (strncmp (argv[1], "rogue_ap_blacklist", strlen("rogue_ap_blacklist")) == 0) {
		if (autelan_rogue_ap_blacklist(argc, argv) < 0) {

			printf("AUTELAN private commadn rogue_ap_blacklist: wrong format\n");
		}
	//} else if (strncmp (argv[1], "display_restrain_table", strlen("display_restrain_table")) == 0) {
		//if (autelan_display_restrain_table(argc, argv) < 0) {

			//printf("AUTELAN private commadn get_scanning_table: wrong format\n");
		//}
	/*zhaoyang1 add end*/
/* End:gengzj added end */
	} else {
