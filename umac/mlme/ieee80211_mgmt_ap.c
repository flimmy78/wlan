...
extern int ieee80211_verify_openssid(struct ieee80211vap *vap, struct ieee80211_node *ni, char *ssid);
...
ieee80211_recv_asreq(struct ieee80211_node *ni, wbuf_t wbuf, int subtype)
{
	...
	/* gengzongjie transplant rate contral from service provider 2013-4-27 */
    u_int8_t    hc_mcsset[3];          /* supported MCS set  added by zhanghu for incresing rate*/
    ...
    if (frm > efrm) {
        IEEE80211_DISCARD(vap, IEEE80211_MSG_ELEMID,
                          wh, ieee80211_mgt_subtype_name[subtype >>
                                                         IEEE80211_FC0_SUBTYPE_SHIFT],
                          "%s: %s\n", __func__, "reached the end of frame");
		IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
		...
		if (reassoc) {
			...
			vap->iv_stats.is_rx_bad_reassoc++;
		} else {
			
			vap->iv_stats.is_rx_bad_assoc++;
	...
	{
		if(!ieee80211_verify_openssid(vap , ni, ssid))
			return -EINVAL;
	}else{
	 ...
	 IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
        ...
        if (ni) {
	      IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
			...
			if (reassoc) { 
				vap->iv_stats.is_rx_reassoc_notauth++;
			} else { 
    ...
    if ((wpa == NULL) && (RSN_AUTH_IS_WPA(&vap->iv_bss->ni_rsn) || RSN_AUTH_IS_RSNA(&vap->iv_bss->ni_rsn))) {
        ...
        if ((vap->iv_tsn_mode) && (capinfo & IEEE80211_CAPINFO_PRIVACY) && (htcap == NULL)) {
            IEEE80211_NOTE_MAC(vap,
                               ...
                               wh->i_addr2,
                               "%s: TSN: STA caps 0x%x, allow on", __func__, capinfo);
		IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
        ...
        if (vap->iv_wps_mode) {
            ...
            IEEE80211_NOTE_MAC(vap,
                               IEEE80211_MSG_ASSOC | IEEE80211_MSG_WPA,
                               wh->i_addr2,
                              "%s: WPS: allow STA with no WPA/RSN IE on in order to do WPS EAPOL, caps 0x%x", 
                              __func__, capinfo);
		IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
		 ...
		 IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
			...
			vap->iv_stats.is_deauth_unnormal++;
			if (reassoc) {
				vap->iv_stats.is_rx_reassoc_badwpaie++;
			} else {
    ...
    if (wpa != NULL) {
        ...
        if (reason != 0) {
		IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
		  	" <SEND> [Step 05 - SEND DEAUTH] %s: reason = %d\n", 
		  	__func__, reason);
    ...
    if ((capinfo & IEEE80211_CAPINFO_ESS) == 0) { 
        IEEE80211_NOTE_MAC(vap, IEEE80211_MSG_ANY, wh->i_addr2,
                           ...
                           __func__, reassoc ? "reassoc" : "assoc", capinfo);
	IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
		...
		if (reassoc) {
			...
			vap->iv_stats.is_rx_reassoc_capmismatch++;
		} else {
    ...
    if (ni->ni_associd != 0 && 
            ...
            != (capinfo & IEEE80211_CAPINFO_SHORT_SLOTTIME))
        {
            IEEE80211_NOTE_MAC(vap, IEEE80211_MSG_ANY, 
                               wh->i_addr2, "%s: deny %s request, short slot time\
    ...
    if (IEEE80211_VAP_IS_PURE11N_ENABLED(vap) && (htcap == NULL)) {
        IEEE80211_NOTE_MAC(vap, IEEE80211_MSG_ANY, wh->i_addr2,
                           "%s: deny %s request, no ht caps in pure 802.11n mode", 
                           __func__, reassoc ? "reassoc" : "assoc");
	IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
...
/* gengzongjie transplant rate contral from service provider 2013-4-27 */
        /*Begin: added by zhanghu for incresing rate*/
#ifndef  FOR_BIT
    #define FOR_BIT(parm)    (sizeof(parm)*8)
#endif
        if((ic->ic_mcs_mask & (1 << (FOR_BIT(ic->ic_mcs_mask) - 1))) \
            && htcap != NULL)
        {
            /*use the intersect rate*/
            struct ieee80211_ie_htcap_cmn * p_htcap = (struct ieee80211_ie_htcap_cmn *)htcap;
            u_int16_t   i = 0;
            u_int8_t    *mcs = (u_int8_t *)(&(ic->ic_mcs_mask));
            mcs++;
            memcpy(hc_mcsset, p_htcap->hc_mcsset, 3);   //backup
            for (; i < 8*3; i++) {
                if ( (p_htcap->hc_mcsset[i/8] & (1<<(i%8))) \
                    && (mcs[2 - i/8] & (1<<(i%8))) ) {
                    p_htcap->hc_mcsset[i/8] |= (1<<(i%8));
                }else{
                    p_htcap->hc_mcsset[i/8] &= ~(1<<(i%8));
                }
            }
        }
        /*End: added by zhanghu for incresing rate*/
/* gengzongjie transplant end */
        ...
        if (!ieee80211_setup_ht_rates(ni, htcap, 
            ...
            /*Begin: deleted by zhanghu for increasing rate*/
                              //        IEEE80211_F_DOFRATE | IEEE80211_F_DOXSECT | IEEE80211_F_DOBRS)) {
            /*End: deleted by zhanghu for increasing rate*/
            IEEE80211_NOTE_MAC(vap, IEEE80211_MSG_ANY, wh->i_addr2,
                               "%s: deny %s request, ht rate set mismatch", 
                               __func__, reassoc ? "reassoc" : "assoc");
		IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
		...
		/* gengzongjie transplant rate contral from service provider 2013-4-27 */
        /*Start: added by zhanghu for incresing rate*/
        if((ic->ic_mcs_mask & (1 << (FOR_BIT(ic->ic_mcs_mask) - 1))) \
            && htcap != NULL){
            memcpy(((struct ieee80211_ie_htcap_cmn *)htcap)->hc_mcsset, hc_mcsset, 3);
        }
        /*End: added by zhanghu for incresing rate*/
		/* gengzongjie transplant end */
    }
	...
	IEEE80211_NOTE_MGMT_DEBUG(vap, ni, 
			" <RECV> [Step %s - RECV %s REQ] %s: recv %s request frame\n", 
			reassoc ? "04" : "03", reassoc ? "REASSOC" : "ASSOC", __func__, reassoc ? "reassoc" : "assoc");
...
ieee80211_setup_assocresp(struct ieee80211_node *ni, wbuf_t wbuf, u_int8_t reassoc, u_int16_t reason)
{
    ...
    *(u_int16_t *)frm = htole16(reason);
    ...
    if (reason != IEEE80211_STATUS_SUCCESS) {
        ...
        frm = ieee80211_add_rates(frm, &ni->ni_rates, ic);
        frm = ieee80211_add_xrates(frm, &ni->ni_rates, ic);
    }
	...
	else if((ni->ni_rates).rs_nrates==8||(ni->ni_rates).rs_nrates==12){
				ni->ni_mode=1;
	}
                              ...
                              struct ieee80211_rx_status *rs, ieee80211_scan_entry_t  scan_entry)
{
    ...
    if (IEEE80211_IS_CHAN_ANYG(ic->ic_curchan) ||
        ...
        if (ic->ic_protmode != IEEE80211_PROT_NONE && (erp & IEEE80211_ERP_NON_ERP_PRESENT)) {
            if (!IEEE80211_IS_PROTECTION_ENABLED(ic) &&
                ieee80211_vaps_ready(ic, IEEE80211_M_STA) == 0) {
                struct ieee80211vap *tmpvap;
                IEEE80211_DPRINTF(ni->ni_vap, IEEE80211_MSG_INPUT,
                                  "%s: setting protection bit "
                                  "(beacon from %s)\n", __func__, 
   ...
   */
    ...
    if (IEEE80211_IS_CHAN_11N(ic->ic_curchan)) {
        if (!has_ht) {
            if (ieee80211_ic_non_ht_ap_is_clear(ic)) {
                IEEE80211_DPRINTF(ni->ni_vap, IEEE80211_MSG_INPUT,
                                  "%s: setting F_NONHT_AP protection bit "
                                  "(beacon from %s)\n", __func__,
                                  ether_sprintf(wh->i_addr2));
            ...
            if (htinfo) {
                if (htinfo->hi_opmode == IEEE80211_HTINFO_OPMODE_MIXED_PROT_ALL) {
                    if (ieee80211_ic_non_ht_ap_is_clear(ic)) {
                        IEEE80211_DPRINTF(ni->ni_vap, IEEE80211_MSG_INPUT,
                                          "%s: Setting HT protection bit "
                                           "(beacon from %s)\n", __func__,
                                           ether_sprintf(wh->i_addr2));
                        ieee80211_ic_non_ht_ap_set(ic);
                    }
...
ieee80211_recv_pspoll(struct ieee80211_node *ni, wbuf_t wbuf)
{
    ...
    if (ni->ni_associd == 0) {
        IEEE80211_DISCARD(vap,
                          ...
                          "%s: %s", __func__, "unassociated station");
    ...
    if (aid != ni->ni_associd) {
        IEEE80211_DISCARD(vap,
                          IEEE80211_MSG_POWER | IEEE80211_MSG_DEBUG,
                          (struct ieee80211_frame *) wh, "ps-poll",
                          "%s: aid mismatch: sta aid 0x%x poll aid 0x%x", 
                          __func__, ni->ni_associd, aid);
    ...
    if (!ieee80211_node_saveq_send(ni,IEEE80211_FC0_TYPE_DATA)) {
        ...
        IEEE80211_NOTE_MAC(vap, IEEE80211_MSG_POWER, wh->i_addr2,
                           "%s: %s", __func__, "recv ps-poll, but queue empty");
